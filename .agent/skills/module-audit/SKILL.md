...
- **Release Gate**: 被 `version-release-protocol` Step 0 Gate 0.5 调用（使用 Release Checkpoint Mode）。

### Release Checkpoint Mode（发布检查点模式）

当被 `version-release-protocol` 调用时，**必须切换到此模式**。此模式与默认模式的区别：

|   维度   | 默认模式            | Release Checkpoint Mode                 |
| :------: | :------------------ | :-------------------------------------- |
| **目标** | 发现问题 → 修复问题 | 发现问题 → **仅报告**                   |
| **轮次** | ≥ 5 轮迭代          | **1 轮快照**                            |
| **整改** | 默认全部整改        | **禁止整改**（发布前整改 = 引入新风险） |
| **输出** | 审计报告 + 整改代码 | **Pass/Fail 门禁报告**                  |
| **范围** | 单个模块全维度      | **仅扫描本次变更涉及的模块**            |

**Release Checkpoint 执行步骤**：

1. 通过 `git diff` 识别本次变更涉及的模块
2. 对每个变更模块执行 **1 轮** 八维扫描（不执行整改）
3. 按严重级别分类问题：
   - 🔴 **Critical**：安全漏洞(D3)、DB 破坏性变更(D4)、认证绕过 → **返回 FAIL**
   - 🟠 **High**：核心逻辑缺陷(D2)、测试缺失(D6) → **记录但不阻断**
   - 🟡 **Medium/Low**：文档、命名、日志等 → **记录但不阻断**
4. 生成 Pass/Fail 报告返回给 `version-release-protocol`

**门禁红线**：存在任何 🔴 Critical 级问题 → 返回 **FAIL**，阻断发布。

> **重要**：Release Checkpoint Mode 下发现的 High/Medium 问题应记录到 `docs/03-audits/release-notes/` 中，作为下一迭代的改进待办。
...
- **Missing in Doc**: Code implements feature X, but doc is silent. -> _Action: Update Doc_.
- **Missing in Code**: Doc requires feature Y, but code is missing. -> _Action: Plan Implementation_.
- **Mismatch**: Doc says A, Code does B. -> _Action: Flag for Decision_.
