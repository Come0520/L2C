...
| 0.2.2 | 迁移文件完整        | `drizzle/` 中迁移编号连续，无跳号                    | `ls drizzle/ \| sort`                                   |
| 0.2.3 | 破坏性变更          | 删列/改类型需用户书面确认                            | 人工确认                                                |
| 0.2.4 | 禁止旁路数据库操作  | 确认本次未使用 GUI 工具或 `db:push` 修改过数据库结构 | 主动询问用户                                            |
| 0.2.5 | Enum 字段脏数据风险 | 若有 Enum 字段变更，确认不存在非法历史数据           | 必要时查询 `SELECT DISTINCT <col> FROM <table>`         |

🚫 **硬性门禁**：0.2.1 未执行 → 中止发布
🚫 **硬性门禁**：0.2.4 发现旁路操作 → 中止发布（必须先通过 `db:generate` + `db:migrate` 修复同步状态）
