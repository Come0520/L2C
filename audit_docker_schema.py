...
def main():
    print("正在查询Docker数据库的表结构...\n")
    docker_schema = get_docker_schema()
    
    print(f"✓ 查询完成，找到 {len(docker_schema)} 张表\n")
    
    save_schema(docker_schema, 'docker_schema.json')
    print("✓ Docker数据库结构已保存到 docker_schema.json\n")
    
    print("表列表:")
    for idx, table_name in enumerate(sorted(docker_schema.keys()), 1):
        table_info = docker_schema[table_name]
