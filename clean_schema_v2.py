...
def clean_schema_sql_advanced():
    input_file = "C:/Users/bigey/Documents/Antigravity/L2C/alibabacloud-rds-openapi-mcp-server/schema.sql"
    output_file = "C:/Users/bigey/Documents/Antigravity/L2C/schema_cleaned_v2.sql"
    
    print("=" * 60)
    print("高级清理 Schema SQL 文件")
    print("=" * 60)
    ...
    for line in lines:
        ...
        # 处理 ALTER TABLE ADD CONSTRAINT 语句，添加 IF NOT EXISTS
        if 'ADD CONSTRAINT' in line.upper() and 'IF NOT EXISTS' not in line.upper():
            line = re.sub(r'ADD CONSTRAINT', 'ADD CONSTRAINT IF NOT EXISTS', line, flags=re.IGNORECASE)
    ...
    print("  - 使用 IF NOT EXISTS 避免重复创建")
    print("  - 使用 IF NOT EXISTS 避免重复约束")

if __name__ == "__main__":
    clean_schema_sql_advanced()
