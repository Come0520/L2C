...
def clean_schema_sql_v5():
    input_file = "C:/Users/bigey/Documents/Antigravity/L2C/alibabacloud-rds-openapi-mcp-server/schema.sql"
    output_file = "C:/Users/bigey/Documents/Antigravity/L2C/schema_cleaned_v5.sql"
    
    print("=" * 60)
    print("智能清理 Schema SQL 文件 V5")
    print("=" * 60)
    ...
    for line in lines:
        ...
        # 对于 CREATE TYPE，不使用 IF NOT EXISTS（PostgreSQL 不支持）
        # 而是包装在 DO 块中
        if 'CREATE TYPE' in line.upper() and 'IF NOT EXISTS' not in line.upper():
            # 提取类型名
            type_match = re.search(r'CREATE TYPE\s+(\w+)', line, re.IGNORECASE)
            if type_match:
                type_name = type_match.group(1)
                # 提取完整的 CREATE TYPE 语句
                create_match = re.match(r'(CREATE TYPE.*?AS)', line, re.IGNORECASE)
                if create_match:
                    create_part = create_match.group(1)
                    type_part = line[create_match.end():]
                    # 创建 DO 块
                    do_block = f"""DO $$
BEGIN
    IF NOT EXISTS (
...
END $$;"""
    ...
    print("  - 使用 IF NOT EXISTS 避免重复创建表、schema")
    print("  - 使用 DO 块避免重复创建类型和外键约束")
