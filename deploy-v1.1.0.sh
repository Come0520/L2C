...
ssh $ECS_USER@$ECS_IP << 'ENDSSH'
    ...
    # 切换到v1.1.0
    echo "切换到v1.1.0..."
    # 尝试切换到已存在的 release-v1.1.0 分支，如果不存在则从 tag 创建
    if git show-ref --verify --quiet refs/heads/release-v1.1.0; then
        git checkout release-v1.1.0
        git pull origin main || true # 确保从 main 合并最新更改（如果 release 分支已存在但滞后）
        git reset --hard v1.1.0 # 强制重置到 v1.1.0 tag
    else
