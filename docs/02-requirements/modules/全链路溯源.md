...
| 字段名 | 类型 | 说明 |
|:---|:---|:---|
| batch_no | VARCHAR(50) | 面料缸号 (与供应链模块关联) |
| roll_no | VARCHAR(50) | 面料卷号 (与供应链模块关联) |
...
#### 2.3.1 证据链数据结构
证据链用于记录从订单到售后的完整证据信息,包括量尺图、生产单、安装前后照片等。
...
| 字段名 | 类型 | 必填 | 说明 |
|:---|:---|:---:|:---|
| measure_report | Object | 否 | 量尺报告,包含报告链接和日期 |
| production_sheet | Object | 否 | 生产单,包含链接和批次号 |
| install_photos_before | String[] | 否 | 安装前照片数组 |
| install_photos_after | String[] | 否 | 安装后照片数组 |
| issue_photos | String[] | 是 | 问题照片数组 |

#### 2.3.3 证据链存储
证据链以 JSONB 格式存储在责任认定单的 `linked_evidence` 字段中,便于前端渲染和查询。
...
JOIN orders o ON po.order_id = o.id
LEFT JOIN after_sales aso ON o.id = aso.order_id AND aso.issue_type = 'QUALITY_DEFECT'
WHERE poi.batch_no = :batch_no
   OR poi.roll_no = :roll_no
GROUP BY poi.batch_no, poi.roll_no, po.id, po.po_no, po.supplier_id, po.supplier_name;
```
...
### 3.4 熔断机制 (Circuit Breaker)

#### 3.4.1 熔断触发条件
当特定供应商或批次的预警级别达到 `HIGH` 时,系统自动触发熔断机制。

| 触发条件 | 预警级别 | 熔断对象 |
|:---|:---:|:---|
| 供应商售后率 > 20% | HIGH | 供应商 |
| 缸号/卷号售后率 > 30% | HIGH | 批次 |
| 同一供应商连续 3 个采购单售后率 > 15% | HIGH | 供应商 |

#### 3.4.2 熔断动作
1. **自动暂停采购/销售**
   - 调用供应链 API,将对应供应商状态置为 `SUSPENDED`
   - 调用商品 API,将对应 SKU 状态置为 `SUSPENDED`

2. **发送紧急通知**
   - 通知级别: `URGENT`
   - 通知对象: 采购主管、质检主管
   - 通知内容: 包含触发原因、影响范围、建议措施
...
interface CircuitBreakerEvent {
  id: string;
  trigger_type: 'SUPPLIER' | 'BATCH';
  trigger_id: string;
  trigger_reason: string;
  alert_level: 'LOW' | 'MEDIUM' | 'HIGH';
  action_taken: 'SUSPEND_PURCHASE' | 'SUSPEND_SALE' | 'NOTIFY';
  triggered_at: Date;
  triggered_by: string;
