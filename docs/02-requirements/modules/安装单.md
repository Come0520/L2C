...
*   **售后返工创建**: 售后工单定责为"返工"时，系统自动生成售后返工安装单，并关联售后工单ID
...
| 当前状态 | 可驳回至 | 触发场景 | 操作人 |
|:---|:---|:---|:---|
| **待确认** | 待上门 | 验收不通过，要求返工 | 销售 |
| **待上门** | 分配中 | 师傅临时无法上门 | 师傅/派单员 |
| **分配中** | 待分配 | 师傅拒绝接单 | 师傅/派单员 |
...
*   只有当所有关联的采购单（面料、轨道、辅料）均显示"已签收"或"已到货"，系统才允许派单员执行"指派"动作
*   `logistics_ready_status` 字段实时更新，反映货齐状态

**校验逻辑**:
```typescript
async function checkLogisticsReady(installTaskId: string): Promise<boolean> {
  const installTask = await getInstallTask(installTaskId);
  const order = await getOrder(installTask.order_id);
