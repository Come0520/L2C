...
async function checkSchedulingConflict(
  ...
  if (previousTask) {
    ...
    const timeInterval = calculateTimeInterval(
      previousTask.scheduled_time_slot,
      timeSlotRange.start
    );
...
```typescript
async function getInstallerAvailableSlots(installerId: string, date: Date): Promise<string[]> {
  // 获取师傅当日所有任务
...
```typescript
// 检查货齐状态
export async function checkLogisticsReady(installTaskId: string): Promise<boolean> {
  const installTask = await getInstallTask(installTaskId);
...
export async function calculateLaborFee(
  ...
  const additionalFees = breakdown.additional_fees.reduce((sum, fee) => sum + fee.amount, 0);
...
export async function completeChecklist(
  ...
  const allCompleted = checklistStatus.items.every((item) => item.completed);
...
// 生成约时链接
export async function generateAppointmentLink(installTaskId: string): Promise<string> {
  const token = generateSecureToken(installTaskId);
  return `https://your-domain.com/appointment/${token}`;
}

// 安装单完成，生成劳务结算
export async function completeInstallTask(installTaskId: string): Promise<void> {
  const installTask = await getInstallTask(installTaskId);
...
// 创建劳务结算
async function createLaborSettlement(installTask: InstallTask): Promise<void> {
