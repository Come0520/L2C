    ...
    for (let i = 0; i < allWorkers.length; i++) {
      for (let j = i + 1; j < allWorkers.length; j++) {
        ...
        if (categories1.length > 0 && categories2.length > 0) {
          const cost = await calculateTotalCost(worker1.id, categories1) + 
...
```typescript
// 派单员合并品类
async function mergeCategories(planId: string, workerId: string, categories: string[]): Promise<void> {
  ...
  // 更新拆单方案
  await db.update(quoteSplitPlans)
    .set({
      worker_ids: [workerId],
      total_cost: newCost,
      worker_count: 1,
...
```typescript
// 确认拆单方案并生成测量任务
async function confirmSplitPlan(quoteId: string, planId: string, dispatcherId: string): Promise<void> {
  const quote = await db.query.quotes.findFirst({
    where: eq(quotes.id, quoteId),
    with: {
      items: true
