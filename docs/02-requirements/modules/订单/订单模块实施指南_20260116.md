# 订单模块实施指南

**版本**: v1.0  
**创建时间**: 2026-01-16  
**基于文档**: [订单模块整改计划_20260116.md](./订单模块整改计划_20260116.md)  
**目标读者**: 后端开发、前端开发、QA测试、技术Leader

---

## 📋 目录

1. [总体概述](#1-总体概述)
2. [Week 1: Schema变更 + 快照机制](#week-1-schema变更--快照机制)
3. [Week 2: 变更单流程](#week-2-变更单流程)
4. [Week 3: 智能拆单逻辑](#week-3-智能拆单逻辑)
5. [Week 4: 发货与物流流程](#week-4-发货与物流流程)
6. [Week 5: 叫停机制 + P1功能](#week-5-叫停机制--p1功能)
7. [Week 6: 测试与上线准备](#week-6-测试与上线准备)
8. [验收标准](#验收标准)
9. [风险应对](#风险应对)

---

## 1. 总体概述

### 1.1 整改目标

将订单模块完成度从 **35%** 提升至 **90%+**，实现生产级别的订单管理能力。

### 1.2 分阶段目标

| 周次 | 目标完成度 | 核心里程碑 |
|:---:|:---:|:---|
| Week 1 | 45% | 完成Schema变更 + 快照机制 |
| Week 2 | 55% | 完成变更单基础流程 |
| Week 3 | 65% | 完成智能拆单算法 |
| Week 4 | 75% | 完成发货与物流流程 |
| Week 5 | 85% | 完成P1功能 + 前端优化 |
| Week 6 | 90%+ | 完成测试与上线准备 |

### 1.3 关键交付物

**后端交付物**:
- 数据库Schema迁移文件
- Service层业务逻辑实现
- 18个核心API接口
- 单元测试代码

**前端交付物**:
- 订单列表页(含筛选分页)
- 订单详情页(Tab布局)
- 拆单页面
- 变更申请Dialog
- 发货相关Dialog

**文档交付物**:
- API文档更新
- 数据库Schema文档
- 使用手册

---

## 2. Week 1: Schema变更 + 快照机制 (P0)

**目标**: 完成数据库Schema变更并实现订单快照机制

### 2.1 数据库Schema变更 (3天)

#### 2.1.1 订单主表字段补充 (Day 1)

**文件**: `src/shared/api/schema/orders.ts`

**新增字段**:
```typescript
export const orders = pgTable('orders', {
  // ... 现有字段
  
  // 新增字段
  snapshotData: jsonb('snapshot_data').notNull(),
  haltedReason: text('halted_reason'),
  haltedAt: timestamp('halted_at'),
  cancelReason: text('cancel_reason'),
  cancelledBy: uuid('cancelled_by').references(() => users.id),
  cancelledAt: timestamp('cancelled_at'),
  lockedBy: uuid('locked_by').references(() => users.id),
  confirmationDeadline: timestamp('confirmation_deadline'),
});
```

**扩展OrderStatus枚举**:
```typescript
export const orderStatusEnum = pgEnum('order_status', [
  'PENDING_CONFIRMATION',  // 新增
  'PENDING_PO',
  'PENDING_PRODUCTION',
  'PENDING_DELIVERY',
  'PENDING_SHIPMENT',
  'SHIPPED',
  'DELIVERED',
  'COMPLETED',
  'HALTED',  // 新增
  'CANCELLED',
]);
```

**验收标准**:
- [ ] 所有新字段已添加到Schema定义
- [ ] OrderStatus枚举包含所有13个状态
- [ ] TypeScript类型定义正确
- [ ] 字段注释完整

#### 2.1.2 订单明细表字段补充 (Day 1.5)

**文件**: `src/shared/api/schema/order-items.ts`

**新增字段**:
```typescript
export const orderItems = pgTable('order_items', {
  // ... 现有字段
  
  // 新增字段
  supplierId: uuid('supplier_id').references(() => suppliers.id),
  purchaseOrderId: uuid('purchase_order_id').references(() => purchaseOrders.id),
  deliveryStatus: text('delivery_status'),  // PENDING, SHIPPED, DELIVERED
  deliveredAt: timestamp('delivered_at'),
});
```

**验收标准**:
- [ ] 所有新字段已添加到Schema定义
- [ ] 外键关联正确
- [ ] 字段注释完整

#### 2.1.3 创建变更单表 (Day 2)

**文件**: `src/shared/api/schema/change-requests.ts`

**完整Schema**:
```typescript
import { pgTable, uuid, text, timestamp, decimal, jsonb } from 'drizzle-orm/pg-core';
import { users } from './users';
import { orders } from './orders';

export const changeRequests = pgTable('change_requests', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').notNull(),
  orderId: uuid('order_id').notNull().references(() => orders.id, { onDelete: 'cascade' }),
  
  // 变更信息
  changeType: text('change_type').notNull(),  // ADD_ITEM, REMOVE_ITEM, MODIFY_ITEM
  changeReason: text('change_reason').notNull(),
  originalItems: jsonb('original_items').notNull(),
  newItems: jsonb('new_items').notNull(),
  priceDifference: decimal('price_difference', { precision: 10, scale: 2 }),
  
  // 审批信息
  status: text('status').notNull().default('PENDING'),  // PENDING, APPROVED, REJECTED
  approvedBy: uuid('approved_by').references(() => users.id),
  approvedAt: timestamp('approved_at'),
  rejectedBy: uuid('rejected_by').references(() => users.id),
  rejectedAt: timestamp('rejected_at'),
  rejectionReason: text('rejection_reason'),
  
  // 审计字段
  createdBy: uuid('created_by').notNull().references(() => users.id),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
});

// 索引
export const changeRequestsIndexOrderId = index('idx_change_requests_order_id').on(changeRequests.orderId);
export const changeRequestsIndexStatus = index('idx_change_requests_status').on(changeRequests.status);
```

**验收标准**:
- [ ] changeRequests表Schema定义完整
- [ ] 外键关联正确
- [ ] 索引已创建
- [ ] 字段注释完整

#### 2.1.4 性能索引创建 (Day 2.5)

**文件**: `drizzle/migrations/XXXX_order_module_enhancement.sql`

**SQL语句**:
```sql
-- 订单表索引
CREATE INDEX idx_orders_status ON orders(status);
CREATE INDEX idx_orders_customer_id ON orders(customer_id);
CREATE INDEX idx_orders_created_at ON orders(created_at DESC);
CREATE INDEX idx_orders_tenant_id ON orders(tenant_id);

-- 订单明细表索引
CREATE INDEX idx_order_items_supplier_id ON order_items(supplier_id);
CREATE INDEX idx_order_items_po_id ON order_items(purchase_order_id);
CREATE INDEX idx_order_items_order_id ON order_items(order_id);

-- 变更单表索引
CREATE INDEX idx_change_requests_order_id ON change_requests(order_id);
CREATE INDEX idx_change_requests_status ON change_requests(status);
CREATE INDEX idx_change_requests_tenant_id ON change_requests(tenant_id);
```

**验收标准**:
- [ ] 所有索引已创建
- [ ] 索引命名规范
- [ ] 复合索引顺序合理(高选择性字段在前)

#### 2.1.5 迁移脚本编写与测试 (Day 3)

**文件**: `drizzle/migrations/XXXX_order_module_enhancement.sql`

**完整迁移脚本**:
```sql
-- ============================================
-- 订单模块增强迁移脚本
-- 版本: 20260116
-- 作者: 后端开发团队
-- ============================================

BEGIN;

-- 1. 扩展OrderStatus枚举
CREATE TYPE order_status_new AS ENUM (
  'PENDING_CONFIRMATION',
  'PENDING_PO',
  'PENDING_PRODUCTION',
  'PENDING_DELIVERY',
  'PENDING_SHIPMENT',
  'SHIPPED',
  'DELIVERED',
  'COMPLETED',
  'HALTED',
  'CANCELLED'
);

-- 迁移现有数据
ALTER TABLE orders ALTER COLUMN status TYPE order_status_new 
  USING status::text::order_status_new;

DROP TYPE order_status;
ALTER TYPE order_status_new RENAME TO order_status;

-- 2. 订单主表新增字段
ALTER TABLE orders ADD COLUMN snapshot_data JSONB NOT NULL DEFAULT '{}';
ALTER TABLE orders ADD COLUMN halted_reason TEXT;
ALTER TABLE orders ADD COLUMN halted_at TIMESTAMP;
ALTER TABLE orders ADD COLUMN cancel_reason TEXT;
ALTER TABLE orders ADD COLUMN cancelled_by UUID REFERENCES users(id);
ALTER TABLE orders ADD COLUMN cancelled_at TIMESTAMP;
ALTER TABLE orders ADD COLUMN locked_by UUID REFERENCES users(id);
ALTER TABLE orders ADD COLUMN confirmation_deadline TIMESTAMP;

-- 3. 订单明细表新增字段
ALTER TABLE order_items ADD COLUMN supplier_id UUID REFERENCES suppliers(id);
ALTER TABLE order_items ADD COLUMN purchase_order_id UUID REFERENCES purchase_orders(id);
ALTER TABLE order_items ADD COLUMN delivery_status TEXT DEFAULT 'PENDING';
ALTER TABLE order_items ADD COLUMN delivered_at TIMESTAMP;

-- 4. 创建变更单表
CREATE TABLE change_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  order_id UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
  
  change_type TEXT NOT NULL CHECK (change_type IN ('ADD_ITEM', 'REMOVE_ITEM', 'MODIFY_ITEM')),
  change_reason TEXT NOT NULL,
  original_items JSONB NOT NULL,
  new_items JSONB NOT NULL,
  price_difference DECIMAL(10,2),
  
  status TEXT NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'APPROVED', 'REJECTED')),
  approved_by UUID REFERENCES users(id),
  approved_at TIMESTAMP,
  rejected_by UUID REFERENCES users(id),
  rejected_at TIMESTAMP,
  rejection_reason TEXT,
  
  created_by UUID NOT NULL REFERENCES users(id),
  created_at TIMESTAMP DEFAULT now(),
  updated_at TIMESTAMP DEFAULT now()
);

-- 5. 创建索引
CREATE INDEX idx_orders_status ON orders(status);
CREATE INDEX idx_orders_customer_id ON orders(customer_id);
CREATE INDEX idx_orders_created_at ON orders(created_at DESC);
CREATE INDEX idx_orders_tenant_id ON orders(tenant_id);

CREATE INDEX idx_order_items_supplier_id ON order_items(supplier_id);
CREATE INDEX idx_order_items_po_id ON order_items(purchase_order_id);
CREATE INDEX idx_order_items_order_id ON order_items(order_id);

CREATE INDEX idx_change_requests_order_id ON change_requests(order_id);
CREATE INDEX idx_change_requests_status ON change_requests(status);
CREATE INDEX idx_change_requests_tenant_id ON change_requests(tenant_id);

-- 6. 创建触发器自动更新updated_at
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
$$ language 'plpgsql';

CREATE TRIGGER update_change_requests_updated_at 
  BEFORE UPDATE ON change_requests 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

COMMIT;
```

**回滚脚本**:
```sql
-- ============================================
-- 订单模块增强回滚脚本
-- ============================================

BEGIN;

-- 删除触发器
DROP TRIGGER IF EXISTS update_change_requests_updated_at ON change_requests;

-- 删除索引
DROP INDEX IF EXISTS idx_change_requests_tenant_id;
DROP INDEX IF EXISTS idx_change_requests_status;
DROP INDEX IF EXISTS idx_change_requests_order_id;
DROP INDEX IF EXISTS idx_order_items_order_id;
DROP INDEX IF EXISTS idx_order_items_po_id;
DROP INDEX IF EXISTS idx_order_items_supplier_id;
DROP INDEX IF EXISTS idx_orders_tenant_id;
DROP INDEX IF EXISTS idx_orders_created_at;
DROP INDEX IF EXISTS idx_orders_customer_id;
DROP INDEX IF EXISTS idx_orders_status;

-- 删除变更单表
DROP TABLE IF EXISTS change_requests;

-- 删除订单明细表新增字段
ALTER TABLE order_items DROP COLUMN IF EXISTS delivered_at;
ALTER TABLE order_items DROP COLUMN IF EXISTS delivery_status;
ALTER TABLE order_items DROP COLUMN IF EXISTS purchase_order_id;
ALTER TABLE order_items DROP COLUMN IF EXISTS supplier_id;

-- 删除订单主表新增字段
ALTER TABLE orders DROP COLUMN IF EXISTS confirmation_deadline;
ALTER TABLE orders DROP COLUMN IF EXISTS locked_by;
ALTER TABLE orders DROP COLUMN IF EXISTS cancelled_at;
ALTER TABLE orders DROP COLUMN IF EXISTS cancelled_by;
ALTER TABLE orders DROP COLUMN IF EXISTS cancel_reason;
ALTER TABLE orders DROP COLUMN IF EXISTS halted_at;
ALTER TABLE orders DROP COLUMN IF EXISTS halted_reason;
ALTER TABLE orders DROP COLUMN IF EXISTS snapshot_data;

-- 恢复OrderStatus枚举
CREATE TYPE order_status_old AS ENUM (
  'PENDING_PO',
  'PENDING_PRODUCTION',
  'PENDING_DELIVERY',
  'PENDING_SHIPMENT',
  'SHIPPED',
  'DELIVERED',
  'COMPLETED',
  'CANCELLED'
);

ALTER TABLE orders ALTER COLUMN status TYPE order_status_old 
  USING status::text::order_status_old;

DROP TYPE order_status;
ALTER TYPE order_status_old RENAME TO order_status;

COMMIT;
```

**验收标准**:
- [ ] 迁移脚本在本地测试通过
- [ ] 回滚脚本测试通过
- [ ] 数据完整性验证通过
- [ ] 无数据丢失

### 2.2 订单快照机制实现 (2天)

#### 2.2.1 Service层改造 (Day 4)

**文件**: `src/services/order.service.ts`

**修改 `convertFromQuote` 方法**:
```typescript
import { db } from '@/shared/db';
import { orders, orderItems } from '@/shared/api/schema/orders';
import { quotes, quoteItems } from '@/shared/api/schema/quotes';

export class OrderService {
  /**
   * 从报价单创建订单
   * @param quoteId 报价单ID
   * @param options 创建选项
   * @returns 创建的订单ID
   */
  async convertFromQuote(
    quoteId: string,
    options: {
      paymentAmount?: number;
      paymentMethod?: 'CASH' | 'WECHAT' | 'ALIPAY' | 'BANK';
      paymentTime?: Date;
      paymentProofImg?: string;
      confirmationImg?: string;
    } = {}
  ): Promise<string> {
    return await db.transaction(async (tx) => {
      // 1. 获取报价单数据
      const [quote] = await tx
        .select()
        .from(quotes)
        .where(eq(quotes.id, quoteId))
        .limit(1);

      if (!quote) {
        throw new Error('报价单不存在');
      }

      if (quote.status !== 'WON') {
        throw new Error('仅WON状态的报价单可转订单');
      }

      // 2. 检查是否已创建订单
      const [existingOrder] = await tx
        .select()
        .from(orders)
        .where(eq(orders.quoteId, quoteId))
        .limit(1);

      if (existingOrder) {
        throw new Error('该报价单已创建订单');
      }

      // 3. 获取报价明细
      const items = await tx
        .select()
        .from(quoteItems)
        .where(eq(quoteItems.quoteId, quoteId));

      // 4. 生成快照数据
      const snapshotData = this.generateSnapshot(quote, items);

      // 5. 确定初始状态
      const initialStatus = quote.hasDeepDesign 
        ? 'PENDING_CONFIRMATION'
        : 'PENDING_PO';

      // 6. 推断结算方式
      const settlementType = this.inferSettlementType(
        quote.totalAmount,
        options.paymentAmount
      );

      // 7. 创建订单
      const [newOrder] = await tx
        .insert(orders)
        .values({
          orderNo: await this.generateOrderNo(),
          quoteId: quote.id,
          quoteVersionId: quote.versionId,
          leadId: quote.leadId,
          customerId: quote.customerId,
          customerName: quote.customerName,
          customerPhone: quote.customerPhone,
          deliveryAddress: quote.deliveryAddress,
          status: initialStatus,
          totalAmount: quote.totalAmount,
          paidAmount: options.paymentAmount || 0,
          settlementType,
          confirmationImg: options.confirmationImg,
          paymentProofImg: options.paymentProofImg,
          paymentAmount: options.paymentAmount,
          paymentMethod: options.paymentMethod,
          paymentTime: options.paymentTime,
          salesId: quote.salesId,
          snapshotData: JSON.stringify(snapshotData),
          confirmationDeadline: quote.hasDeepDesign 
            ? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // 7天后
            : null,
        })
        .returning();

      // 8. 创建订单明细
      const orderItemValues = items.map(item => ({
        orderId: newOrder.id,
        quoteItemId: item.id,
        roomName: item.roomName,
        productId: item.productId,
        productName: item.productName,
        category: item.category,
        unitPrice: item.unitPrice,
        quantity: item.quantity,
        width: item.width,
        height: item.height,
        subtotal: item.subtotal,
        status: 'PENDING',
      }));

      await tx.insert(orderItems).values(orderItemValues);

      // 9. 更新报价单状态为LOCKED
      await tx
        .update(quotes)
        .set({ status: 'LOCKED' })
        .where(eq(quotes.id, quoteId));

      // 10. 创建AR对账单
      await this.createARStatement(tx, newOrder);

      return newOrder.id;
    });
  }

  /**
   * 生成订单快照数据
   * @param quote 报价单
   * @param items 报价明细
   * @returns 快照数据
   */
  private generateSnapshot(
    quote: any,
    items: any[]
  ): any {
    return {
      quote: {
        id: quote.id,
        quoteNo: quote.quoteNo,
        versionId: quote.versionId,
        totalAmount: quote.totalAmount,
        hasDeepDesign: quote.hasDeepDesign,
        createdAt: quote.createdAt,
        items: items.map(item => ({
          id: item.id,
          roomName: item.roomName,
          productId: item.productId,
          productName: item.productName,
          category: item.category,
          unitPrice: item.unitPrice,
          quantity: item.quantity,
          width: item.width,
          height: item.height,
          subtotal: item.subtotal,
        })),
      },
      customer: {
        id: quote.customerId,
        name: quote.customerName,
        phone: quote.customerPhone,
        address: quote.deliveryAddress,
      },
      snapshotTime: new Date().toISOString(),
    };
  }

  /**
   * 推断结算方式
   * @param totalAmount 订单总金额
   * @param paidAmount 已付金额
   * @returns 结算方式
   */
  private inferSettlementType(
    totalAmount: number,
    paidAmount?: number
  ): 'PREPAID' | 'MONTHLY' {
    const paid = Number(paidAmount || 0);
    const total = Number(totalAmount || 0);

    // 预付款 >= 30% 则为预付
    if (paid >= total * 0.3) {
      return 'PREPAID';
    }

    return 'MONTHLY';
  }

  /**
   * 生成订单号
   * @returns 订单号 (OD20260116001)
   */
  private async generateOrderNo(): Promise<string> {
    const today = new Date();
    const dateStr = today.toISOString().slice(0, 10).replace(/-/g, '');
    
    // 查询今日订单数量
    const [result] = await db
      .select({ count: sql<number>`count(*)` })
      .from(orders)
      .where(
        and(
          sql`DATE(created_at) = CURRENT_DATE`,
          eq(orders.tenantId, getTenantId())
        )
      );

    const seq = (result?.count || 0) + 1;
    return `OD${dateStr}${String(seq).padStart(3, '0')}`;
  }

  /**
   * 创建AR对账单
   * @param tx 数据库事务
   * @param order 订单
   */
  private async createARStatement(tx: any, order: any): Promise<void> {
    // TODO: 集成财务模块创建AR对账单
    // 暂时跳过,待财务模块完成后再实现
  }
}
```

**验收标准**:
- [ ] `convertFromQuote` 方法完整实现
- [ ] 快照数据结构正确
- [ ] 初始状态判断逻辑正确
- [ ] 结算方式推断逻辑正确
- [ ] 订单号生成逻辑正确
- [ ] 事务处理正确

#### 2.2.2 初始状态逻辑优化 (Day 4.5)

**验收标准**:
- [ ] `hasDeepDesign=true` 时初始状态为 `PENDING_CONFIRMATION`
- [ ] `hasDeepDesign=false` 时初始状态为 `PENDING_PO`
- [ ] `confirmationDeadline` 字段正确设置

#### 2.2.3 结算方式推断逻辑 (Day 5)

**验收标准**:
- [ ] 预付款 >= 30% 时返回 `PREPAID`
- [ ] 预付款 < 30% 时返回 `MONTHLY`
- [ ] 边界值测试通过(30% exactly)

**测试用例**:
```typescript
describe('OrderService.inferSettlementType', () => {
  it('should return PREPAID when paid >= 30%', () => {
    expect(service.inferSettlementType(1000, 300)).toBe('PREPAID');
    expect(service.inferSettlementType(1000, 500)).toBe('PREPAID');
  });

  it('should return MONTHLY when paid < 30%', () => {
    expect(service.inferSettlementType(1000, 200)).toBe('MONTHLY');
    expect(service.inferSettlementType(1000, 0)).toBe('MONTHLY');
  });
});
```

### 2.3 Week 1 验收标准

**功能验收**:
- [ ] 数据库Schema变更完成
- [ ] 订单快照机制100%可用
- [ ] 创建订单后 `snapshotData` 字段非空
- [ ] 快照数据与报价数据一致
- [ ] 报价数据变更后订单快照不受影响

**代码质量验收**:
- [ ] TypeScript无类型错误
- [ ] ESLint无P0/P1错误
- [ ] 代码注释完整
- [ ] 单元测试覆盖率 >= 60%

---

## 3. Week 2: 变更单流程 (P0)

**目标**: 实现完整的订单变更请求、审批、应用流程

### 3.1 变更单Service层 (2天)

#### 3.1.1 创建变更请求逻辑 (Day 6)

**文件**: `src/services/change-request.service.ts`

**完整实现**:
```typescript
import { db } from '@/shared/db';
import { changeRequests } from '@/shared/api/schema/change-requests';
import { orders, orderItems } from '@/shared/api/schema/orders';
import { eq, and } from 'drizzle-orm';
import { Decimal } from 'decimal.js';

export class ChangeRequestService {
  /**
   * 创建变更请求
   * @param orderId 订单ID
   * @param data 变更数据
   * @param userId 操作人ID
   * @returns 变更请求ID
   */
  async create(
    orderId: string,
    data: {
      changeType: 'ADD_ITEM' | 'REMOVE_ITEM' | 'MODIFY_ITEM';
      changeReason: string;
      originalItems: any[];
      newItems: any[];
    },
    userId: string
  ): Promise<string> {
    return await db.transaction(async (tx) => {
      // 1. 验证订单状态
      const [order] = await tx
        .select()
        .from(orders)
        .where(eq(orders.id, orderId))
        .limit(1);

      if (!order) {
        throw new Error('订单不存在');
      }

      // 仅允许特定状态下变更
      const allowedStatuses = ['PENDING_PO', 'PENDING_CONFIRMATION'];
      if (!allowedStatuses.includes(order.status)) {
        throw new Error(`当前状态 ${order.status} 不允许变更`);
      }

      // 2. 计算差价
      const priceDifference = this.calculatePriceDifference(
        data.originalItems,
        data.newItems
      );

      // 3. 创建变更请求
      const [changeRequest] = await tx
        .insert(changeRequests)
        .values({
          tenantId: getTenantId(),
          orderId,
          changeType: data.changeType,
          changeReason: data.changeReason,
          originalItems: JSON.stringify(data.originalItems),
          newItems: JSON.stringify(data.newItems),
          priceDifference: priceDifference.toString(),
          status: 'PENDING',
          createdBy: userId,
        })
        .returning();

      return changeRequest.id;
    });
  }

  /**
   * 计算差价
   * @param originalItems 原始商品列表
   * @param newItems 新商品列表
   * @returns 差价(正数=补差价,负数=退差价)
   */
  private calculatePriceDifference(
    originalItems: any[],
    newItems: any[]
  ): Decimal {
    const originalTotal = originalItems.reduce(
      (sum, item) => sum.plus(new Decimal(item.subtotal || 0)),
      new Decimal(0)
    );

    const newTotal = newItems.reduce(
      (sum, item) => sum.plus(new Decimal(item.subtotal || 0)),
      new Decimal(0)
    );

    return newTotal.minus(originalTotal);
  }

  /**
   * 审批变更请求
   * @param changeRequestId 变更请求ID
   * @param approved 是否批准
   * @param userId 审批人ID
   * @param rejectionReason 拒绝原因(可选)
   */
  async approve(
    changeRequestId: string,
    approved: boolean,
    userId: string,
    rejectionReason?: string
  ): Promise<void> {
    return await db.transaction(async (tx) => {
      // 1. 获取变更请求
      const [changeRequest] = await tx
        .select()
        .from(changeRequests)
        .where(eq(changeRequests.id, changeRequestId))
        .limit(1);

      if (!changeRequest) {
        throw new Error('变更请求不存在');
      }

      if (changeRequest.status !== 'PENDING') {
        throw new Error('变更请求已处理');
      }

      // 2. 更新变更请求状态
      if (approved) {
        await tx
          .update(changeRequests)
          .set({
            status: 'APPROVED',
            approvedBy: userId,
            approvedAt: new Date(),
          })
          .where(eq(changeRequests.id, changeRequestId));

        // 3. 应用变更到订单
        await this.applyChange(tx, changeRequest);
      } else {
        await tx
          .update(changeRequests)
          .set({
            status: 'REJECTED',
            rejectedBy: userId,
            rejectedAt: new Date(),
            rejectionReason,
          })
          .where(eq(changeRequests.id, changeRequestId));
      }
    });
  }

  /**
   * 应用变更到订单
   * @param tx 数据库事务
   * @param changeRequest 变更请求
   */
  private async applyChange(
    tx: any,
    changeRequest: any
  ): Promise<void> {
    const orderId = changeRequest.orderId;
    const newItems = JSON.parse(changeRequest.newItems);
    const originalItems = JSON.parse(changeRequest.originalItems);

    switch (changeRequest.changeType) {
      case 'ADD_ITEM':
        // 新增商品
        const addItems = newItems.map(item => ({
          orderId,
          quoteItemId: item.id,
          roomName: item.roomName,
          productId: item.productId,
          productName: item.productName,
          category: item.category,
          unitPrice: item.unitPrice,
          quantity: item.quantity,
          width: item.width,
          height: item.height,
          subtotal: item.subtotal,
          status: 'PENDING',
        }));
        await tx.insert(orderItems).values(addItems);
        break;

      case 'REMOVE_ITEM':
        // 删除商品
        const removeIds = originalItems.map(item => item.quoteItemId);
        await tx
          .delete(orderItems)
          .where(
            and(
              eq(orderItems.orderId, orderId),
              sql`${orderItems.quoteItemId} = ANY(${removeIds})`
            )
          );
        break;

      case 'MODIFY_ITEM':
        // 修改商品
        for (const newItem of newItems) {
          await tx
            .update(orderItems)
            .set({
              quantity: newItem.quantity,
              width: newItem.width,
              height: newItem.height,
              subtotal: newItem.subtotal,
            })
            .where(
              and(
                eq(orderItems.orderId, orderId),
                eq(orderItems.quoteItemId, newItem.id)
              )
            );
        }
        break;
    }

    // 更新订单总金额
    await this.updateOrderTotalAmount(tx, orderId);
  }

  /**
   * 更新订单总金额
   * @param tx 数据库事务
   * @param orderId 订单ID
   */
  private async updateOrderTotalAmount(
    tx: any,
    orderId: string
  ): Promise<void> {
    const [result] = await tx
      .select({ total: sql<number>`sum(subtotal)` })
      .from(orderItems)
      .where(eq(orderItems.orderId, orderId));

    const totalAmount = result?.total || 0;

    await tx
      .update(orders)
      .set({ totalAmount })
      .where(eq(orders.id, orderId));
  }

  /**
   * 获取订单的变更历史
   * @param orderId 订单ID
   * @returns 变更历史列表
   */
  async getHistory(orderId: string): Promise<any[]> {
    return await db
      .select()
      .from(changeRequests)
      .where(eq(changeRequests.orderId, orderId))
      .orderBy(changeRequests.createdAt);
  }
}
```

**验收标准**:
- [ ] `create` 方法实现完整
- [ ] 订单状态验证正确
- [ ] 差价计算准确
- [ ] `approve` 方法实现完整
- [ ] 变更应用逻辑正确
- [ ] 订单总金额更新正确

#### 3.1.2 审批逻辑 (Day 6.5)

**验收标准**:
- [ ] 审批通过后自动应用变更
- [ ] 审批拒绝后记录原因
- [ ] 状态流转正确

#### 3.1.3 拒绝逻辑 (Day 7)

**验收标准**:
- [ ] 拒绝原因必填
- [ ] 拒绝后订单不受影响

### 3.2 变更单API实现 (1.5天)

#### 3.2.1 API Action创建 (Day 7.5)

**文件**: `src/features/orders/actions/change-requests.ts`

**完整实现**:
```typescript
'use server';

import { createSafeAction } from '@/lib/actions';
import { z } from 'zod';
import { ChangeRequestService } from '@/services/change-request.service';
import { getTenantId, getUserId } from '@/lib/auth';

// 创建变更请求
const createChangeRequestSchema = z.object({
  orderId: z.string().uuid(),
  changeType: z.enum(['ADD_ITEM', 'REMOVE_ITEM', 'MODIFY_ITEM']),
  changeReason: z.string().min(1, '变更原因不能为空'),
  originalItems: z.array(z.any()),
  newItems: z.array(z.any()),
});

export const createChangeRequest = createSafeAction(
  createChangeRequestSchema,
  async (data, ctx) => {
    const userId = getUserId();
    const tenantId = getTenantId();

    const service = new ChangeRequestService();
    const changeRequestId = await service.create(
      data.orderId,
      data,
      userId
    );

    return { success: true, changeRequestId };
  }
);

// 获取变更历史
const getChangeHistorySchema = z.object({
  orderId: z.string().uuid(),
});

export const getChangeHistory = createSafeAction(
  getChangeHistorySchema,
  async (data, ctx) => {
    const service = new ChangeRequestService();
    const history = await service.getHistory(data.orderId);

    return { success: true, history };
  }
);

// 审批变更请求
const approveChangeRequestSchema = z.object({
  changeRequestId: z.string().uuid(),
  approved: z.boolean(),
  rejectionReason: z.string().optional(),
});

export const approveChangeRequest = createSafeAction(
  approveChangeRequestSchema,
  async (data, ctx) => {
    const userId = getUserId();

    const service = new ChangeRequestService();
    await service.approve(
      data.changeRequestId,
      data.approved,
      userId,
      data.rejectionReason
    );

    return { success: true };
  }
);
```

**验收标准**:
- [ ] 所有API Action创建完成
- [ ] Schema验证正确
- [ ] 权限检查正确
- [ ] 错误处理完善

#### 3.2.2 权限控制 (Day 8)

**验收标准**:
- [ ] 创建变更: 销售/客服/店长
- [ ] 审批变更: 仅店长

### 3.3 变更单前端UI (2.5天)

#### 3.3.1 变更申请Dialog (Day 8.5-9.5)

**文件**: `src/features/orders/components/change-request-dialog.tsx`

**组件结构**:
```typescript
'use client';

import { useState } from 'react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogFooter,
} from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { createChangeRequest } from '../actions/change-requests';

interface ChangeRequestDialogProps {
  orderId: string;
  orderItems: any[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function ChangeRequestDialog({
  orderId,
  orderItems,
  open,
  onOpenChange,
  onSuccess,
}: ChangeRequestDialogProps) {
  const [changeType, setChangeType] = useState<'ADD_ITEM' | 'REMOVE_ITEM' | 'MODIFY_ITEM'>('MODIFY_ITEM');
  const [changeReason, setChangeReason] = useState('');
  const [selectedItems, setSelectedItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    if (!changeReason.trim()) {
      alert('请填写变更原因');
      return;
    }

    setLoading(true);
    try {
      await createChangeRequest({
        orderId,
        changeType,
        changeReason,
        originalItems: selectedItems,
        newItems: selectedItems, // TODO: 实际应根据changeType处理
      });

      onSuccess();
      onOpenChange(false);
      setChangeReason('');
      setSelectedItems([]);
    } catch (error) {
      alert(error instanceof Error ? error.message : '创建失败');
    } finally {
      setLoading(false);
    }
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>申请变更订单</DialogTitle>
        </DialogHeader>

        <div className="space-y-4 py-4">
          {/* 变更类型选择 */}
          <div>
            <label className="text-sm font-medium">变更类型</label>
            <Select value={changeType} onValueChange={(v: any) => setChangeType(v)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="ADD_ITEM">新增商品</SelectItem>
                <SelectItem value="REMOVE_ITEM">删除商品</SelectItem>
                <SelectItem value="MODIFY_ITEM">修改商品</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {/* 变更原因 */}
          <div>
            <label className="text-sm font-medium">变更原因 *</label>
            <Textarea
              value={changeReason}
              onChange={(e) => setChangeReason(e.target.value)}
              placeholder="请填写变更原因"
              rows={3}
            />
          </div>

          {/* 商品选择/编辑 */}
          <div>
            <label className="text-sm font-medium">商品明细</label>
            <div className="mt-2 border rounded-md">
              {orderItems.map((item) => (
                <div key={item.id} className="p-3 border-b last:border-0">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-medium">{item.productName}</div>
                      <div className="text-sm text-gray-500">
                        {item.roomName} - ¥{item.subtotal}
                      </div>
                    </div>
                    {/* TODO: 根据changeType显示不同的操作 */}
                  </div>
                </div>
              ))}
            </div>
          </div>

          {/* 差价显示 */}
          <div className="bg-blue-50 p-3 rounded-md">
            <div className="text-sm font-medium">预计差价</div>
            <div className="text-lg font-bold text-blue-600">
              ¥0.00
            </div>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>
            取消
          </Button>
          <Button onClick={handleSubmit} disabled={loading}>
            {loading ? '提交中...' : '提交变更'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
```

**验收标准**:
- [ ] Dialog组件UI完整
- [ ] 支持三种变更类型
- [ ] 实时显示差价计算
- [ ] 变更原因必填验证
- [ ] 提交成功后刷新列表

#### 3.3.2 变更历史Tab (Day 10)

**文件**: `src/features/orders/components/change-history-tab.tsx`

**验收标准**:
- [ ] 时间轴展示所有变更记录
- [ ] 显示变更前后对比
- [ ] 状态标签颜色正确

#### 3.3.3 审批页面集成 (Day 10.5)

**验收标准**:
- [ ] 待办事项显示待审批变更单
- [ ] 审批Dialog查看详情
- [ ] 批准/拒绝按钮可用

### 3.4 Week 2 验收标准

**功能验收**:
- [ ] 端到端流程可走通(申请→审批→应用)
- [ ] 差价计算准确
- [ ] 变更历史可追溯

**代码质量验收**:
- [ ] TypeScript无类型错误
- [ ] ESLint无P0/P1错误
- [ ] 单元测试覆盖率 >= 60%

---

## 4. Week 3: 智能拆单逻辑 (P0)

**目标**: 实现供应商智能匹配与拆单算法

### 4.1 拆单算法设计与实现 (3天)

#### 4.1.1 供应商匹配算法 (Day 11-12.5)

**文件**: `src/services/supplier-matcher.service.ts`

**完整实现**:
```typescript
import { db } from '@/shared/db';
import { productSuppliers, suppliers } from '@/shared/api/schema/suppliers';
import { eq, and, sql } from 'drizzle-orm';

export class SupplierMatcherService {
  /**
   * 为订单商品匹配最优供应商
   * @param orderItems 订单商品列表
   * @returns 按供应商分组的商品
   */
  async matchSuppliers(orderItems: any[]): Promise<Map<string, any[]>> {
    const groups = new Map<string, any[]>();

    for (const item of orderItems) {
      const supplier = await this.findBestSupplier(item);
      const supplierId = supplier.id;

      if (!groups.has(supplierId)) {
        groups.set(supplierId, []);
      }
      groups.get(supplierId)!.push({
        ...item,
        supplierId,
        supplierName: supplier.name,
      });
    }

    return groups;
  }

  /**
   * 查找最优供应商
   * @param item 订单商品
   * @returns 最优供应商
   */
  private async findBestSupplier(item: any): Promise<any> {
    // 1. 查询该产品的所有供应商
    const suppliers = await db
      .select({
        supplier: suppliers,
        productSupplier: productSuppliers,
      })
      .from(productSuppliers)
      .innerJoin(suppliers, eq(productSuppliers.supplierId, suppliers.id))
      .where(eq(productSuppliers.productId, item.productId));

    if (suppliers.length === 0) {
      throw new Error(`商品 ${item.productName} 无可用供应商`);
    }

    // 2. 按优先级排序
    const ranked = suppliers.sort((a, b) => {
      // 优先级: 库存充足 > 价格低 > 历史评分高
      const aStock = a.productSupplier.stockQuantity || 0;
      const bStock = b.productSupplier.stockQuantity || 0;
      const itemQty = item.quantity || 0;

      // 库存充足优先
      if (aStock >= itemQty && bStock < itemQty) return -1;
      if (aStock < itemQty && bStock >= itemQty) return 1;

      // 价格优先
      const aPrice = Number(a.productSupplier.unitPrice || 0);
      const bPrice = Number(b.productSupplier.unitPrice || 0);
      if (aPrice !== bPrice) {
        return aPrice - bPrice;
      }

      // 评分优先
      const aRating = a.supplier.rating || 0;
      const bRating = b.supplier.rating || 0;
      return bRating - aRating;
    });

    return ranked[0].supplier;
  }

  /**
   * 获取供应商运费
   * @param supplierId 供应商ID
   * @param totalAmount 订单金额
   * @returns 运费
   */
  async getShippingFee(
    supplierId: string,
    totalAmount: number
  ): Promise<number> {
    // TODO: 根据供应商运费规则计算
    // 暂时返回固定值
    return 0;
  }
}
```

**验收标准**:
- [ ] 供应商匹配算法实现
- [ ] 优先级排序正确
- [ ] 库存检查正确
- [ ] 价格比较正确
- [ ] 评分比较正确

#### 4.1.2 按供应商分组 (Day 13)

**验收标准**:
- [ ] 商品按供应商分组
- [ ] 分组预览数据正确

#### 4.1.3 运费分摊逻辑 (Day 13.5)

**文件**: `src/features/orders/logic/order-split-router.ts`

**完整实现**:
```typescript
import { SupplierMatcherService } from '@/services/supplier-matcher.service';
import { Decimal } from 'decimal.js';

export interface SplitResult {
  supplierId: string;
  supplierName: string;
  items: any[];
  subtotal: Decimal;
  shippingFee: Decimal;
  total: Decimal;
}

export class OrderSplitRouter {
  private supplierMatcher: SupplierMatcherService;

  constructor() {
    this.supplierMatcher = new SupplierMatcherService();
  }

  /**
   * 拆单预览
   * @param orderId 订单ID
   * @returns 拆单结果
   */
  async preview(orderId: string): Promise<SplitResult[]> {
    // 1. 获取订单商品
    const orderItems = await this.getOrderItems(orderId);

    // 2. 匹配供应商
    const groups = await this.supplierMatcher.matchSuppliers(orderItems);

    // 3. 计算运费分摊
    const results = await this.allocateShippingFee(groups);

    return results;
  }

  /**
   * 确认拆单
   * @param orderId 订单ID
   * @param splitResults 拆单结果
   */
  async confirm(
    orderId: string,
    splitResults: SplitResult[]
  ): Promise<void> {
    // TODO: 生成采购单
    // TODO: 更新orderItems.supplierId和purchaseOrderId
  }

  /**
   * 获取订单商品
   * @param orderId 订单ID
   * @returns 订单商品列表
   */
  private async getOrderItems(orderId: string): Promise<any[]> {
    // TODO: 查询orderItems表
    return [];
  }

  /**
   * 运费分摊
   * @param groups 按供应商分组的商品
   * @returns 拆单结果
   */
  private async allocateShippingFee(
    groups: Map<string, any[]>
  ): Promise<SplitResult[]> {
    const results: SplitResult[] = [];

    // 计算总金额
    const totalAmount = Array.from(groups.values())
      .flat()
      .reduce((sum, item) => sum.plus(new Decimal(item.subtotal || 0)), new Decimal(0));

    // 按供应商分摊运费
    for (const [supplierId, items] of groups.entries()) {
      const groupTotal = items.reduce(
        (sum, item) => sum.plus(new Decimal(item.subtotal || 0)),
        new Decimal(0)
      );

      const ratio = groupTotal.div(totalAmount);
      const shippingFee = await this.supplierMatcher.getShippingFee(
        supplierId,
        Number(groupTotal)
      );

      const allocatedFee = new Decimal(shippingFee).mul(ratio);

      results.push({
        supplierId,
        supplierName: items[0].supplierName,
        items,
        subtotal: groupTotal,
        shippingFee: allocatedFee,
        total: groupTotal.plus(allocatedFee),
      });
    }

    return results;
  }
}
```

**验收标准**:
- [ ] 运费分摊逻辑正确
- [ ] 按金额比例分摊
- [ ] 计算精度正确(使用Decimal)

### 4.2 拆单API实现 (1天)

#### 4.2.1 拆单预览API (Day 14)

**文件**: `src/features/orders/actions/split.ts`

**验收标准**:
- [ ] `POST /api/orders/:id/split/preview` 实现
- [ ] 返回分组结果和运费分摊

#### 4.2.2 拆单确认API (Day 14.5)

**验收标准**:
- [ ] `POST /api/orders/:id/split/confirm` 实现
- [ ] 生成多个采购单
- [ ] 更新orderItems字段

### 4.3 拆单前端UI (2天)

#### 4.3.1 拆单页面 (Day 15-16)

**文件**: `src/app/(dashboard)/orders/[id]/split/page.tsx`

**验收标准**:
- [ ] 左侧Items列表
- [ ] 右侧分组预览
- [ ] 底部运费分摊明细表

#### 4.3.2 拆单Dialog (Day 16.5)

**文件**: `src/features/orders/components/split-order-dialog.tsx`

**验收标准**:
- [ ] 显示预览结果
- [ ] 确认后跳转采购单列表

### 4.4 Week 3 验收标准

**功能验收**:
- [ ] 智能匹配准确率 >= 95%
- [ ] 运费分摊逻辑正确
- [ ] 生成的采购单数据完整

**代码质量验收**:
- [ ] TypeScript无类型错误
- [ ] ESLint无P0/P1错误
- [ ] 单元测试覆盖率 >= 60%

---

## 5. Week 4: 发货与物流流程 (P0)

**目标**: 实现发货申请、确认和物流轨迹查询

### 5.1 发货流程后端 (2天)

#### 5.1.1 申请发货API (Day 17-18)

**文件**: `src/features/orders/actions/delivery.ts`

**完整实现**:
```typescript
'use server';

import { createSafeAction } from '@/lib/actions';
import { z } from 'zod';
import { db } from '@/shared/db';
import { orders, orderItems } from '@/shared/api/schema/orders';
import { eq, and } from 'drizzle-orm';

const requestDeliverySchema = z.object({
  orderId: z.string().uuid(),
  deliveryAddress: z.string().min(1, '发货地址不能为空'),
});

export const requestDelivery = createSafeAction(
  requestDeliverySchema,
  async (data, ctx) => {
    return await db.transaction(async (tx) => {
      // 1. 验证订单状态
      const [order] = await tx
        .select()
        .from(orders)
        .where(eq(orders.id, data.orderId))
        .limit(1);

      if (!order) {
        throw new Error('订单不存在');
      }

      if (order.status !== 'PENDING_DELIVERY') {
        throw new Error('仅待发货状态可申请发货');
      }

      // 2. 验证库存状态
      const items = await tx
        .select()
        .from(orderItems)
        .where(eq(orderItems.orderId, data.orderId));

      const allInStock = items.every(item => item.deliveryStatus === 'DELIVERED');
      if (!allInStock) {
        throw new Error('部分商品未入库,无法发货');
      }

      // 3. 更新订单状态
      await tx
        .update(orders)
        .set({
          status: 'PENDING_SHIPMENT',
          deliveryAddress: data.deliveryAddress,
        })
        .where(eq(orders.id, data.orderId));

      return { success: true };
    });
  }
);
```

**验收标准**:
- [ ] 库存状态验证正确
- [ ] 发货地址验证正确
- [ ] 状态变更正确

#### 5.1.2 确认发货API (Day 18.5)

**验收标准**:
- [ ] 填写物流公司和运单号
- [ ] 状态变更至SHIPPED
- [ ] 触发物流追踪

### 5.2 物流轨迹集成 (2天)

#### 5.2.1 快递100 SDK安装与配置 (Day 19)

**验收标准**:
- [ ] `pnpm add kuaidi100-sdk`
- [ ] 配置API Key(环境变量)

#### 5.2.2 LogisticsService创建 (Day 19.5-20)

**文件**: `src/services/logistics.service.ts`

**完整实现**:
```typescript
import Kuaidi100 from 'kuaidi100-sdk';

export interface LogisticsTracking {
  trackingNumber: string;
  company: string;
  companyName: string;
  status: string;
  timeline: Array<{
    time: string;
    status: string;
    location: string;
    description: string;
  }>;
  estimatedDelivery: string | null;
}

export class LogisticsService {
  private client: any;

  constructor() {
    this.client = new Kuaidi100({
      key: process.env.KUAIDI100_API_KEY,
      secret: process.env.KUAIDI100_SECRET,
    });
  }

  /**
   * 订阅物流推送
   * @param trackingNumber 运单号
   * @param company 物流公司代码
   */
  async subscribe(
    trackingNumber: string,
    company: string
  ): Promise<void> {
    await this.client.subscribe({
      company,
      number: trackingNumber,
      phone: '', // 可选
    });
  }

  /**
   * 查询物流轨迹
   * @param trackingNumber 运单号
   * @param company 物流公司代码
   * @returns 物流轨迹
   */
  async query(
    trackingNumber: string,
    company: string
  ): Promise<LogisticsTracking> {
    const result = await this.client.query({
      company,
      number: trackingNumber,
    });

    return {
      trackingNumber,
      company,
      companyName: this.getCompanyName(company),
      status: result.state,
      timeline: result.data.map((item: any) => ({
        time: item.time,
        status: item.context,
        location: item.location || '',
        description: item.context,
      })),
      estimatedDelivery: null, // TODO: 解析预计送达时间
    };
  }

  /**
   * 获取物流公司名称
   * @param code 物流公司代码
   * @returns 物流公司名称
   */
  private getCompanyName(code: string): string {
    const companies: Record<string, string> = {
      SF: '顺丰速运',
      DB: '德邦快递',
      ZTO: '中通快递',
      YTO: '圆通速递',
      STO: '申通快递',
      SELF_PICKUP: '自提',
    };
    return companies[code] || code;
  }

  /**
   * 处理物流回调
   * @param data 回调数据
   */
  async handleWebhook(data: any): Promise<void> {
    // TODO: 更新订单物流状态
    // TODO: 通知相关用户
  }
}
```

**验收标准**:
- [ ] `subscribe` 方法实现
- [ ] `query` 方法实现
- [ ] Webhook处理实现

#### 5.2.3 物流轨迹API (Day 20.5)

**文件**: `src/features/orders/actions/logistics.ts`

**验收标准**:
- [ ] `GET /api/orders/:id/logistics-track` 实现
- [ ] 返回物流状态和轨迹列表

### 5.3 发货前端UI (2天)

#### 5.3.1 发货申请Dialog (Day 21)

**文件**: `src/features/orders/components/delivery-request-dialog.tsx`

**验收标准**:
- [ ] 显示发货地址(允许修改)
- [ ] 库存检查提示

#### 5.3.2 确认发货Dialog (Day 21.5)

**文件**: `src/features/orders/components/delivery-confirm-dialog.tsx`

**验收标准**:
- [ ] 填写物流公司和运单号
- [ ] 支持扫码录入运单号

#### 5.3.3 物流轨迹展示 (Day 22)

**文件**: `src/features/orders/components/logistics-track-card.tsx`

**验收标准**:
- [ ] 时间轴展示物流轨迹
- [ ] 实时更新(轮询或WebSocket)

### 5.4 Week 4 验收标准

**功能验收**:
- [ ] 发货流程端到端可执行
- [ ] 物流轨迹实时更新
- [ ] 异常物流状态有提示

**代码质量验收**:
- [ ] TypeScript无类型错误
- [ ] ESLint无P0/P1错误
- [ ] 单元测试覆盖率 >= 60%

---

## 6. Week 5: 叫停机制 + P1功能 (P0+P1)

**目标**: 完成叫停机制和其他P1重要功能

### 6.1 叫停机制 (1.5天)

#### 6.1.1 叫停API (Day 23)

**文件**: `src/features/orders/actions/halt.ts`

**完整实现**:
```typescript
'use server';

import { createSafeAction } from '@/lib/actions';
import { z } from 'zod';
import { db } from '@/shared/db';
import { orders } from '@/shared/api/schema/orders';
import { eq } from 'drizzle-orm';

const haltOrderSchema = z.object({
  orderId: z.string().uuid(),
  haltedReason: z.string().min(1, '叫停原因不能为空'),
});

export const haltOrder = createSafeAction(
  haltOrderSchema,
  async (data, ctx) => {
    return await db.transaction(async (tx) => {
      // 1. 验证订单状态
      const [order] = await tx
        .select()
        .from(orders)
        .where(eq(orders.id, data.orderId))
        .limit(1);

      if (!order) {
        throw new Error('订单不存在');
      }

      // 检查是否可以叫停
      const canHaltStatuses = [
        'PENDING_PO',
        'PENDING_CONFIRMATION',
        'PENDING_PRODUCTION',
        'FABRIC_PURCHASING',
        'PROCESSING',
        'IN_PRODUCTION',
      ];

      if (!canHaltStatuses.includes(order.status)) {
        throw new Error(`当前状态 ${order.status} 不允许叫停`);
      }

      // 2. 记录叫停
      await tx
        .update(orders)
        .set({
          previousStatus: order.status,
          status: 'HALTED',
          haltedReason: data.haltedReason,
          haltedAt: new Date(),
        })
        .where(eq(orders.id, data.orderId));

      return { success: true };
    });
  }
);
```

**验收标准**:
- [ ] 记录previous_status
- [ ] 记录halted_reason和halted_at
- [ ] 状态变更至HALTED

#### 6.1.2 恢复API (Day 23.5)

**验收标准**:
- [ ] 恢复至previous_status
- [ ] 清空halted_reason和halted_at

#### 6.1.3 叫停Dialog (Day 24)

**文件**: `src/features/orders/components/halt-order-dialog.tsx`

**验收标准**:
- [ ] 必填叫停原因
- [ ] 显示叫停历史

### 6.2 撤单审批流程 (2天)

#### 6.2.1 撤单API (Day 24.5-25)

**文件**: `src/features/orders/actions/cancel.ts`

**验收标准**:
- [ ] 提交审批工作流
- [ ] 审批通过后状态变为CANCELLED

#### 6.2.2 撤单Dialog (Day 25.5)

**文件**: `src/features/orders/components/cancel-order-dialog.tsx`

**验收标准**:
- [ ] 必填撤单原因
- [ ] 显示审批进度

### 6.3 前端优化 (2.5天)

#### 6.3.1 Tab布局重构 (Day 26)

**文件**: `src/app/(dashboard)/orders/[id]/page.tsx`

**验收标准**:
- [ ] 订单详情页改为Tab布局
- [ ] Tab: 订单详情/变更历史/操作日志/关联单据

#### 6.3.2 高级筛选与分页 (Day 27)

**文件**: `src/features/orders/components/orders-filter-dialog.tsx`

**验收标准**:
- [ ] 列表页添加筛选Dialog
- [ ] 筛选条件: 状态/销售/金额区间/日期范围
- [ ] 实现前端分页组件
- [ ] 连接后端分页API

#### 6.3.3 快照对比UI (Day 27.5)

**文件**: `src/features/orders/components/snapshot-compare-view.tsx`

**验收标准**:
- [ ] 左右对比原始报价vs当前订单
- [ ] 高亮变更部分

### 6.4 Week 5 验收标准

**功能验收**:
- [ ] 叫停/恢复流程可执行
- [ ] 撤单审批集成成功
- [ ] 筛选和分页功能正常

**代码质量验收**:
- [ ] TypeScript无类型错误
- [ ] ESLint无P0/P1错误
- [ ] 单元测试覆盖率 >= 60%

---

## 7. Week 6: 测试与上线准备 (P2)

**目标**: 完成测试覆盖和生产准备

### 7.1 单元测试 (2天)

#### 7.1.1 OrderService测试 (Day 28)

**文件**: `src/services/__tests__/order.service.test.ts`

**验收标准**:
- [ ] 测试订单创建逻辑
- [ ] 测试快照保存
- [ ] 测试结算方式推断

#### 7.1.2 ChangeRequestService测试 (Day 28.5)

**文件**: `src/services/__tests__/change-request.service.test.ts`

**验收标准**:
- [ ] 测试差价计算
- [ ] 测试审批后应用变更

#### 7.1.3 拆单算法测试 (Day 29)

**文件**: `src/features/orders/logic/__tests__/order-split-router.test.ts`

**验收标准**:
- [ ] 测试供应商匹配
- [ ] 测试运费分摊

#### 7.1.4 状态流转测试 (Day 29.5)

**文件**: `src/features/orders/__tests__/state-machine.test.ts`

**验收标准**:
- [ ] 测试状态机合法性
- [ ] 测试非法状态转换拦截

### 7.2 E2E测试 (2天)

#### 7.2.1 完整订单流程 (Day 30)

**文件**: `tests/e2e/order-lifecycle.spec.ts`

**验收标准**:
- [ ] 从报价转订单
- [ ] 拆单
- [ ] 发货
- [ ] 物流追踪
- [ ] 完成

#### 7.2.2 变更单流程 (Day 30.5)

**文件**: `tests/e2e/order-change-request.spec.ts`

**验收标准**:
- [ ] 申请变更
- [ ] 审批
- [ ] 应用变更

#### 7.2.3 异常流程 (Day 31)

**验收标准**:
- [ ] 叫停与恢复
- [ ] 撤单审批

### 7.3 性能优化与安全加固 (2天)

#### 7.3.1 性能优化 (Day 31.5-32)

**验收标准**:
- [ ] 订单列表查询优化(索引验证)
- [ ] 拆单算法性能测试(1000 items < 3s)
- [ ] 前端列表虚拟滚动(如需)

#### 7.3.2 安全加固 (Day 32.5)

**验收标准**:
- [ ] 权限检查完善
- [ ] 操作日志集成(AuditLog)
- [ ] 敏感字段脱敏(手机号)
- [ ] SQL注入防护验证

### 7.4 Week 6 验收标准

**功能验收**:
- [ ] 单元测试覆盖率 >= 70%
- [ ] E2E测试通过率 100%
- [ ] 性能指标达标
- [ ] 安全检查无P0/P1问题

---

## 8. 验收标准

### 8.1 功能完整性

- [ ] 订单快照机制100%可用
- [ ] 变更单流程端到端可执行
- [ ] 智能拆单算法正确率>=95%
- [ ] 所有18个核心API实现且测试通过
- [ ] UI/UX Flow完整无阻塞点

### 8.2 性能标准

- [ ] 1000+订单列表加载时间 <2s
- [ ] 拆单算法响应时间 <3s
- [ ] 订单详情页首屏加载 <1s

### 8.3 质量标准

- [ ] 代码单元测试覆盖率 >=70%
- [ ] E2E测试覆盖核心Flow
- [ ] 无P0/P1级别lint错误

---

## 9. 风险应对

### 9.1 高风险应对

**风险1: 拆单算法复杂度超预期**
- **预防**: Week 1提前进行算法原型验证
- **应对**: 若算法难度过高,先实现手动选择供应商版本,智能匹配放到P2
- **降级**: 最坏情况下仅实现手动拆单

**风险2: 采购模块未完成**
- **预防**: Week 1立即审计采购模块,评估完成度
- **应对**: 若PO模块严重不足,先Mock采购单创建,后续补齐
- **降级**: 拆单仅分组预览,不实际生成PO

### 9.2 中风险应对

**风险3: 快递100 API不稳定**
- **预防**: Week 4先测试API连通性和稳定性
- **应对**: 增加降级方案:手动录入物流信息
- **降级**: 物流轨迹改为手动更新

**风险4: 审批流程未完成**
- **预防**: Week 2先确认Approval模块状态
- **应对**: 简化审批流程,店长直接批准,不走复杂工作流
- **降级**: 变更单和撤单无需审批,仅记录操作人

**风险5: 变更单流程与实际业务不符**
- **预防**: Week 2开发前与业务确认详细流程
- **应对**: 快速迭代,分阶段交付验证
- **降级**: 先实现基础变更功能,复杂场景后续迭代

---

## 10. 附录

### 10.1 关键文件清单

**Schema**:
- `src/shared/api/schema/orders.ts` - 需大量修改
- `src/shared/api/schema/change-requests.ts` - 新建

**Service**:
- `src/services/order.service.ts` - 需扩展
- `src/services/change-request.service.ts` - 新建
- `src/services/supplier-matcher.service.ts` - 新建
- `src/services/logistics.service.ts` - 新建

**Actions**:
- `src/features/orders/actions/orders.ts` - 需补全API
- `src/features/orders/actions/change-requests.ts` - 新建
- `src/features/orders/actions/split.ts` - 新建
- `src/features/orders/actions/delivery.ts` - 新建
- `src/features/orders/actions/logistics.ts` - 新建
- `src/features/orders/actions/halt.ts` - 新建
- `src/features/orders/actions/cancel.ts` - 新建

**UI**:
- `src/app/(dashboard)/orders/page.tsx` - 需添加Tab/筛选
- `src/app/(dashboard)/orders/[id]/page.tsx` - 需重构为Tab布局
- `src/app/(dashboard)/orders/[id]/split/page.tsx` - 新建
- `src/features/orders/components/change-request-dialog.tsx` - 新建
- `src/features/orders/components/change-history-tab.tsx` - 新建
- `src/features/orders/components/split-order-dialog.tsx` - 新建
- `src/features/orders/components/delivery-request-dialog.tsx` - 新建
- `src/features/orders/components/delivery-confirm-dialog.tsx` - 新建
- `src/features/orders/components/logistics-track-card.tsx` - 新建
- `src/features/orders/components/halt-order-dialog.tsx` - 新建
- `src/features/orders/components/cancel-order-dialog.tsx` - 新建
- `src/features/orders/components/orders-filter-dialog.tsx` - 新建
- `src/features/orders/components/snapshot-compare-view.tsx` - 新建

**逻辑**:
- `src/features/orders/logic/order-split-router.ts` - 需完全重写

**测试**:
- `src/services/__tests__/order.service.test.ts` - 新建
- `src/services/__tests__/change-request.service.test.ts` - 新建
- `src/features/orders/logic/__tests__/order-split-router.test.ts` - 重写
- `src/features/orders/__tests__/state-machine.test.ts` - 新建
- `tests/e2e/order-lifecycle.spec.ts` - 新建
- `tests/e2e/order-change-request.spec.ts` - 新建

### 10.2 外部依赖

| 依赖项 | 状态 | 风险 | 缓解措施 |
|:---|:---:|:---:|:---|
| 采购模块(PO) | ⚠️ 未知 | 高 | Week 1先审计PO模块 |
| 审批流程(Approval) | ⚠️ 未知 | 中 | 简化为店长直接审批(不走工作流) |
| 快递100 API | ❌ 未集成 | 中 | 申请API Key,测试连通性 |
| 报价模块(Quote) | ✅ 已完成 | 低 | - |

---

**文档结束**
