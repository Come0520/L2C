...
**基于文档**: [订单模块差异分析报告\_20260116.md](./订单模块差异分析报告_20260116.md)
...
1. **创建变更请求逻辑** (1天)
   ...
   const calculatePriceDifference = (
     ...
     const originalTotal = originalItems.reduce((sum, item) => sum + Number(item.subtotal), 0);
     const newTotal = newItems.reduce((sum, item) => sum + Number(item.subtotal), 0);
