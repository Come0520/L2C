...
```typescript
export async function recommendPaymentMatch(payment: {
  amount: number;
  reference?: string;
  customerId?: string;
  supplierId?: string;
...
export const financeStatementItems = pgTable('finance_statement_items', {
  ...
  statementId: uuid('statement_id')
    .notNull()
    .references('finance_statements.id', { onDelete: 'cascade' }),
...
async function processTransaction(transactionId: string) {
  ...
  const result = await processDifference(
    parseFloat(statement[0].totalAmount.toString()) -
      parseFloat(statement[0].paidAmount.toString()),
  ...
  const newStatus =
    newPaidAmount >= parseFloat(statement[0].totalAmount.toString()) ? 'COMPLETED' : 'PARTIAL';

  await db
    .update(financeStatements)
    .set({
...
export async function recommendPaymentMatch(params: {
    ...
    if (order[0]) {
      ...
      for (const statement of statements) {
        const remainingAmount =
          parseFloat(statement.totalAmount.toString()) -
          parseFloat(statement.paidAmount.toString());
  ...
  for (const statement of pendingStatements) {
    const remainingAmount =
      parseFloat(statement.totalAmount.toString()) - parseFloat(statement.paidAmount.toString());
    const diff = Math.abs(remainingAmount - params.amount);
    const confidence = diff < 1 ? 0.8 : diff < 10 ? 0.6 : 0.4;
    matches.push({
      statementId: statement.id,
...
// 订单状态变更事件
export const onOrderStatusChanged = createEvent<{
  orderId: string;
  oldStatus: string;
  newStatus: string;
}>();
...
// 采购单状态变更事件
export const onPurchaseOrderStatusChanged = createEvent<{
  purchaseOrderId: string;
  oldStatus: string;
  newStatus: string;
}>();
...
export const checkOverdueStatements = cronJob('0 0 * * *', async () => {
  const overdueStatements = await db
    ...
    .from(financeStatements)
    .where(
      and(lt(financeStatements.dueDate, new Date()), ne(financeStatements.status, 'COMPLETED'))
    );
