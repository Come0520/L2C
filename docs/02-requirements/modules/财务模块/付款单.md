...
export async function createCustomerRefund(
  ...
  // 创建退款单
  const refund = await db.insert(paymentBills).values({
...
// 审核客户退款单
export async function verifyCustomerRefund(
  refundId: string,
  verifiedBy: string
): Promise<void> {
...
export const apLaborFeeDetails = pgTable('ap_labor_fee_details', {
  id: uuid('id').defaultRandom().primaryKey(),
  statementId: uuid('statement_id').notNull().references(() => apLaborStatements.id),
...
```typescript
// 付款审核逻辑
export async function verifyPaymentBill(
  ...
  // 更新采购对账单付款记录
  await updateAPSupplierStatementPaidAmount(
    paymentBill.statementId,
    paymentBill.amount
  );
...
// 劳务结算计算 - 与安装单联动
export async function calculateLaborSettlement(
  workerId: string,
  period: string
): Promise<void> {
  // 获取该工人在该周期内完成的安装任务
...
// 安装单完成后自动触发劳务结算
export async function createLaborSettlementFromInstallTask(
