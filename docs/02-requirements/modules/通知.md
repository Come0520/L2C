...
- **服务端过滤**：支持基于通知类型（如系统通知、订单状态、审批等）在服务端进行严格过滤获取列表。
- **分类一键已读**：支持在特定通知类型分类下，仅将该分类下的所有未读消息标记为已读。
- **安全防误触**：执行“全部标记为已读”操作时，强制弹出二次确认（Confirm）弹窗，防止用户发生误触。
...
export class SmsAdapter implements ChannelAdapter, TemplateChannelAdapter {
  async send(payload: NotificationPayload): Promise<boolean> {
    ...
    return this.sendWithTemplate(recipient, 'DEFAULT_SMS_TEMPLATE', { content: payload.content });
  }
  ...
  async sendWithTemplate(
    ...
    // TODO: 集成阿里云/腾讯云短信SDK
    console.log(
      `[SMS Mock] Sending to ${recipient}: Template=${externalTemplateId}, Params=${JSON.stringify(mappedParams)}`
    );
    return true;
  }
...
export class LarkAdapter implements ChannelAdapter, TemplateChannelAdapter {
  async send(payload: NotificationPayload): Promise<boolean> {
    ...
    return this.sendWithTemplate(recipient, 'DEFAULT_LARK_TEMPLATE', {
      title: payload.title,
                  ...
                  ? [
                      {
                        tag: 'a',
                        text: '点击查看详情',
                        href: `${process.env.NEXTAUTH_URL}${mappedParams.link}`,
                      },
                    ]
                  : [],
              ],
...
export class WeChatAdapter implements ChannelAdapter, TemplateChannelAdapter {
  ...
  async sendWithTemplate(
    ...
    // TODO: 调用微信SDK发送
    console.log(
      `[WeChat Mock] Sending to ${recipient}: Template=${externalTemplateId}, Data=${JSON.stringify(data)}`
    );
    return true;
  }
