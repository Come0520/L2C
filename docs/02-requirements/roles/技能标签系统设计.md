...
async function canAssignTask(
  ...
  // 检查师傅是否具备该技能
  const hasSkill = worker.skills?.some((skill) => skill.skillTag.code === skillTagCode);
...
async function getAvailableWorkers(taskType: string, category: string): Promise<Worker[]> {
  ...
  const workers = await db.query.users.findMany({
    where: and(isNotNull(users.worker_type), eq(users.is_active, true)),
    with: {
      skills: {
        with: {
          skillTag: true,
  ...
  return workers.filter((worker) =>
    worker.skills?.some((skill) => skill.skillTag.code === skillTagCode)
  );
}
```
...
```typescript
// 创建自定义标签
async function createCustomSkillTag(
  tenantId: string,
  code: string,
  name: string,
  parentId: string
