...
Page({
    ...
    async fetchConfig() {
        ...
        try {
            ...
            if (res.success) {
                ...
                // 初始化默认房间
                const rooms = defaultRooms.map((name, i) => ({
                    id: Date.now() + i,
                    name,
                    items: []
                }));
     ...
     * 商品选择器会调用此方法
     */
    async addProductToRoom(roomIndex, product) {
        // 默认尺寸，稍后用户可编辑
        const width = 3.0;
        const height = 2.7;
        const foldRatio = 2.0;
    ...
    async onSelectCustomer() {
        ...
        try {
            ...
            wx.hideLoading();
            if (!res.success || !((_a = res.data) === null || _a === void 0 ? void 0 : _a.length)) {
                // 没有客户时直接新建
    ...
    onSubmit: throttleTap(async function () {
        ...
        try {
            const app = getApp();
            // 开发模式检测
            if ((_a = authStore.token) === null || _a === void 0 ? void 0 : _a.startsWith('dev-mock-token-')) {
                wx.hideLoading();
