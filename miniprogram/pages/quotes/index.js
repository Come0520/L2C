...
Page({
    ...
    onShow() {
        if (typeof this.getTabBar === 'function' && this.getTabBar()) {
            ...
            const index = tabBar.data.list.findIndex((item) => item.pagePath === '/pages/quotes/index');
            if (index !== -1)
    ...
    async fetchList() {
        ...
        try {
            ...
            if (res.success) {
                const formatted = (res.data || []).map((item) => (Object.assign(Object.assign({}, item), { statusText: STATUS_MAP[item.status] || item.status, updatedAt: item.updatedAt ? item.updatedAt.split('T')[0] : '' })));
