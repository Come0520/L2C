...
Page({
    ...
    loadResources(tab) {
        ...
        setTimeout(() => {
            const rawData = this.data.mockData[tab] || [];
            // Apply Filtering
            ...
            const data = filteredData.map((item) => (Object.assign(Object.assign({}, item), { selected: selectedIds.includes(String(item.id)) })));
            this.setData({
                resources: data,
                loading: false
            });
    ...
    selectFilterOption(e) {
        ...
        const currentVal = this.data.activeFilters[key];
    ...
    updateResourceSelection(selectedIds) {
        const resources = this.data.resources.map((item) => (Object.assign(Object.assign({}, item), { selected: selectedIds.includes(String(item.id)) })));
