...
async function main() {
    try {
        ...
        for (const role of rolesData) {
            ...
            const preset = ROLES[role.code as keyof typeof ROLES];
        ...
        // ===== 10. 创建交易链路 (Lead -> Quote -> Order -> Install) =====
        ...
        // Quote Items
        const product = await db.query.products.findFirst({ where: eq(schema.products.category, 'CURTAIN_FABRIC') });
        if (product) {
            // Create room first
        ...
        await db.insert(schema.installTasks).values({
            tenantId: tenant.id,
            taskNo: generateDocNo('INS1'),
            orderId: order.id,
            customerId: customer.id,
            customerName: customer.name,
            customerPhone: customer.phone,
        ...
        // Task 2: Pending Confirm (for acceptance tests)
        await db.insert(schema.installTasks).values({
            tenantId: tenant.id,
            taskNo: generateDocNo('INS2'),
            orderId: order.id,
            customerId: customer.id,
            customerName: customer.name,
            customerPhone: customer.phone,
            ...
            salesId: salesId,
            installerId: users['INSTALLER'].id,
            installerName: users['INSTALLER'].name,
            assignedAt: new Date(),
        ...
        console.log('✅ Cycle: Created Lead -> Quote -> Order -> 2 Install Tasks');
