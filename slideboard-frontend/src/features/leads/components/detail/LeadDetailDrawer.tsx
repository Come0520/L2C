...
export function LeadDetailDrawer({ open, onOpenChange, leadId }: LeadDetailDrawerProps) {
    ...
    // Fetch lead details when leadId changes
    useEffect(() => {
        if (!leadId) return
        let mounted = true
        ;(async () => {
    ...
    return (
        <PaperDrawer isOpen={open} onClose={() => onOpenChange(false)} width="lg">
            <div className="flex flex-col h-full bg-paper-50">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-paper-200">
                ...
                {/* Tabs */}
                <div className="flex border-b border-paper-300 px-6 pt-4 bg-white">
                {tabs.map((tab) => (
                    <button
                        key={tab.key}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-ink-800 mt-1">{lead.name}</p>
                                </div>
                                <div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-ink-800 mt-1">{lead.assignedToName || '-'}</p>
                                </div>
                                {lead.designerName && (
                                    <div>
                                        <label className="text-sm text-ink-500">设计师</label>
                                        <p className="text-ink-800 mt-1">{lead.designerName}</p>
                                    </div>
                                )}
                                {lead.shoppingGuideName && (
                                    <div>
                                        <label className="text-sm text-ink-500">导购</label>
                                        <p className="text-ink-800 mt-1">{lead.shoppingGuideName}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-1">
                                        {(lead.requirements || []).map((req: string, index: number) => (
