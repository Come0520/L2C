    ...
    <PaperTableRow key={order.id} className="content-visibility auto">
      ...
      <PaperTableCell>
        <div className="flex space-x-2">
            {canConfirm && <StatefulButton size="sm" status="idle" onClick={() => openDetail(order)}>审核确认</StatefulButton>}
            {canReject && <StatefulButton size="sm" variant="outline" status="idle" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => { setSelectedOrder(order); setIsRejectModalOpen(true); }}>驳回</StatefulButton>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-xl w-[500px]">
            ...
            <div className="p-4 border-t flex justify-end gap-3">
                <StatefulButton variant="outline" status="idle" onClick={() => setIsRejectModalOpen(false)}>取消</StatefulButton>
                <StatefulButton variant="primary" status="idle" className="bg-red-600 hover:bg-red-700 border-red-600" onClick={() => { toast.info('已驳回'); setIsRejectModalOpen(false); setIsDetailOpen(false); }}>确认驳回</StatefulButton>
            </div>
          </div>
        </div>
      )}
