...
export function PendingSurveyView() {
  ...
  useEffect(() => {
    if (rawResponse?.data?.orders) {
      // 直接使用 rawResponse.data.orders，无需类型转换
      ...
      // 计算总金额
      const total = mappedOrders.reduce((sum, order) => sum + order.amount, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <PaperDialogFooter>
          <StatefulButton variant="outline" status="idle" onClick={() => setShowGoSurveyDialog(false)}>
              取消
            </StatefulButton>
            <StatefulButton variant="primary" status="idle" onClick={confirmGoSurvey}>
              确认
            </StatefulButton>
        </PaperDialogFooter>
      </PaperDialog>
      ...
      >
        ...
        <PaperDialogFooter>
          <StatefulButton variant="outline" status="idle" onClick={() => setShowCloseDialog(false)}>
            取消
          </StatefulButton>
          <StatefulButton
            variant="primary"
      ...
      >
        ...
        <PaperDialogFooter>
          <div className="flex space-x-2">
            <StatefulButton variant="outline" status="idle" onClick={() => setShowVersionHistoryDialog(false)}>
              取消
            </StatefulButton>
            <StatefulButton variant="primary" status="idle" onClick={() => {
                // 设置为当前版本
                setShowVersionHistoryDialog(false)
          ...
          <div className="flex space-x-2">
              <StatefulButton variant="outline" status="idle" onClick={() => {
                // 导出Excel
                exportToExcel()
              }}>
                导出Excel
