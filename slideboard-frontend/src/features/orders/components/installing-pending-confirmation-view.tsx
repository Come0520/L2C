      ...
      <div className="grid grid-cols-3 gap-4">
        <PaperCard className="bg-green-50 border-green-100">
          <PaperCardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="text-sm text-green-600">正常状态 (&gt;4h)</div>
        ...
        <PaperCard className="bg-orange-50 border-orange-100">
          <PaperCardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="text-sm text-orange-600">紧急状态 (2-4h)</div>
        ...
        <PaperCard className="bg-red-50 border-red-100">
          <PaperCardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="text-sm text-red-600">超期预警 (&lt;2h)</div>
        ...
        <PaperCard className="border-blue-200 shadow-sm ring-1 ring-blue-100 flex-1">
            ...
            <PaperTable>
                ...
                {orders.map(order => (
                    ...
                    <PaperTableCell>
                        <PaperButton size="small" variant="primary" onClick={() => openDetail(order)}>
                            审核
                        </PaperButton>
                    </PaperTableCell>
                    </PaperTableRow>
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    <PaperButton variant="outline" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => setIsRejectModalOpen(true)}>驳回安装单</PaperButton>
                    <PaperButton 
                        variant="primary" 
                        onClick={async () => { 
                            if (selectedOrder) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-xl w-[500px]">
            ...
            <div className="p-4 border-t flex justify-end gap-3">
                <button onClick={() => setIsRejectModalOpen(false)} className="px-4 py-2 border rounded hover:bg-gray-50">取消</button>
                <button onClick={async () => { if (selectedOrder) { try { const { error } = await supabase.from('orders').update({ status: ORDER_STATUS.INSTALLING_PENDING_ASSIGNMENT }).eq('id', selectedOrder.id); if (error) throw error } catch (err) { logger.error('更新订单状态失败', { resourceType: 'order', resourceId: selectedOrder.id, details: { err } }); toast.error('更新状态失败'); return } } toast.error('已驳回'); setIsRejectModalOpen(false); setIsDetailOpen(false); }} className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">确认驳回</button>
            </div>
          </div>
        </div>
      )}
