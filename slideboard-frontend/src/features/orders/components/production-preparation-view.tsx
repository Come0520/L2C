            ...
            <PaperTableBody>
              {filteredOrders.map(order => (
                <PaperTableRow key={order.id} className={order.stayDuration > 7 || order.shipReminderStatus !== 'none' ? 'bg-red-50/30' : ''}>
                  ...
                  <PaperTableCell>
                    <div className={`flex items-center gap-2 ${order.shipReminderStatus !== 'none' ? 'font-bold' : ''}`}>
                      <span className={
                        order.shipReminderStatus === 'urgent' ? 'text-red-600' :
                        order.shipReminderStatus === 'warning' ? 'text-orange-600' :
                        'text-gray-700'
                      }>
                        {new Date(order.estimatedShipDate).toLocaleString('zh-CN', {
                          year: 'numeric',
                          month: '2-digit',
                          day: '2-digit',
                          hour: '2-digit',
                      ...
                      {order.shipReminderStatus === 'urgent' && (
                        <PaperBadge className="bg-red-50 text-red-700 border-red-200">
                          24小时内
                        </PaperBadge>
                      )}
                      {order.shipReminderStatus === 'warning' && (
                        <PaperBadge className="bg-orange-50 text-orange-700 border-orange-200">
                          48小时内
                        </PaperBadge>
                      )}
                    </div>
