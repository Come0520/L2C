...
type MeasurementTaskRow = Database['public']['Tables']['measurement_tasks']['Row']
...
export async function getMeasurementTasks(): Promise<MeasurementTask[]> {
    ...
    return (data || []).map(mapDbToMeasurementTask)
}

export async function getMeasurementTaskById(id: string): Promise<MeasurementTask | null> {
    const supabase = await createClient()
    ...
    return mapDbToMeasurementTask(data)
}

function mapDbToMeasurementTask(row: MeasurementTaskRow): MeasurementTask {
    return {
        id: row.id,
