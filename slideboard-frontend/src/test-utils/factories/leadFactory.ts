...
export const createLead = (overrides?: Partial<Lead>): Lead => {
  const baseLead: Lead = {
    ...
    project_area: faker.number.float({ min: 50, max: 500, fractionDigits: 1 }),
    ...
    expected_budget: faker.number.float({ min: 10000, max: 1000000, fractionDigits: 0 }),
...
export const createLeadWithQuotes = (overrides?: Partial<Lead> & { quoteCount?: number }) => {
  ...
  return {
    ...
    quotes: Array.from({ length: quoteCount }, () => ({
      ...
      quote_amount: faker.number.float({ min: 5000, max: 500000, fractionDigits: 0 }),
...
export const createLeadWithOrder = (overrides?: Partial<Lead>) => {
  ...
  return {
    ...
    order: {
      ...
      order_amount: faker.number.float({ min: 5000, max: 500000, fractionDigits: 0 }),
