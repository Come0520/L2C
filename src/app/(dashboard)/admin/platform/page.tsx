...
/**
 * 平台管理专属页面（仅超级管理员可访问）
 * 包含：租户管理 / 套餐配置 / AI 模板 / 积分透视台
 *
 * 路由保护：proxy.ts 已通过 checkProxyRouteAccess 拦截非超管访问，
 * 此处作双重保险（defense in depth）再次校验 isPlatformAdmin。
 */
export default async function PlatformPage() {
  ...
  const [tenantsResult, overviewResult, templates, stats, planConfig] = await Promise.all([
  ...
  const tenants: TenantWithPlan[] = rawTenants.map((t) => ({
    ...
    status: t.status,
    planType:
      ((t as unknown as Record<string, unknown>).planType as 'base' | 'pro' | 'enterprise') ??
      'base',
    planExpiresAt: null,
        ...
        <div className="glass-liquid relative h-full w-full overflow-auto rounded-2xl border border-white/10 p-2 md:p-6">
          <Suspense fallback={<Skeleton className="h-full w-full rounded-2xl" />}>
            <TenantManagementClient
              initialTenants={rawTenants}
              initialTotal={tenantsResult.success ? (tenantsResult.data?.total ?? 0) : 0}
              overview={overview}
            />
          </Suspense>
        </div>
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-8rem)] w-full flex-col items-start justify-start perspective-[1000px]">
      <div className="mb-6 px-1">
        <h1 className="text-2xl font-bold text-gray-100">平台管理</h1>
        <p className="mt-1 text-sm text-gray-400">
          仅超级管理员可见 · 管理所有租户、套餐与 AI 能力
        </p>
      </div>
      <AceternityTabs
        tabs={tabs}
...
function AiCreditsTab({
  ...
  stats: TenantCreditsStats[];
  planConfig: Record<string, number>;
}) {
  return (
          ...
          {(
            [
              ...
              { name: 'Pro 专业版', key: 'pro', color: 'bg-blue-900/20 border-blue-800/40' },
              {
                name: 'Enterprise 旗舰版',
                key: 'enterprise',
                color: 'bg-purple-900/20 border-purple-800/40',
              },
            ] as const
          ).map((plan) => (
          ...
          <div className="overflow-hidden rounded-xl border border-white/10 bg-black/20">
            <table className="min-w-full divide-y divide-white/5">
              ...
              <tbody className="divide-y divide-white/5">
                {stats.map((row) => (
                  <tr key={row.tenantId} className="transition-colors hover:bg-white/5">
                    <td className="px-6 py-4 font-mono text-sm whitespace-nowrap text-gray-200">
                      {row.tenantId}
                    </td>
                    <td className="px-6 py-4 text-sm font-bold whitespace-nowrap text-indigo-400">
