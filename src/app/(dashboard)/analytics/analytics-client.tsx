...
export default function AnalyticsClient({ initialData }: { initialData: AnalyticsInitialData }) {
  ...
  const [metrics, setMetrics] = useState<KeyMetrics | null>(initialData.metrics);
  const [funnelData, setFunnelData] = useState<FunnelResponse | null>(initialData.funnelData);
  const [trendData, setTrendData] = useState<TrendData[]>(initialData.trendData);
  const [leaderboardData, setLeaderboardData] = useState<LeaderboardItem[]>(
    initialData.leaderboardData
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [targetOverview, setTargetOverview] = useState<any>(initialData.targetOverview);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [targetTrend, setTargetTrend] = useState<any[]>(initialData.targetTrend);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [targetWarnings, setTargetWarnings] = useState<any[]>(initialData.targetWarnings);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [quarterlyData, setQuarterlyData] = useState<any[]>(initialData.quarterlyData);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [annualProgressData, setAnnualProgressData] = useState<any>(initialData.annualProgressData);
