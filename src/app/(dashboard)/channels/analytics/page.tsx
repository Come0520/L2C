...
export default async function ChannelAnalyticsPage({
  ...
  return (
    <div className="space-y-6">
      <DashboardPageHeader title="渠道业绩分析" subtitle="查看渠道合作数据、业绩排名与趋势分析" />
...
function TrendChart({
  ...
  data: { month: string; dealAmount: number; dealCount: number }[];
}) {
  if (data.length === 0) {
    return (
        ...
        return (
          <div key={item.month} className="flex flex-1 flex-col items-center gap-2">
            <div className="flex w-full flex-col items-center">
              <span className="mb-1 text-xs font-medium">{formatAmount(item.dealAmount)}</span>
              <div
                className="bg-primary/80 hover:bg-primary w-full rounded-t transition-all duration-300"
                style={{ height: `${Math.max(height, 5)}%` }}
              />
            </div>
            <span className="text-muted-foreground text-xs">{item.month.slice(5)}月</span>
          </div>
        );
      })}
    </div>
