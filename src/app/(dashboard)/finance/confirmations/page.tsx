    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">管理月结客户和供应商的对账确认</p>
        </div>
        <Button>
        ...
        {confirmations.length === 0 ? (
          <Card>
            <CardContent className="text-muted-foreground py-12 text-center">
              <FileCheck2 className="mx-auto mb-4 h-12 w-12 opacity-50" />
              暂无对账确认单
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4">
            {confirmations.map((confirmation) => (
              <Card key={confirmation.id} className="transition-shadow hover:shadow-md">
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    <div className="grid grid-cols-3 gap-6">
                      <div>
                        <p className="text-muted-foreground text-xs">账单金额</p>
                        <p className="text-primary text-lg font-semibold">
                          ¥{Number(confirmation.totalAmount).toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-muted-foreground text-xs">已确认</p>
                        <p className="text-lg font-semibold text-green-600">
                          ¥{Number(confirmation.confirmedAmount || 0).toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-muted-foreground text-xs">争议金额</p>
                  ...
                  <div className="text-muted-foreground mt-3 border-t pt-3 text-xs">
                    ...
                    {confirmation.confirmedAt &&
                      ` · 确认于 ${formatDate(confirmation.confirmedAt)}`}
                  </div>
                </CardContent>
              </Card>
            ))}
