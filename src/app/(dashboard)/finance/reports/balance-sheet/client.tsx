    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex justify-between pt-4 font-bold">
                <span>资产总计</span>
                <span>
                  {data.assets.total.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                </span>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between pt-4 font-bold">
                  <span>负债合计</span>
                  <span>
                    {data.liabilities.total.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between pt-4 font-bold">
                  <span>所有者权益合计</span>
                  <span>
                    {data.equity.total.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </CardContent>
      ...
      {!data.isBalanced && (
        <div className="bg-destructive/10 text-destructive border-destructive/20 rounded-md border p-4 text-sm">
          警告：当前资产负债表不平衡（资产总计 ≠ 负债合计 + 所有者权益合计），差异金额为{' '}
          {Math.abs(data.assets.total - data.liabilities.total - data.equity.total).toLocaleString(
            'zh-CN',
