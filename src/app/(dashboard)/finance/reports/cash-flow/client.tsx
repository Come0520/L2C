...
export function CashFlowClient({ data, initialStartDate, initialEndDate }: CashFlowClientProps) {
  ...
  const excelData = [
    { title: '一、经营活动产生的现金流量', category: '' },
    {
      title: '经营活动现金流入小计',
      category: '流入',
      balance: data.operatingActivities.totalInflow,
    },
    {
      title: '经营活动现金流出小计',
      category: '流出',
      balance: data.operatingActivities.totalOutflow,
    },
    {
      title: '经营活动产生的现金流量净额',
      category: '净额',
      balance: data.operatingActivities.netCashFlow,
    },
    { title: '', category: '' }, // 空行
    { title: '二、投资与筹资活动产生的现金流量', category: '简述', balance: 0.0 }, // 目前按项目约束先简化计算为经营流水
    { title: '', category: '' }, // 空行
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {/* 经营活动 */}
            <div>
              <div className="bg-muted/50 rounded-t-md border-b px-2 py-2 font-bold">
                一、经营活动产生的现金流量
              </div>

              <div className="space-y-4 border-b px-4 py-2">
                <div>
                  <h4 className="text-muted-foreground mb-2 text-sm font-semibold">现金流入</h4>
                  <div className="space-y-1">
                    <div className="flex justify-between border-b border-dashed py-1 text-sm">
                      <span>经营活动现金流入小计</span>
                      <span className="font-medium text-green-600">
                        +
                        {data.operatingActivities.totalInflow.toLocaleString('zh-CN', {
                ...
                <div>
                  <h4 className="text-muted-foreground mb-2 text-sm font-semibold">现金流出</h4>
                  <div className="space-y-1">
                    <div className="flex justify-between border-b border-dashed py-1 text-sm">
                      <span>经营活动现金流出小计</span>
                      <span className="font-medium text-red-500">
                        -
                        {data.operatingActivities.totalOutflow.toLocaleString('zh-CN', {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </div>
              ...
              <div className="flex justify-between px-2 py-3 font-bold">
                ...
                <span
                  className={
                    data.operatingActivities.netCashFlow >= 0 ? 'text-green-600' : 'text-red-500'
                  }
                >
                  {data.operatingActivities.netCashFlow.toLocaleString('zh-CN', {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
            ...
            <div className="opacity-50">
              <div className="bg-muted/50 border-b px-2 py-2 font-bold">
                二、投资与筹资活动产生的现金流量 (当前系统归类暂计为 0)
              </div>
            </div>

            {/* 净增加额 */}
            <div className="text-primary bg-primary/5 mt-4 flex justify-between rounded-b-md border-t-2 px-2 py-4 text-lg font-bold">
              <span>五、现金及现金等价物净增加额</span>
              <span>{data.netIncrease.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</span>
            </div>

            <div className="text-muted-foreground mt-8 border-t pt-4 text-xs">
              <p>
                附注：现金流明细参考凭证日志记录的流出与流入。金额前带 "+" 代表流入，带 "-"
                代表流出。
              </p>
            </div>
          </div>
