        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>向面料/配件供应商的付款方式</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">现付</div>
                  <div className="text-muted-foreground mt-1 text-sm">货到即付款（默认）</div>
                </div>
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">月结</div>
                  <div className="text-muted-foreground mt-1 text-sm">按月汇总付款</div>
                </div>
                <div className="bg-primary/5 border-primary rounded-lg border p-4 text-center">
                  <div className="text-primary font-medium">预存</div>
                  <div className="text-muted-foreground mt-1 text-sm">先充值后扣款</div>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>测量师/安装师傅的工费结算方式</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">按单即时结</div>
                  <div className="text-muted-foreground mt-1 text-sm">完成任务后立即结算</div>
                </div>
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">月结</div>
                  <div className="text-muted-foreground mt-1 text-sm">按月汇总工单结算</div>
                </div>
              </div>
              <p className="text-muted-foreground mt-4 text-sm">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>配送物流商的费用结算方式</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">按单即时结</div>
                  <div className="text-muted-foreground mt-1 text-sm">每次配送后即时结算</div>
                </div>
                <div className="rounded-lg border p-4 text-center">
                  <div className="font-medium">月结</div>
                  <div className="text-muted-foreground mt-1 text-sm">按月汇总配送费结算</div>
                </div>
              </div>
              <p className="text-muted-foreground mt-4 text-sm">
                💡 结算方式在物流商资料中单独配置
        ...
        <TabsContent value="prepaid" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>配置供应商预存充值时的赠送方式</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label>赠送方式</Label>
                <Select
                  value={prepaidBonusType}
                  onValueChange={(v) => setPrepaidBonusType(v as typeof prepaidBonusType)}
                >
              ...
              {prepaidBonusType === 'BALANCE' && (
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-xs">
                    如设置 10，则充值 ¥10000，账户增加 ¥
                    {(10000 * (1 + prepaidBonusRatio / 100)).toFixed(0)}
                  </p>
                </div>
              )}

              <div className="bg-muted/50 rounded-lg p-4">
                <h4 className="mb-2 text-sm font-medium">预存流程</h4>
