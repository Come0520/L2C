...
export function TestimonialsClient({ initialData }: { initialData: TestimonialItem[] }) {
  ...
  const handleToggleApproval = async (id: string, currentStatus: boolean) => {
    ...
    if (res.success) {
      ...
      setData((prev) =>
        prev.map((item) => (item.id === id ? { ...item, isApproved: newStatus } : item))
      );
    } else {
            ...
            {data.length === 0 ? (
              ...
              data.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="max-w-[300px] wrap-break-word whitespace-pre-wrap">
                    {item.content}
                  </TableCell>
                  <TableCell>
                    <Badge
                      variant={item.isApproved ? 'default' : 'secondary'}
                      className={item.isApproved ? 'bg-green-500 hover:bg-green-600' : ''}
                    >
