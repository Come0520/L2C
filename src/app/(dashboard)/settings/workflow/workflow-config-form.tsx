...
export function WorkflowConfigForm({ initialConfig }: WorkflowConfigFormProps) {
  ...
  // 业务模式开关
  const [enableLeadAssignment, setEnableLeadAssignment] = useState(
    initialConfig.enableLeadAssignment
  );
  ...
  const [enableOutsourceProcessing, setEnableOutsourceProcessing] = useState(
    initialConfig.enableOutsourceProcessing
  );
  const [enablePurchaseApproval, setEnablePurchaseApproval] = useState(
    initialConfig.enablePurchaseApproval
  );
  ...
  return (
    <>
      ...
      {/* 模式提示 */}
      <div
        className={`rounded-lg p-4 ${isSmallShopMode ? 'border border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30' : 'border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30'}`}
      >
        <div className="flex items-center gap-2">
          <Settings2
            className={`h-5 w-5 ${isSmallShopMode ? 'text-green-600' : 'text-blue-600'}`}
          />
          <span
            className={`font-medium ${isSmallShopMode ? 'text-green-800 dark:text-green-200' : 'text-blue-800 dark:text-blue-200'}`}
          >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>选择"自己做"则跳过派单环节，任务直接由当前用户完成</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label>测量派单模式</Label>
              <Select
                value={measureDispatchMode}
                onValueChange={(v) => setMeasureDispatchMode(v as typeof measureDispatchMode)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              <Label>安装派单模式</Label>
              <Select
                value={installDispatchMode}
                onValueChange={(v) => setInstallDispatchMode(v as typeof installDispatchMode)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <div className="text-muted-foreground text-sm">禁用后完成工单不计算测量/安装工费</div>
            </div>
            <Switch
              id="enable-labor-fee"
