...
vi.mock('@/shared/api/db', () => {
  ...
  const mockTx = {
    insert: vi.fn(() => ({
      values: vi.fn(() => ({ returning: vi.fn().mockResolvedValue([{ id: 'm-id' }]) })),
    })),
    update: vi.fn(() => ({ set: vi.fn(() => ({ where: vi.fn().mockResolvedValue({}) })) })),
    query: {
      quotes: {
        findFirst: vi.fn().mockResolvedValue({ id: 'q1', status: 'PAID', totalAmount: '100' }),
      },
    },
  };
  ...
  return {
    db: {
      insert: vi.fn(() => ({
        values: vi.fn(() => ({ returning: vi.fn().mockResolvedValue([{ id: 'db-id' }]) })),
      })),
      ...
      query: {
        quotes: {
          ...
          findMany: vi
            .fn()
            .mockResolvedValue([
              { id: 'q1', finalAmount: '100', quoteNo: 'Q1', createdAt: new Date().toISOString() },
            ]),
        },
...
vi.mock('@/shared/services/miniprogram/cache.service', () => ({
  CacheService: { getOrSet: vi.fn(async (k, c) => await c()) },
}));
...
describe('小程序 L5 业务逻辑收官套件 (Total Coverage 30+)', () => {
  ...
  describe('1. Dashboard 看板极限压力与参数化测试', () => {
    ...
    it.each(combinations)('应正确响应仪表盘请求 Y:%s M:%s', async (year, month) => {
      const res = await dashboardHandler(
        createReq(`http://localhost/api/dashboard?year=${year}&month=${month}`)
      );
  ...
  describe('2. 客户管理与游标分页矩阵', () => {
    const searches = [
      ...
      '138',
      '',
      'VeryLongNameThatExceedsNormalLength',
      'Special!@#$%^&*()',
      '1234567890',
    ];
    ...
    it.each(searches)('搜索关键字验证: "%s"', async (kw) => {
      ...
      const res = await getCustomersHandler(
        createReq(`http://localhost/api/customers?keyword=${kw}`)
      );
      expect(res.status).toBe(200);
    });
  ...
  describe('3. 报价单生命周期健壮性', () => {
    it('空房间创建申请应能被 Zod 通过并成功落库', async () => {
      const res = await createQuoteHandler(
        createReq('http://localhost/api/quotes', 'POST', { customerId: VALID_UUID, rooms: [] })
      );
      expect(res.status).toBe(200);
    });
    ...
    it('特大型报价单（50+ 项目）不应触发堆栈溢出或超时', async () => {
      ...
      const res = await createQuoteHandler(
        createReq('http://localhost/api/quotes', 'POST', {
    ...
    it('非 UUID 格式的客户 ID 应被强效拦截 (400)', async () => {
      const res = await createQuoteHandler(
        createReq('http://localhost/api/quotes', 'POST', { customerId: 'admin' })
      );
  ...
  describe('4. 订单转换与防重放/容灾', () => {
    it('报价单未确认状态转换应返回 400', async () => {
      vi.mocked(OrderService.createOrderFromQuote).mockRejectedValue(
        new Error('QUOTE_NOT_CONFIRMED')
      );
      const res = await createOrderHandler(
        createReq('http://localhost/api/orders', 'POST', { quoteId: VALID_UUID })
      );
      expect(res.status).toBe(400);
    });
    ...
    it('AuditService 宕机时，核心创建流程不应被阻断', async () => {
      ...
      const res = await createCustomerHandler(
        createReq('http://localhost/api/customers', 'POST', { name: 'Resilient' })
      );
      expect(res.status).toBe(200);
    });
  ...
  describe('5. 类型稳定性验证', () => {
    it('Zod 类型转换验证 (coerce)', async () => {
      ...
      const res = await dashboardHandler(
        createReq('http://localhost/api/dashboard?year=2024&month=05')
      );
      expect(res.status).toBe(200);
    });
  });
