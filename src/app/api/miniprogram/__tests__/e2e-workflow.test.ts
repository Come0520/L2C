      ...
      const tx = {
        ...
        query: {
          orders: {
            findFirst: vi
              .fn()
              .mockResolvedValue({ id: 'mock-order-uuid', paidAmount: '0', totalAmount: '1000' }),
          },
        },
      };
      return await cb(tx);
...
describe('军工级联调与破坏性用例 (E2E & Chaos)', () => {
  ...
  describe('1. 防刷单与频控 (Rate Limiter) 拦截测试', () => {
    it('应允许合理频率的 API 调用并拦截毫秒级的恶意疯狂重放（429）', async () => {
      ...
      const req1 = createReq('http://localhost/api/orders', {
      ...
      const req2 = createReq('http://localhost/api/orders', {
        quoteId: 'b0a7d57d-f43c-47ea-8d6d-2c3f8f1c8e23',
      });
      const res2 = await createOrderHandler(req2);
      const req3 = createReq('http://localhost/api/orders', {
        quoteId: '695d71c1-4b71-41e9-867c-3f417f7d3c3d',
      });
      const res3 = await createOrderHandler(req3);
      const req4 = createReq('http://localhost/api/orders', {
        quoteId: 'a3b8c9d0-e1f2-43b4-85d6-c7e8f9a0b1c2',
      });
      const res4 = await createOrderHandler(req4);
      const req5 = createReq('http://localhost/api/orders', {
        quoteId: 'd4e5f6a7-b8c9-40d1-a2b3-c4d5e6f7a8b9',
      });
  ...
  describe('2. 极致并发抢占 (Idempotency) 脏数据隔离测试', () => {
    it('单用户针对同一个 Quote 并发连击 5 次，必须只流转成功 1 次并立刻封锁其他请求且保证不产生错误插库', async () => {
          ...
          const tx = {
            insert: vi.fn(() => ({
              values: vi.fn(() => ({
                returning: vi
                  .fn()
                  .mockResolvedValue([{ id: 'mock-order-uuid', orderNo: 'ORD-RACE' }]),
              })),
            })),
            update: vi.fn(() => ({
              set: vi.fn(() => ({
      ...
      vi.mocked(db.query.quotes.findFirst).mockResolvedValue({
        id: 'f8e9d0c1-b2a3-44d5-8e6f-7a8b9c0d1e2f',
        status: 'ORDERED',
        customerId: 'c1',
        totalAmount: '1000',
        items: [],
      ...
      const requests = [
        createOrderHandler(
          createReq('http://localhost/api/orders', {
            quoteId: 'f8e9d0c1-b2a3-44d5-8e6f-7a8b9c0d1e2f',
          })
        ),
        createOrderHandler(
          createReq('http://localhost/api/orders', {
