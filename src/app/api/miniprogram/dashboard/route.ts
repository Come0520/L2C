    ...
    const cachedData = await CacheService.getOrSet(
      ...
      async () => {
        ...
        if (user.role === 'admin' || user.role === 'BOSS') {
          ...
          const pendingLeads = leadsStats
            .filter((s) => ['UNASSIGNED', 'PENDING_FOLLOWUP'].includes(s.status || ''))
            .reduce((acc, c) => acc + c.count, 0);
          const followingLeads = leadsStats
            .filter((s) =>
              ['PENDING_MEASUREMENT', 'PENDING_QUOTE', 'QUOTE_SENT'].includes(s.status || '')
            )
            .reduce((acc, c) => acc + c.count, 0);
          const wonLeads = leadsStats
            .filter((s) =>
              ['IN_PRODUCTION', 'PENDING_DELIVERY', 'PENDING_INSTALLATION'].includes(s.status || '')
            )
            .reduce((acc, c) => acc + c.count, 0);
          ...
          dbData.target = {
            ...
            achieved: totalCash,
            percentage:
              targetAmount > 0 ? Math.min(Math.round((totalCash / targetAmount) * 100), 100) : 0,
          };
          ...
          const myLeadsStats = await db
            ...
            .from(customers)
            .where(
              and(eq(customers.tenantId, user.tenantId), eq(customers.assignedSalesId, user.id))
            )
            .groupBy(customers.pipelineStatus);

          const totalLeads = myLeadsStats.reduce((acc, curr) => acc + curr.count, 0);
          const pendingLeads = myLeadsStats
            .filter((s) => ['PENDING_ASSIGNMENT', 'PENDING_FOLLOWUP'].includes(s.status || ''))
            .reduce((acc, c) => acc + c.count, 0);
          const followingLeads = myLeadsStats
            .filter((s) => (s.status as string) === 'FOLLOWING_UP')
            .reduce((acc, c) => acc + c.count, 0);
          const wonLeads = myLeadsStats
            .filter((s) => (s.status as string) === 'WON')
            .reduce((acc, c) => acc + c.count, 0);
          ...
          dbData.target = {
            ...
            achieved: myCash,
            percentage:
              targetAmount > 0 ? Math.min(Math.round((myCash / targetAmount) * 100), 100) : 0,
          };
