 ...
 * POST /api/miniprogram/leads — 创建新线索
 *
 * @description 复用 Web 端 createLeadSchema 进行数据验证，
 * 直接写入同一 leads 表，确保数据一致性。
 */
  ...
  try {
    ...
    if (input.customerPhone) {
      const existing = await db.query.leads.findFirst({
        where: and(eq(leads.tenantId, user.tenantId), eq(leads.customerPhone, input.customerPhone)),
