...
export const PaginationSchema = z.object({
    ...
    cursor: z.string().optional().describe('游标指针 (以某条记录的特征值为主键)，用于替代 offset 进行百万级极限深分页'),
});
...
export const InviteGenerateSchema = z.object({
    role: z.string().min(1, '请选择角色').describe('如 SALES(销售)、MEASURER(测量工)、INSTALLER(安装工)'),
...
export const CreateActivitySchema = z.object({
    ...
    images: z.array(z.string().url('必须为有效的 URL格式')).max(9).optional().default([]).describe('活动上传图组'),
...
const QuoteItemSchema = z.object({
    ...
    category: z.string().optional().default('GENERAL').describe('类目类型编码 (FABRIC, TRACK, MAIN...)'),
});
...
export const ConfirmQuoteSchema = z.object({
    signatureUrl: z.string().min(1, '签名URL不能为空').url('签名URL格式无效').describe('带签名的画板或承诺函 PDF 地址'),
});
...
export const SubmitMeasureDataSchema = z.object({
    rooms: z.array(z.object({
        ...
        })).min(1).describe('房间名下所有测过的各窗体表'),
    })).min(1, '至少提交一个房间的测量数据').describe('需要按房间打包分组结构'),
...
export const CreateServiceTicketSchema = z.object({
    ...
    type: z.enum(['REPAIR', 'RETURN', 'EXCHANGE', 'COMPLAINT', 'CONSULTATION']).describe('客诉归属枚举类型'),
