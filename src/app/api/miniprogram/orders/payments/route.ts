...
export const POST = withMiniprogramAuth(
  async (request: NextRequest, user) => {
    try {
      ...
      if (!schedule) {
        return apiNotFound('付款计划不存在');
      }

      if (schedule.status === 'PAID') {
        return apiBadRequest('该笔款项已支付');
      }
      ...
      try {
        await db.transaction(async (tx) => {
          ...
          const newBalance = (
            parseFloat(order.totalAmount as string) - parseFloat(newPaid)
          ).toFixed(2);

          // 状态流转：支付后从待付款变为生产中
          let statusToSet = order.status;
          if (
            (order.status === 'PENDING_PO' || order.status === 'DRAFT') &&
            parseFloat(newPaid) > 0
          ) {
      ...
      return apiServerError('由于系统或网络原因，收款款项记录保存失败');
    }
  },
