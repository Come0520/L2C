...
describe('微信小程序登录 API', () => {
  const createReq = (body: any) =>
    new NextRequest('http://localhost/api/mobile/auth/wechat', {
      method: 'POST',
      headers: {
  ...
  it('如果新用户登录（无手机号匹配）应当返回需要绑定状态并打标', async () => {
    ...
    expect(AuditService.log).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        action: 'LOGIN_FAILED',
        details: expect.objectContaining({ reason: 'need_binding' }),
      })
    );
  });
  ...
  it('如果 openid 未绑定但手机号匹配匹配，应当自动绑定并放行', async () => {
    ...
    expect(updateSetMock).toHaveBeenCalledWith(
      expect.objectContaining({ wechatOpenId: testOpenId })
    );
