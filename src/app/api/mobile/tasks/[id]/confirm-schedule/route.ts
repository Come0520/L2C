...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  if (taskType === 'measure') {
    await db.update(measureTasks).set(updateData).where(eq(measureTasks.id, taskId));
  } else {
    ...
    await db.update(installTasks).set(installUpdateData).where(eq(installTasks.id, taskId));
  }
