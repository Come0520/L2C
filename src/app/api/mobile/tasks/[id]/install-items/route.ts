        ...
        await tx
          ...
          .set({
            ...
            actualInstalledQuantity:
              item.actualInstalledQuantity !== undefined
                ? String(item.actualInstalledQuantity)
                : undefined,
            issueCategory: item.issueCategory || 'NONE',
            updatedAt: new Date(),
          })
          .where(and(eq(installItems.id, item.id), eq(installItems.tenantId, session.tenantId)));
      }
    });
