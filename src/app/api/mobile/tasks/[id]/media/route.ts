...
export async function POST(request: NextRequest, { params }: MediaParams) {
  ...
  if (taskType === 'install') {
    ...
    const sheet = await db.query.measureSheets.findFirst({
      where: and(eq(measureSheets.taskId, taskId), eq(measureSheets.tenantId, session.tenantId)),
    ...
    await db
      ...
      .where(and(eq(measureSheets.id, sheet.id), eq(measureSheets.tenantId, session.tenantId)));
  }
