...
import { ArrowLeft, MapPin, Phone, Clock, CheckCircle, Loader2, Navigation } from 'lucide-react';
...
function CheckInButton({ taskId, onSuccess }: { taskId: string; onSuccess: () => void }) {
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setLocationError(error.code === 1 ? '请允许获取位置权限' : '获取位置失败，请重试');
      },
  ...
  return (
    <div className="space-y-2">
      {locationError && <p className="text-center text-sm text-red-500">{locationError}</p>}
      {location && (
...
export default function MobileTaskDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="px-4 py-4">
      {/* 返回按钮 */}
      <button onClick={() => router.back()} className="mb-4 flex items-center text-gray-500">
      ...
      <div className="space-y-3">
        ...
        {task.status === 'PENDING_ACCEPT' && (
          <div className="flex gap-3">
            <Button variant="outline" className="h-12 flex-1" onClick={() => handleAccept(false)}>
              拒绝
            </Button>
            <Button className="h-12 flex-1" onClick={() => handleAccept(true)}>
              接单
            </Button>
          </div>
        )}
        ...
        {/* 待上门状态 - 打卡 */}
        {task.status === 'PENDING_VISIT' && <CheckInButton taskId={taskId} onSuccess={fetchTask} />}

        {/* 进行中状态 - 完工 */}
        {task.status === 'IN_PROGRESS' && (
          <Button className="h-12 w-full" onClick={handleComplete}>
