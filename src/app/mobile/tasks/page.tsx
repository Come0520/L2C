...
function TaskCard({ task, onClick }: { task: Task; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="mb-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm active:bg-gray-50 dark:border-zinc-700 dark:bg-zinc-800 dark:active:bg-zinc-700"
    >
...
export default function MobileTasksPage() {
  ...
  return (
    <div className="px-4 py-4">
      {/* Tab 切换 */}
      <div className="mb-4 flex rounded-lg bg-gray-100 p-1 dark:bg-zinc-800">
        {[
      ...
      {filteredTasks.length === 0 ? (
        <div className="py-12 text-center text-gray-500 dark:text-gray-400">
          暂无
          {activeTab === 'pending' ? '待处理' : activeTab === 'inProgress' ? '进行中' : '已完成'}
          任务
        </div>
      ) : (
