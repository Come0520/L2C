...
describe('Admin 模块 L5 安全测试', () => {
  ...
  describe('getWorkers 分页安全', () => {
    ...
    it('pageSize 超过 100 应抛出校验错误', async () => {
      const session = makeSession();
      await expect(getWorkers({ page: 1, pageSize: 999 }, session)).rejects.toThrow();
    });

    it('pageSize 为 0 应抛出校验错误', async () => {
      const session = makeSession();
      await expect(getWorkers({ page: 1, pageSize: 0 }, session)).rejects.toThrow();
    });

    it('page 为 0 应抛出校验错误', async () => {
      const session = makeSession();
      await expect(getWorkers({ page: 0, pageSize: 10 }, session)).rejects.toThrow();
    });

    it('非管理员无权限应被拒绝', async () => {
      mockCheckPermission.mockRejectedValue(new Error('权限不足'));
      const session = makeSession('sales');
      await expect(getWorkers({ page: 1, pageSize: 10 }, session)).rejects.toThrow('权限不足');
    });
  });
  ...
  describe('getWorkerById 安全', () => {
    it('不存在的师傅应抛出错误', async () => {
      const session = makeSession();
      await expect(getWorkerById('non-existent-id', session)).rejects.toThrow('未找到该师傅');
    });
