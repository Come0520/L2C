...
export async function getMfaConfig(session: Session): Promise<{
  ...
  try {
    ...
    logger.info(
      `[Admin] 用户 ${session.user.id} 正在查询租户 ${session.user.tenantId} 的 MFA 安全配置`
    );
 ...
 */
const updateTenantInfoInternal = createSafeAction(
 ...
 * @param context 包含 session 的上下文对象
 */
const updateMfaConfigInternal = createSafeAction(
  updateMfaConfigSchema,
  async (data, { session }) => {
    if (!(await checkPermission(session, PERMISSIONS.ADMIN.SETTINGS))) {
