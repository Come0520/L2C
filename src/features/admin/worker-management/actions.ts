...
const getWorkersParamsSchema = z.object({
  ...
  search: z
    .string()
    .trim()
    .max(MAX_SEARCH_LENGTH, `搜索关键词最长 ${MAX_SEARCH_LENGTH} 字符`)
    .optional(),
});
...
export async function getWorkers(
  ...
  logger.info(
    `[Admin] 用户 ${session.user.id} 正在查询师傅列表, 页码: ${params.page}, 关键词: ${params.search || '无'}`
  );
  ...
  const conditions = [eq(users.tenantId, session.user.tenantId), eq(users.role, 'WORKER')];
 ...
 */
const updateWorkerActionInternal = createSafeAction(
  updateWorkerSchema,
  async (data, { session }) => {
    if (!(await checkPermission(session, PERMISSIONS.SETTINGS.USER_MANAGE))) {
