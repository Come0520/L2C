...
import { createAfterSalesTicket, updateTicketStatus, closeResolutionCostClosure } from '../actions/ticket';
import { createLiabilityNotice, submitLiabilityNotice, disputeLiabilityNotice, arbitrateLiabilityNotice, confirmLiabilityNotice } from '../actions/liability';
...
vi.mock('@/shared/api/db', () => ({
    db: {
        ...
        insert: vi.fn(() => ({
            values: vi.fn(() => ({
                returning: vi.fn(() => Promise.resolve([{ id: 'mock-id', ticketNo: 'AS123', noticeNo: 'LN123' }]))
            }))
        })),
        update: vi.fn(() => ({
            set: vi.fn(() => ({
...
describe('After-Sales Integration Tests', () => {
    ...
    it('should complete full ticket lifecycle: create -> investigate -> close with audit', async () => {
        ...
        (db.transaction as any).mockImplementationOnce(async (cb: any) => {
            const tx = {
                query: {
                    orders: { findFirst: vi.fn().mockResolvedValue({ id: VALID_ORDER_ID, tenantId: VALID_TENANT_ID, customerId: 'c1', version: 1, updatedAt: new Date() }) }
                },
                insert: vi.fn(() => ({
                    values: vi.fn(() => ({
        ...
        expect(AuditService.recordFromSession).toHaveBeenCalledWith(expect.anything(), 'after_sales_tickets', VALID_TICKET_ID, 'CREATE', expect.anything());

        // 2. Update to INVESTIGATING
        (db.query.afterSalesTickets.findFirst as any).mockResolvedValue({ id: VALID_TICKET_ID, status: 'PENDING', tenantId: VALID_TENANT_ID });
        const updateRes = await updateTicketStatus({ ticketId: VALID_TICKET_ID, status: 'INVESTIGATING' });
        expect(updateRes.success).toBe(true);
        expect(AuditService.recordFromSession).toHaveBeenCalledWith(expect.anything(), 'after_sales_tickets', VALID_TICKET_ID, 'UPDATE', expect.objectContaining({
    ...
    it('should process liability notice workflow with dispute and arbitration', async () => {
        ...
        (db.transaction as any).mockImplementationOnce(async (cb: any) => {
            const tx = {
                query: {
                    afterSalesTickets: { findFirst: vi.fn().mockResolvedValue({ id: VALID_TICKET_ID, tenantId: VALID_TENANT_ID, status: 'PROCESSING' }) }
                },
                insert: vi.fn(() => ({
                    values: vi.fn(() => ({
        ...
        (db.query.liabilityNotices.findFirst as any).mockResolvedValue({ id: VALID_NOTICE_ID, status: 'DRAFT', tenantId: VALID_TENANT_ID, version: 1, updatedAt: new Date() });
        await submitLiabilityNotice({ noticeId: VALID_NOTICE_ID });
        expect(AuditService.recordFromSession).toHaveBeenCalledWith(expect.anything(), 'liability_notices', VALID_NOTICE_ID, 'UPDATE', expect.objectContaining({
            new: expect.objectContaining({ status: 'PENDING_CONFIRM' })
        }));

        // 3. Dispute
        (db.query.liabilityNotices.findFirst as any).mockResolvedValue({ id: VALID_NOTICE_ID, status: 'PENDING_CONFIRM', tenantId: VALID_TENANT_ID, version: 1, updatedAt: new Date() });
        ...
        // 4. Arbitrate
        (db.query.liabilityNotices.findFirst as any).mockResolvedValue({ id: VALID_NOTICE_ID, status: 'DISPUTED', tenantId: VALID_TENANT_ID, version: 1, updatedAt: new Date() });
        ...
        (db.transaction as any).mockImplementationOnce(async (cb: any) => {
            const tx = {
                query: {
                    liabilityNotices: { findFirst: vi.fn().mockResolvedValue({ id: VALID_NOTICE_ID, status: 'DRAFT', tenantId: VALID_TENANT_ID, afterSalesId: VALID_TICKET_ID, amount: '100', liablePartyType: 'FACTORY', liablePartyId: VALID_PARTY_ID, version: 1, updatedAt: new Date() }) }
                },
                update: vi.fn(() => ({ set: vi.fn(() => ({ where: vi.fn().mockResolvedValue({}) })) })),
                select: vi.fn(() => ({ from: vi.fn(() => ({ where: vi.fn().mockResolvedValue([{ total: '100' }]) })) })),
            };
            return cb(tx);
        });
