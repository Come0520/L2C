 ...
 * 3. 工单状态分布。
 */
const getAfterSalesQualityAnalyticsAction = createSafeAction(
  getQualityAnalyticsSchema,
  async (params, { session }) => {
    ...
    return {
      ...
      summary: {
        totalLiabilityAmount: liabilityByParty.reduce(
          (acc, item) => acc + parseFloat(item.totalAmount?.toString() || '0'),
          0
        ),
        totalLiabilityCount: liabilityByParty.reduce((acc, item) => acc + Number(item.count), 0),
      },
    };
  }
);
