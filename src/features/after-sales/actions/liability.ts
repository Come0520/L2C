 ...
 */
const createLiabilityNoticeAction = createSafeAction(
  createLiabilitySchema,
  async (data, { session }) => {
    ...
    try {
      ...
      logger.info(
        `[After Sales] Successfully created liability notice: ${newNotice.id} for ticket: ${data.afterSalesId}`
      );
 ...
 */
const confirmLiabilityNoticeAction = createSafeAction(
  confirmLiabilitySchema,
  async (data, { session }) => {
    const tenantId = session.user.tenantId;
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const notice = await tx.query.liabilityNotices.findFirst({
        where: and(eq(liabilityNotices.id, data.noticeId), eq(liabilityNotices.tenantId, tenantId)),
      });

      if (!notice) return { success: false, message: '定责单不存在或无权操作' };
      if (notice.status !== 'DRAFT')
        return { success: false, message: '只有草稿状态的定责单可以确认' };
    ...
    if (
      result.notice &&
      result.notice.liablePartyType === 'FACTORY' &&
      result.notice.liablePartyId
    ) {
      try {
 ...
 */
const submitLiabilityNoticeAction = createSafeAction(
  confirmLiabilitySchema,
  async (data, { session }) => {
    const tenantId = session.user.tenantId;
    ...
    try {
      ...
      logger.error('[After Sales] Submit Liability Notice Failed:', {
        error: err,
 ...
 */
const disputeLiabilityNoticeAction = createSafeAction(
  disputeLiabilitySchema,
  async (data, { session }) => {
    const tenantId = session.user.tenantId;
    ...
    try {
      ...
      if (!notice) return { success: false, message: '定责单不存在' };
      if (notice.status !== 'PENDING_CONFIRM')
        return { success: false, message: '只有待确认的定责单可以提起争议' };
      ...
      logger.info(
        `[After Sales] Liability notice ${data.noticeId} disputed with reason: ${data.disputeReason}`
      );

      return { success: true, message: '已提交争议原因，等待仲裁' };
    } catch (err: unknown) {
      logger.error('[After Sales] Dispute Liability Notice Failed:', {
        error: err,
        noticeId: data.noticeId,
      });
      return { success: false, message: err instanceof Error ? err.message : '服务器内部错误' };
 ...
 */
const arbitrateLiabilityNoticeAction = createSafeAction(
  arbitrateLiabilitySchema,
  async (data, { session }) => {
    const tenantId = session.user.tenantId;
    ...
    try {
      ...
      if (!notice) return { success: false, message: '定责单不存在' };
      if (notice.status !== 'DISPUTED')
        return { success: false, message: '只有争议中的定责单可以仲裁' };
      ...
      logger.info(
        `[After Sales] Liability notice ${data.noticeId} arbitrated by user ${session.user.id}`
      );

      return { success: true, message: '仲裁完成' };
    } catch (err: unknown) {
      logger.error('[After Sales] Arbitrate Liability Notice Failed:', {
        error: err,
        noticeId: data.noticeId,
      });
      return { success: false, message: err instanceof Error ? err.message : '服务器内部错误' };
