...
export function ArbitrationWorkbench({
  ...
  // 更新责任方金额
  const updateLiabilityAmount = useCallback(
          ...
          <CardContent>
            <ScrollArea className="h-[300px] pr-4">
              <div className="relative space-y-4">
                {/* 时间轴线 */}
                <div className="bg-border absolute top-2 bottom-2 left-[7px] w-0.5" />

                {timeline.map((event, index) => (
                  <div key={event.id} className="relative flex gap-4 pl-6">
                    {/* 时间点 */}
                    <div
                      className={`absolute left-0 h-4 w-4 rounded-full ${eventTypeColor(event.type)} flex items-center justify-center`}
                    >
                    ...
                    <div className="flex-1 pb-4">
                      ...
                      {event.actor && (
                        <p className="text-muted-foreground mt-1 text-xs">操作人: {event.actor}</p>
                      )}
                      {event.photos && event.photos.length > 0 && (
                        <div className="mt-2 flex gap-2">
                          {event.photos.map((photo, i) => (
                            <button
                              key={i}
                              className="ring-primary h-12 w-12 overflow-hidden rounded border hover:ring-2"
          ...
          <CardContent>
            <div className="grid grid-cols-4 gap-2">
              {photos.map((photo, index) => (
                <button
                  key={index}
                  className={`ring-primary aspect-square overflow-hidden rounded border hover:ring-2 ${
                    selectedEvidence === photo ? 'ring-2' : ''
                  }`}
                  onClick={() => setSelectedEvidence(photo)}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {liabilities.length > 0 ? (
              ...
              <div className="text-muted-foreground py-8 text-center">请选择责任方</div>
            )}
              ...
              <div className="bg-muted h-2 overflow-hidden rounded-full">
                <div
                  className={`h-full transition-all ${
                    remaining === 0 ? 'bg-green-500' : 'bg-orange-500'
                  }`}
