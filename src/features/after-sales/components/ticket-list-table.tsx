...
export function TicketListTable({ tickets, isLoading }: TicketListTableProps) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-white/50 shadow-sm backdrop-blur-sm">
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            <TableRow>
              <TableCell
                colSpan={6}
                className="text-muted-foreground animate-pulse py-12 text-center"
              >
                数据加载中...
              </TableCell>
            </TableRow>
              ...
              <TableRow key={ticket.id} className="hover:bg-muted/10 transition-colors">
                ...
                <TableCell>
                  <div className="flex flex-col gap-1.5">
                    <span
                      className={`w-fit rounded border px-2 py-0.5 text-[10px] font-bold ${getPriorityColor(ticket.priority || '')}`}
                    >
                      {ticket.priority}
                    </span>
                    <Badge
                      variant={
                        getStatusVariant(ticket.status) as
                          | 'default'
                          | 'secondary'
                          | 'destructive'
                          | 'outline'
