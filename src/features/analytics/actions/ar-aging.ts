 ...
 */
const getARAgingAnalysisAction = createSafeAction(
  arAgingAnalysisSchema,
        ...
        try {
          ...
          pendingARStatements.forEach((statement) => {
            const ageDays = Math.floor(
              (asOfDate.getTime() - new Date(statement.createdAt ?? Date.now()).getTime()) /
                (1000 * 60 * 60 * 24)
            );
          ...
          const totalPending = Object.values(agingBuckets).reduce(
            (sum, bucket) => sum + bucket.amount,
            0
          );

          logger.info('AR 账龄分析查询成功', {
            tenantId,
            totalPendingAmount: totalPending,
            count: pendingARStatements.length,
          });
            ...
            agingBuckets: [
              {
                ...
                percentage:
                  totalPending > 0
                    ? ((agingBuckets.current.amount / totalPending) * 100).toFixed(1)
                    : '0',
              },
              {
                range: '31-60天',
                ...
                percentage:
                  totalPending > 0
                    ? ((agingBuckets.days30.amount / totalPending) * 100).toFixed(1)
                    : '0',
              },
              {
                range: '61-90天',
                ...
                percentage:
                  totalPending > 0
                    ? ((agingBuckets.days60.amount / totalPending) * 100).toFixed(1)
                    : '0',
              },
              {
                range: '90天以上',
                ...
                percentage:
                  totalPending > 0
                    ? ((agingBuckets.days90Plus.amount / totalPending) * 100).toFixed(1)
                    : '0',
                riskLevel: 'HIGH',
              },
            ],
