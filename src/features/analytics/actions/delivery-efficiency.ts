 ...
 */
const getDeliveryEfficiencyAction = createSafeAction(
  deliveryEfficiencySchema,
      ...
      async () => {
        try {
          ...
          const result = {
            ...
            installOnTimeRate,
            totalPendingTasks:
              Number(pendingMeasure[0]?.count || 0) + Number(pendingInstall[0]?.count || 0),
            overdueTaskCount:
              Number(overdueMeasure[0]?.count || 0) + Number(overdueInstall[0]?.count || 0),
          };
