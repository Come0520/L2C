  ...
  return unstable_cache(
    async () => {
      try {
        ...
        const calcConversion = (cur: number, prev: number) =>
          prev > 0 ? ((cur / prev) * 100).toFixed(1) : null;
          ...
          stages: [
            {
              ...
              trend: calcTrend(currentLeads, prevLeads),
            },
            {
              stage: '测量',
              count: currentMeasures,
              conversionRate: calcConversion(currentMeasures, currentLeads),
              avgDaysInStage: Number(measureStats[0]?.avgDays || 0).toFixed(1),
              previousPeriodCount: prevMeasures,
              trend: calcTrend(currentMeasures, prevMeasures),
            },
            {
              stage: '报价',
              count: currentQuotes,
              conversionRate: calcConversion(currentQuotes, currentMeasures),
              avgDaysInStage: Number(quoteStats[0]?.avgDays || 0).toFixed(1),
              previousPeriodCount: prevQuotes,
              trend: calcTrend(currentQuotes, prevQuotes),
            },
            {
              stage: '成交',
              count: currentOrders,
              conversionRate: calcConversion(currentOrders, currentQuotes),
              avgDaysInStage: Number(orderStats[0]?.avgDays || 0).toFixed(1),
              previousPeriodCount: prevOrders,
              trend: calcTrend(currentOrders, prevOrders),
            },
          ],
          summary: {
            overallConversion:
              currentLeads > 0 ? ((currentOrders / currentLeads) * 100).toFixed(1) : '0',
            avgCycleTime: (
        ...
        logger.info('销售漏斗分析查询成功', {
          tenantId,
          overallConversion: result.summary.overallConversion,
        });
        return result;
      } catch (error) {
