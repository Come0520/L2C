        ...
        {currentLevel.data.length === 0 ? (
          ...
          currentLevel.data.map((item) => (
            <TableRow key={item.id} className="hover:bg-muted/50">
              ...
              <TableCell className="text-right font-mono">{renderValue(item.value)}</TableCell>
...
// 导出钻取 Hook
export function useDrilldown<T extends DrilldownItem>(fetchFn: (parentId: string) => Promise<T[]>) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const drilldown = useCallback(
    async (item: DrilldownItem): Promise<T[]> => {
      setIsLoading(true);
      setError(null);
      try {
