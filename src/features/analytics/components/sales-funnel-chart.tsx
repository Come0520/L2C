...
export function SalesFunnelChart({ data, className, summary }: SalesFunnelChartProps) {
  return (
    <AnalyticsErrorBoundary fallbackTitle="销售漏斗图表加载失败">
              ...
              >
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="z-50 rounded-lg border bg-white p-3 text-sm shadow-lg">
                          <div className="mb-1 font-bold">{item.stage}</div>
                          ...
                          {item.trend !== undefined && item.trend !== null && (
                            <div className="mb-1 flex justify-between gap-4">
                              ...
                              <span
                                className={
                                  Number(item.trend) >= 0 ? 'text-green-600' : 'text-red-500'
                                }
                              >
                                {Number(item.trend) > 0 ? '+' : ''}
                                {item.trend}%
                ...
                <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                  {data.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={STAGE_COLORS[entry.stage as keyof typeof STAGE_COLORS] || '#cbd5e1'}
                    />
                  ))}
                </Bar>
              </BarChart>
              ...
              return (
                <div key={item.stage} className="relative">
                  ...
                  {index > 0 && item.conversionRate !== null && (
                    <div className="absolute -top-3 left-14 flex w-full items-center justify-center">
                      <div className="flex -translate-y-1/2 transform items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-500">
                    ...
                    <div className="flex flex-col">
                      ...
                      {item.avgDaysInStage && (
                        <span className="text-muted-foreground flex items-center gap-1 text-xs">
                          <Clock className="h-3 w-3" /> {item.avgDaysInStage}天
                        </span>
                      )}
                    </div>
