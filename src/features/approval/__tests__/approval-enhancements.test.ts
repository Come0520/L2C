...
describe('Approval Enhancements', () => {
  ...
  it('should handle Parallel Approval (ANY)', async () => {
    ...
    // 2. Create Entity
    const [bill] = (await db
      .insert(schema.paymentBills)
      .values({
        tenantId,
        paymentNo: `BILL_ANY_${Date.now()}`,
        payeeType: 'SUPPLIER',
        payeeId: adminId,
        payeeName: 'Sup',
        amount: '100',
