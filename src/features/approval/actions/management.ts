 ...
 * @returns 撤回结果
 */
export async function withdrawApproval(payload: { instanceId: string; reason?: string }) {
  // 1. 输入校验
  ...
  logger.info(
    `[Approval-Management] User ${session.user.id} attempting to withdraw approval ${payload.instanceId}`,
    {
  ...
  return db.transaction(async (tx) => {
    ...
    if (!instance) {
      logger.warn(
        `[Approval-Management] Withdrawal failed: instance ${payload.instanceId} not found`
      );
    ...
    if (instance.requesterId !== session.user.id) {
      logger.warn(
        `[Approval-Management] Unauthorized withdrawal attempt: user ${session.user.id} for instance ${instance.id}`
      );
    ...
    if (instance.status !== 'PENDING') {
      logger.warn(
        `[Approval-Management] Withdrawal failed: instance ${instance.id} is in status ${instance.status}`
      );
    ...
    await tx
      ...
      .where(
        and(eq(approvalTasks.approvalId, payload.instanceId), eq(approvalTasks.status, 'PENDING'))
      );
    ...
    logger.info(
      `[Approval-Management] Successfully withdrawn approval ${instance.id} for entity ${instance.entityType}:${instance.entityId}`
    );
