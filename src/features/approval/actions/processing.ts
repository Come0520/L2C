...
import { approvals, approvalNodes, approvalTasks, users } from '@/shared/api/schema';
...
export async function _processApprovalLogic(
  ...
  if (!task || !task.approval || !task.node) {
    logger.error(
      `[Approval] Task not found or invalid: ${payload.taskId}, tenant: ${session.user.tenantId}`
    );
  ...
  if (!currentTask || currentTask.status !== 'PENDING') {
    logger.warn(
      `[Approval] Task ${payload.taskId} already processed or canceled. Current status: ${currentTask?.status}`
    );
  ...
  if (!isSystemCall) {
    if (task.approverId && task.approverId !== session.user.id) {
      logger.warn(
        `[Approval] Unauthorized access to task ${payload.taskId} by user ${session.user.id}`
      );
  ...
  logger.info(
    `[Approval] Processing task ${payload.taskId}, action: ${payload.action}, user: ${session.user.id}`
  );
  ...
  if (payload.action === 'REJECT') {
    ...
    if (shouldRejectWholeFlow) {
      ...
      try {
        await revertEntityStatus(
          tx,
          task.approval.entityType,
          task.approval.entityId,
          task.tenantId,
          'REJECTED'
        );
        logger.info(
          `[Approval] Flow ${task.approvalId} REJECTED, entity ${task.approval.entityType}:${task.approval.entityId} reverted.`
        );
      } catch (err) {
      ...
      if (nextNode) {
        ...
        if (nextNode.approverUserId) {
          ...
          if (nextApprovers.length === 0) {
            logger.error(
              `[Approval-NextNode] No active users found representing role: ${nextNode.approverRole}`
            );
          }
        }
        ...
        const isNextApproverSelf =
          nextApprovers.length === 1 && nextApprovers[0] === session.user.id;
        ...
        for (const approver of nextApprovers) {
          ...
          if (isNextApproverSelf && finalApprover === session.user.id) {
            logger.info(
              `[Auto-Approval] Node ${nextNode.name} automatically approved by ${session.user.id}`
            );
            const subResult = await _processApprovalLogic(
              tx,
              {
                taskId: newTask.id,
                action: 'APPROVE',
        ...
        try {
          ...
          logger.info(
            `[Approval] Flow ${task.approvalId} fully APPROVED, entity ${task.approval.entityType}:${task.approval.entityId} completed.`
          );
        } catch (err) {
  ...
  if (payload.action === 'REJECT') {
    const finalApproval = await tx.query.approvals.findFirst({
      where: eq(approvals.id, task.approvalId),
    });
...
export async function processApproval(payload: {
  ...
  logger.info(
    `[Approval-Process] User ${session.user.id} attempting ${payload.action} for task ${payload.taskId}`,
    {
      tenantId: session.user.tenantId,
      comment: payload.comment,
    }
  ...
  return db.transaction(async (tx) => {
    ...
    if (result.success) {
      ...
      if (result.pendingNotifications?.length) {
        import('../services/approval-notification.service').then(
      ...
      logger.info(
        `[Approval-Process] Successfully processed task ${payload.taskId} (${payload.action})`
      );
...
export async function addApprover(payload: {
  ...
  logger.info(
    `[Approval-AddApprover] User ${session.user.id} adding approver ${payload.targetUserId} for task ${payload.taskId}`
  );
  return await db.transaction(async (tx) => {
    ...
    if (task.approverId !== session.user.id) {
      logger.warn(
        `[Approval-AddApprover] User ${session.user.id} unauthorized to add approver to task ${task.id}`
      );
