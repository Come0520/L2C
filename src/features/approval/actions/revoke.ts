...
export async function revokeApprovalAction(approvalId: string) {
  ...
  logger.info(
    `[Approval-Revoke] User ${session.user.id} attempting to revoke action for ${approvalId}`
  );

  return db.transaction(async (tx) => {
    const approval = await tx.query.approvals.findFirst({
      where: and(eq(approvals.id, approvalId), eq(approvals.tenantId, session.user.tenantId)),
      with: {
        tasks: true,
      },
    });
    ...
    if (!approval) {
      logger.warn(
        `[Approval-Revoke] Approval ${approvalId} not found for tenant ${session.user.tenantId}`
      );
      return { success: false, error: '审批不存在' };
    }

    const isInitiator = approval.requesterId === session.user.id;
    const isApprover = approval.tasks.some(
      (t) => t.approverId === session.user.id && t.status === 'APPROVED'
    );

    if (!isInitiator && !isApprover) {
      logger.warn(
        `[Approval-Revoke] User ${session.user.id} has no permission to revoke ${approvalId}`
      );
    ...
    if (isInitiator) {
      ...
      await tx
        ...
        .where(and(eq(approvalTasks.approvalId, approvalId), eq(approvalTasks.status, 'PENDING')));
      ...
      const othersActed = subsequentActionTasks.some(
        (t) => t.node && (t.node.sortOrder || 0) > (lastNode.sortOrder || 0)
      );
      ...
      logger.info(
        `[Approval-Revoke] Approver ${session.user.id} rolling back task ${lastTask.id} for ${approvalId}`
      );
      ...
      const idsToDelete = allPendingTasks.filter((t) => t.id !== lastTask.id).map((t) => t.id);
