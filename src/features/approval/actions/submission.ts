...
export async function submitApproval(
  ...
  logger.info(
    `[Approval-Submit] User ${requesterId} submitting ${payload.flowCode} for ${payload.entityType}:${payload.entityId}`,
    {
      tenantId,
  ...
  const run = async (tx: Transaction) => {
    ...
    if (!flow) {
      logger.error(
        `[Approval-Submit] Flow definition not found or disabled: ${payload.flowCode}, tenant: ${tenantId}`
      );
      throw new Error(`审批流程未定义或已禁用: ${payload.flowCode}`);
    }

    logger.info(
      `[Approval-Submit] Found flow: ${flow.name} (${flow.code}) for entity: ${payload.entityType}:${payload.entityId}`
    );
    ...
    const activeNodes = allNodes.filter((node) => {
      ...
      if (!amountMatch) {
        logger.info(
          `[Approval-Submit] Node ${node.name} amount mismatch: ${amountNum} not in [${min}, ${max}]`
        );
        return false;
      }
    ...
    if (!firstNode) {
      logger.warn(
        `[Approval-Submit] No matching nodes for flow ${payload.flowCode}, amount: ${payload.amount}`
      );
      throw new Error(`当前业务金额 [${payload.amount}] 不在任何审批环节范围内`);
    }

    logger.info(
      `[Approval-Submit] Matched ${activeNodes.length} nodes, starting with node: ${firstNode.name}`
    );
    ...
    for (const userId of approverIds) {
      const actualApproverId = await ApprovalDelegationService.getEffectiveApprover(
        userId,
        flow.id,
        tenantId
      );
      ...
      logger.info(
        `[Approval-Submit] Created task ${newTask.id} for user ${actualApproverId}${actualApproverId !== userId ? ' (Delegated)' : ''}`
      );
    }
