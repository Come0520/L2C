...
export async function processTimeouts() {
  ...
  const overdueTasks = await db.query.approvalTasks.findMany({
    where: and(eq(approvalTasks.status, 'PENDING'), lte(approvalTasks.timeoutAt, now)),
    with: {
      node: true,
      approval: {
        with: {
...
async function processTimeout(task: {
  ...
  return db.transaction(async (tx) => {
    ...
    const freshTask = await tx.query.approvalTasks.findFirst({
      where: and(eq(approvalTasks.id, task.id), eq(approvalTasks.status, 'PENDING')),
    });

    if (!freshTask) {
      logger.warn(
        `[Approval-Timeout] Task ${task.id} already processed or status changed, skipping.`
      );
      return;
    }
