 ...
 * @returns 是否匹配
 */
export function checkApproverMatch(
...
export async function revertEntityStatus(
  ...
  switch (entityType) {
    ...
    case 'ORDER_CANCEL':
      ...
      break;
    case 'CUSTOMER_MERGE':
      // 客户合并没有独立的实体状态字段，驳回/撤回时无需回退
      logger.info(
        `[Approval] Customer merge ${entityId} reverted to ${targetStatus} - no entity status change needed`
      );
      break;
    default:
...
export async function completeEntityStatus(
  ...
  switch (entityType) {
    ...
    case 'CUSTOMER_MERGE': {
      ...
      const mergeApproval = await tx.query.approvals.findFirst({
        where: and(
          eq(approvalsTable.entityType, 'CUSTOMER_MERGE'),
      ...
      if (mergeApproval?.comment) {
        try {
          const mergeParams = JSON.parse(mergeApproval.comment);
          const { CustomerService } = await import('@/services/customer.service');
          await CustomerService.mergeCustomers(
            entityId,
            mergeParams.sourceCustomerIds,
            mergeParams.fieldPriority || 'PRIMARY',
            tenantId,
            mergeApproval.requesterId
          );
          logger.info(`[Approval] Customer merge completed for ${entityId}`, { mergeParams });
        } catch (mergeErr) {
          logger.error(`[Approval] Customer merge execution failed for ${entityId}`, mergeErr);
          throw mergeErr;
        }
      } else {
        logger.error(`[Approval] No merge params found in approval comment for ${entityId}`);
      }
      break;
    }
    default:
