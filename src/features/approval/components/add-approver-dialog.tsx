    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2">
          <UserPlus className="h-4 w-4" /> 邀请协作 (加签)
        </Button>
      </DialogTrigger>
          ...
          <div className="max-h-[200px] space-y-2 overflow-y-auto rounded-md border p-2">
            {users.length === 0 ? (
              ...
              users.map((u) => (
                <div
                  key={u.id}
                  className={`flex cursor-pointer items-center justify-between rounded-md p-2 transition-colors ${selectedUserId === u.id ? 'bg-primary/10 border-primary border' : 'hover:bg-muted border border-transparent'}`}
