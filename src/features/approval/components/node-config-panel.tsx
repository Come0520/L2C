...
export function NodeConfigPanel({ selectedNode, onUpdate, onClose }: NodeConfigPanelProps) {
  ...
  return (
    <Card className="bg-background/95 absolute top-0 right-0 z-10 h-full w-[300px] rounded-none border-l shadow-xl backdrop-blur">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label>节点名称</Label>
          <Input value={selectedNode.data.label} onChange={handleLabelChange} />
        </div>
            ...
            {(selectedNode.data.approverType === 'ROLE' ||
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    onUpdate(selectedNode.id, { approverMode: val as 'ANY' | 'ALL' | 'MAJORITY' })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
