...
import { requestResetSchema, resetPasswordSchema } from '../schemas/password-reset-schema';
...
// BCRYPT_ROUNDS 从共享常量模块导入（src/shared/lib/auth-constants.ts），
// 保证所有模块的密码哈希轮数一致。
 ...
 *
 * @returns {Promise<{ user: unknown, vcId: string } | null>} 返回携带对应合法用户的映射及凭证ID，或 null
 */
