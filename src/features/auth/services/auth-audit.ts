...
export class AuthAuditService {
     ...
     *
     * @param {any} db - 活动的持久游标，可以是单次 DB 对象或者 Transaction 的句柄对象
     * @param {LoginSuccessOptions} options - 从鉴权拦截器萃取的数据切片对象
     *
     * @returns {Promise<void>} 挂载进系统库表的异步确认过程
     */
    static async logLoginSuccess(db: any, { userId, tenantId, userAgent }: LoginSuccessOptions) {
     ...
     *
     * @param {any} db - 底层驱动游标/事务
     * @param {MagicLinkLoginOptions} options - 魔术连线所涉及的发端人和落脚人双方信息
     *
     * @returns {Promise<void>} 挂载过程
     */
    static async logMagicLinkLogin(db: any, { userId, tenantId, generatedBy }: MagicLinkLoginOptions) {
        return AuditService.log(db, {
            tableName: 'users',
            recordId: userId,
            action: 'MAGIC_LOGIN',
     ...
     */
    static async logLoginFailed(db: any, { username, reason, userAgent }: LoginFailedOptions) {
        return AuditService.log(db, {
            tableName: 'users',
            recordId: 'auth',
            action: 'LOGIN_FAILED',
     ...
     *
     * @param {any} db - 数据库访问控制类驱动
     * @param {LogoutOptions} options - 含主谓宾识别的基本实体标识
     *
     * @returns {Promise<void>} 落日志入库
     */
    static async logLogout(db: any, { userId, tenantId }: LogoutOptions) {
        return AuditService.log(db, {
            tableName: 'users',
            recordId: userId,
            action: 'LOGOUT',
     ...
     *
     * @param {any} db - 数据库事务对象
     * @param {PasswordResetRequestedOptions} options - 发起人对象体集
     *
     * @returns {Promise<void>} 落库存档
     */
    static async logPasswordResetRequested(db: any, { userId, tenantId, email }: PasswordResetRequestedOptions) {
        return AuditService.log(db, {
            tableName: 'users',
            recordId: userId,
     ...
     *
     * @param {any} db - 数据库挂件上下文
     * @param {PasswordResetCompletedOptions} options - 操作完成落地标量
     *
     * @returns {Promise<void>} 落库存档
     */
    static async logPasswordResetCompleted(db: any, { userId, tenantId }: PasswordResetCompletedOptions) {
        return AuditService.log(db, {
            tableName: 'users',
            recordId: userId,
