...
export async function createChannelCategory(input: z.infer<typeof channelCategorySchema>) {
  ...
  const existing = await db.query.channelCategories.findFirst({
    where: and(
      ...
      or(eq(channelCategories.code, validated.code), eq(channelCategories.name, validated.name))
    ),
...
export async function updateChannelCategory(
  id: string,
  input: z.infer<typeof channelCategorySchema>
) {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('Unauthorized');
    ...
    if (checks.length > 0) {
      ...
      if (existing) {
        if (validated.code !== undefined && existing.code === validated.code)
          throw new Error(`分类编码 ${validated.code} 已存在`);
        if (validated.name !== undefined && existing.name === validated.name)
          throw new Error(`分类名称 ${validated.name} 已存在`);
      }
    }
  }
...
export async function deleteChannelCategory(id: string) {
  ...
  const hasChannels = await db.query.channels.findFirst({
    where: and(eq(channels.categoryId, id), eq(channels.tenantId, tenantId)),
  });
...
export async function toggleChannelCategoryActive(id: string, _isActive: boolean) {
  ...
  if (current.isActive) {
    const activeChannelsCount = await db.$count(
      channels,
      and(
