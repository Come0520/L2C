...
export async function createCommissionRecord(params: {
  ...
  const result = await import('@/features/channels/logic/commission.service').then((m) =>
    m.calculateOrderCommission(order, channel)
  );
...
export async function getPendingCommissionSummary() {
  ...
  const result = await db
    ...
    .where(and(eq(channelCommissions.tenantId, tenantId), eq(channelCommissions.status, 'PENDING')))
  ...
  const channelList =
    channelIds.length > 0
      ? await db.query.channels.findMany({
          where: and(eq(channels.tenantId, tenantId), inArray(channels.id, channelIds)),
        })
      : [];
...
export async function voidCommission(id: string, reason: string) {
  ...
  const commission = await db.query.channelCommissions.findFirst({
    where: and(eq(channelCommissions.id, id), eq(channelCommissions.tenantId, tenantId)),
