...
export async function createChannel(input: ChannelInput) {
  ...
  const existing = await db.query.channels.findFirst({
    where: and(
      ...
      or(eq(channels.channelNo, validated.channelNo), eq(channels.name, validated.name))
    ),
    columns: { id: true, name: true, channelNo: true },
  });

  if (existing) {
    if (existing.channelNo === validated.channelNo)
      throw new Error(`渠道编码 ${validated.channelNo} 已存在`);
  ...
  return await db.transaction(async (tx) => {
    const [newChannel] = await tx
      ...
      .values({
        ...
        priceDiscountRate: validated.priceDiscountRate
          ? validated.priceDiscountRate.toString()
          : null,
...
export async function updateChannel(id: string, input: Partial<ChannelInput>) {
  ...
  if (validated.commissionRate !== undefined)
    updateData.commissionRate = validated.commissionRate?.toString();
  ...
  if (validated.cooperationMode !== undefined)
    updateData.cooperationMode = validated.cooperationMode;
  if (validated.priceDiscountRate !== undefined)
    updateData.priceDiscountRate = validated.priceDiscountRate?.toString();
  ...
  if (validated.customChannelType !== undefined)
    updateData.customChannelType = validated.customChannelType;

  if (validated.assignedManagerId !== undefined)
    updateData.assignedManagerId = validated.assignedManagerId;
  ...
  if (validated.creditLimit !== undefined)
    updateData.creditLimit = validated.creditLimit?.toString();
  if (validated.commissionTriggerMode !== undefined)
    updateData.commissionTriggerMode = validated.commissionTriggerMode;
  ...
  if (validated.name !== undefined || validated.channelNo !== undefined) {
    ...
    if (checks.length > 0) {
      ...
      if (existing) {
        if (validated.channelNo !== undefined && existing.channelNo === validated.channelNo)
          throw new Error(`渠道编码 ${validated.channelNo} 已存在`);
        if (validated.name !== undefined && existing.name === validated.name)
          throw new Error(`渠道名称 ${validated.name} 已存在`);
      }
    }
  }
...
export async function deleteChannel(id: string) {
  ...
  await db.transaction(async (tx) => {
    ...
    const childrenCount = await tx.$count(
      channels,
      and(eq(channels.parentId, id), eq(channels.tenantId, tenantId))
    );
    if (childrenCount > 0)
      throw new Error(`该渠道下存在 ${childrenCount} 个子渠道，无法删除。请先处理子渠道。`);

    // 2. 检查线索 (C-04 Fix: Add tenantId filter)
    const leadsCount = await tx.$count(
      leads,
      and(eq(leads.channelId, id), eq(leads.tenantId, tenantId))
    );
    if (leadsCount > 0)
      throw new Error(`该渠道下存在 ${leadsCount} 条关联线索，无法删除。请先转移或清理线索。`);

    // 3. 检查佣金记录 (C-04 Fix: Add tenantId filter)
    const commissionsCount = await tx.$count(
      channelCommissions,
      and(eq(channelCommissions.channelId, id), eq(channelCommissions.tenantId, tenantId))
    );
    if (commissionsCount > 0)
      throw new Error(`该渠道存在 ${commissionsCount} 条佣金记录，无法删除。`);

    await tx.delete(channels).where(and(eq(channels.id, id), eq(channels.tenantId, tenantId)));
