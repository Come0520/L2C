  ...
  while (true) {
    try {
      return await db.transaction(async (tx) => {
        ...
        const updatedCommissions = await tx
...
export async function getSettlements(params: GetSettlementsParams) {
  ...
  if (channelId) {
    ...
    whereClause = and(
      whereClause,
      eq(channelSettlements.channelId, channelId)
...
export async function getSettlementById(id: string) {
  ...
  const [settlement, commissions] = await Promise.all([
    db.query.channelSettlements.findFirst({
      where: and(eq(channelSettlements.id, id), eq(channelSettlements.tenantId, tenantId)),
      with: {
        channel: true,
...
export async function markSettlementPaid(id: string, paymentBillId?: string) {
  ...
  return await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(
        and(eq(channelCommissions.settlementId, id), eq(channelCommissions.tenantId, tenantId))
      );
