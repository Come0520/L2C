...
export function ChannelAnalytics() {
  ...
  // Avg ROI
  const avgRoi =
    totalCommission > 0 ? ((totalRevenue - totalCommission) / totalCommission) * 100 : 0;

  return (
    <div className="space-y-6">
      {/* Top Level Stats */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="总线索数" value={totalLeads} icon={Users} description="来自所有渠道" />
        <StatCard
          title="总转化订单"
          value={totalOrders}
          icon={ShoppingCart}
        ...
        <StatCard
          ...
          icon={ArrowUpRight}
          className={cn(
            avgRoi > 0 ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-red-500'
          )}
        />
      </div>
            ...
            <TableBody>
              {data.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell className="text-right">
                    ¥{row.avgTransactionValue.toLocaleString()}
                  </TableCell>
