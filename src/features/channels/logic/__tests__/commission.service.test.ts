...
import {
  ...
  checkAndGenerateCommission,
  handleCommissionClawback,
...
const mockTx = {
  ...
  insert: vi.fn(() => ({
    values: vi.fn(() => ({ returning: vi.fn(() => [{ id: 'new-comm' }]) })),
  })),
...
describe('佣金计算逻辑', () => {
  ...
  describe('底价模式 (BASE_PRICE)', () => {
    ...
    it('S级折扣应产生更高利润', () => {
      ...
      const profitC = retailPrice - basePrice * 1.0; // C级
      const profitS = retailPrice - basePrice * 0.9; // S级
  ...
  describe('等级折扣配置', () => {
    it('默认配置应有正确的等级折扣', () => {
      const gradeDiscounts = { S: 0.9, A: 0.95, B: 1.0, C: 1.0 };

      expect(gradeDiscounts['S']).toBe(0.9);
      expect(gradeDiscounts['A']).toBe(0.95);
      expect(gradeDiscounts['B']).toBe(1.0);
      expect(gradeDiscounts['C']).toBe(1.0);
    });
    ...
    it('应能正确解析 JSON 配置', () => {
      ...
      const defaults = { S: 0.9, A: 0.95, B: 1.0, C: 1.0 };
      const merged = { ...defaults, ...parsed };

      expect(merged.S).toBe(0.85);
      expect(merged.A).toBe(0.92);
      expect(merged.B).toBe(1.0); // 保持默认
    });
  });
...
describe('calculateOrderCommission() 函数', () => {
  ...
  describe('底价模式 (BASE_PRICE)', () => {
    it('B级渠道(无额外折扣)应正确计算利润', async () => {
      const order = {
        totalAmount: '300',
        items: [{ productId: 'p1', unitPrice: 150, quantity: 2, productName: 'Product 1' }],
        tenantId: 't1',
      };
      const channel = {
    ...
    it('S级渠道(9折)应有更高利润', async () => {
      const order = {
        totalAmount: '150',
        items: [{ productId: 'p1', unitPrice: 150, quantity: 1 }],
        tenantId: 't1',
      };
      const channel = {
        cooperationMode: 'BASE_PRICE',
    ...
    it('当利润为负时不应返回佣金', async () => {
      const order = {
        totalAmount: '90',
        items: [{ productId: 'p1', unitPrice: 90, quantity: 1 }],
        tenantId: 't1',
      };
      const channel = {
        cooperationMode: 'BASE_PRICE',
    ...
    it('全局等级配置格式错误时使用默认折扣率', async () => {
      const order = {
        totalAmount: '300',
        items: [{ productId: 'p1', unitPrice: 150, quantity: 2 }],
        tenantId: 't1',
      };
      const channel = {
        cooperationMode: 'BASE_PRICE',
...
describe('checkAndGenerateCommission() 集成测试流程', () => {
  ...
  it('当佣金记录已存在时应跳过生成（幂等性）', async () => {
    ...
    mockTx.query.channelCommissions.findFirst.mockResolvedValue({
      id: 'existing-comm',
    } as unknown as ReturnType<typeof db.query.channelCommissions.findFirst>);
  ...
  it('触发事件不匹配时应跳过', async () => {
    ...
    vi.mocked(db.query.channels.findFirst).mockResolvedValue({
      id: 'ch-1',
      tenantId: 't1',
      commissionTriggerMode: 'ORDER_COMPLETED',
    } as unknown as ReturnType<typeof db.query.channels.findFirst>);
    ...
    vi.mocked(db.query.channelCommissions.findMany).mockResolvedValue([
      {
        ...
        status: 'SETTLED',
        amount: '100',
        orderAmount: '0', // orderAmount 0 -> skip ratio
      },
      {
        id: 'comm-2',
        tenantId: 't1',
        ...
        status: 'SETTLED',
        amount: '0',
        orderAmount: '1000', // zero comm -> skip clawback
      },
    ] as unknown as ReturnType<typeof db.query.channelCommissions.findMany>);
