  ...
  if (mode === 'COMMISSION') {
    ...
    if (channel.commissionType === 'TIERED' && channel.tieredRates) {
      ...
      if (typeof channel.tieredRates === 'string') {
        try {
          rates = JSON.parse(channel.tieredRates);
        } catch (e) {
          logger.error('Failed to parse tieredRates', { error: e });
        }
      } else {
      ...
      if (Array.isArray(rates)) {
        ...
        const matched = rates.find((r) => {
          ...
          const max =
            r.maxAmount !== undefined && r.maxAmount !== null
              ? new Decimal(r.maxAmount)
    ...
    let gradeDiscounts: Record<string, number> = { S: 0.9, A: 0.95, B: 1.0, C: 1.0 };
    if (gradeConfig) {
      try {
    ...
    const discountRateVal = gradeDiscounts[channelLevel] ?? 1.0;
...
export async function checkAndGenerateCommission(
  ...
  await db.transaction(async (tx) => {
    ...
    if (existingInTx) {
      logger.warn(
        `[Commission] Commission already exists for order ${orderId} (Race condition intercepted)`
      );
      return;
    }
    ...
    if (comm.status === 'PENDING') {
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.insert(commissionAdjustments).values({
          ...
          adjustmentType: refundAmountDecimal.greaterThanOrEqualTo(originalOrderAmount)
            ? 'FULL_REFUND'
            : 'PARTIAL_REFUND',
