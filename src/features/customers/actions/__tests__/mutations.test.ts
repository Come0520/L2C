...
const {
    MOCK_TENANT_ID, MOCK_USER_ID, MOCK_CUSTOMER_ID, MOCK_ADDR_ID,
    mockDbInsert, mockDbUpdate, mockDbDelete, mockDbQuery, mockTx
} = vi.hoisted(() => {
    ...
    const tx = {
        insert: vi.fn(() => ({
            values: vi.fn(() => ({
                returning: vi.fn().mockResolvedValue([{ id: ADDR_ID, customerId: CUS_ID, address: 'Test', isDefault: true }]),
            })),
        })),
    ...
    return {
        MOCK_TENANT_ID: TNT_ID, MOCK_USER_ID: USR_ID, MOCK_CUSTOMER_ID: CUS_ID, MOCK_ADDR_ID: ADDR_ID,
        mockDbInsert: insertFn, mockDbUpdate: updateFn, mockDbDelete: deleteFn, mockDbQuery: queryObj, mockTx: tx
    };
});
...
vi.mock('@/shared/api/schema', () => ({
    customers: { id: 'customers.id', tenantId: 'customers.tenantId', deletedAt: 'customers.deletedAt' },
    customerAddresses: { id: 'ca.id', customerId: 'ca.customerId', tenantId: 'ca.tenantId', isDefault: 'ca.isDefault' },
...
vi.mock('@/shared/config/permissions', () => ({
    PERMISSIONS: {
        CUSTOMER: { CREATE: 'CUSTOMER.CREATE', EDIT: 'CUSTOMER.EDIT', DELETE: 'CUSTOMER.DELETE', VIEW: 'CUSTOMER.VIEW', MANAGE: 'CUSTOMER.MANAGE' },
    },
}));
