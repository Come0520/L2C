...
export async function logPhoneView(input: ViewPhoneLogInput) {
  try {
    ...
    logger.info('[customers] 记录手机号查看日志:', {
      ...input,
...
export async function getPhoneViewLogs(customerId: string): Promise<PhoneViewLog[]> {
  ...
  try {
    ...
    logger.info('[customers] 获取手机号查看日志:', {
      customerId,
      userId: session.user?.id,
      tenantId: session.user.tenantId,
    });
    ...
    const logs = await db.query.phoneViewLogs.findMany({
      where: (t, { eq, and }) => and(eq(t.customerId, customerId), eq(t.tenantId, tenantId)),
    ...
    logger.info('[customers] 获取号码查看日志完成:', {
      customerId,
