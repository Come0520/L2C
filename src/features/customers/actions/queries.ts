...
import {
  ...
  customerPipelineStatuses,
  getReferralChainSchema,
} from '../schemas';
  ...
  return unstable_cache(
    async () => {
      ...
      try {
        ...
        if (lifecycleStage) {
          whereConditions.push(
            eq(customers.lifecycleStage, lifecycleStage as (typeof customerLifecycleStages)[number])
          );
        }
        if (pipelineStatus) {
          whereConditions.push(
            eq(
              customers.pipelineStatus,
              pipelineStatus as (typeof customerPipelineStatuses)[number]
            )
          );
        }
        ...
        const dataPromise = db.query.customers
          .findMany({
            where: whereClause,
        ...
        logger.info('[customers] 获取客户列表 (Cached) 完成:', {
          count: data.length,
          total,
          duration,
        });
...
const getCustomerProfileActionInternal = createSafeAction(
  getCustomerProfileSchema,
  async ({ customerId }: { customerId: string }, { session }) => {
    if (!session.user.tenantId)
      throw new AppError('Unauthorized', ERROR_CODES.PERMISSION_DENIED, 401);
    const tenantId = session.user.tenantId;
    ...
    const frequencyScore =
      orderCount >= 10 ? 5 : orderCount >= 5 ? 4 : orderCount >= 3 ? 3 : orderCount >= 1 ? 2 : 1;
    const monetaryScore =
      totalAmount >= 100000
        ? 5
        : totalAmount >= 50000
    ...
    return {
      customer: {
        ...
        assignedSalesName: (
          customer as typeof customer & { assignedSales?: { name: string | null } | null }
        ).assignedSales?.name,
        referrerName: (customer as typeof customer & { referrer?: { name: string | null } | null })
          .referrer?.name,
...
const getReferralChainActionInternal = createSafeAction(
  getReferralChainSchema,
  async ({ customerId }: { customerId: string }, { session }) => {
    const startTime = Date.now();
    if (!session.user.tenantId)
