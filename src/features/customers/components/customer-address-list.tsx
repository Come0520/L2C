...
import {
  ...
  setDefaultAddress,
} from '@/features/customers/actions/mutations';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {addresses.length === 0 ? (
          <div className="rounded-lg border border-dashed py-4 text-center text-gray-500">
            暂无地址
          </div>
        ) : (
          addresses.map((addr) => (
            <Card
              key={addr.id}
              className={cn('group relative', addr.isDefault && 'border-blue-200 bg-blue-50')}
            >
