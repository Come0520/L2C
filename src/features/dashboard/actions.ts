    ...
    if (currentRole === 'ADMIN' || currentRole === 'MANAGER') {
      const [leadRes, orderRes, measureRes, arRes, installRes] = await Promise.all([
        ...
        db
          .select({ value: count() })
          .from(orders)
          .where(and(eq(orders.tenantId, tenantId), inArray(orders.status, ['SIGNED', 'PAID']))),
        db
          .select({ value: count() })
          .from(measureTasks)
          .where(and(eq(measureTasks.tenantId, tenantId), eq(measureTasks.status, 'PENDING'))),
        db
          .select({ sum: sql<number>`COALESCE(SUM(${arStatements.totalAmount}), 0)` })
          .from(arStatements)
          .where(and(eq(arStatements.tenantId, tenantId), eq(arStatements.status, 'PARTIAL'))),
        db
          .select({ value: count() })
          .from(installTasks)
          .where(
            and(eq(installTasks.tenantId, tenantId), eq(installTasks.status, 'PENDING_DISPATCH'))
          ),
      ]);
      ...
      stats.cards = [
        ...
        {
          title: '待收款金额',
          value: `¥${Number(arRes[0]?.sum || 0).toLocaleString()}`,
          subValue: '未结算',
          icon: 'credit-card',
          color: 'rose',
        },
      ...
      const [myLeadRes, myOrderRes] = await Promise.all([
        db
          .select({ value: count() })
          .from(leads)
          .where(and(eq(leads.tenantId, tenantId), eq(leads.assignedSalesId, userId))),
        db
          .select({ value: count() })
          .from(orders)
          .where(and(eq(orders.tenantId, tenantId), eq(orders.salesId, userId))),
      ]);

      stats.cards = [
        {
          title: '我的线索',
          value: myLeadRes[0]?.value || 0,
          icon: 'users',
          color: 'blue',
          link: '/leads',
        },
