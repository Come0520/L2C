    ...
    <div className="space-y-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="max-h-80 overflow-y-auto">
                  {hiddenWidgets.map((widget) => (
                    ...
                    >
                      <widget.icon className={cn('mr-2 h-4 w-4', widget.iconColor)} />
                      {widget.title}
                    </DropdownMenuItem>
                  ))}
                  {hiddenWidgets.length === 0 && (
                    <DropdownMenuItem disabled>已添加所有可用组件</DropdownMenuItem>
                  )}
                </DropdownMenuContent>
              </DropdownMenu>
      ...
      <div
        className={cn(
          ...
          isEditing && 'bg-muted/20 border-muted-foreground/20 border-2 border-dashed p-2'
        )}
      >
              ...
              <div key={widget.id} className="group relative">
                ...
                {isEditing && (
                  <div className="absolute top-2 right-2 z-20 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                    <div className="bg-background/90 flex items-center gap-1 rounded-md border p-1 shadow-md backdrop-blur-sm">
