...
import {
  createPaymentBill,
  verifyPaymentBill,
  generateLaborSettlement,
  createSupplierRefundStatement,
  getAPSupplierStatements,
  getAPSupplierStatement,
  getAPLaborStatements,
  getAPLaborStatement,
  getApStatementById,
  createSupplierLiabilityStatement,
} from '../actions/ap';
...
const createChainedMock = (returnValue: unknown = []) => {
  ...
  mock.then = (onFulfilled: (val: unknown) => unknown) =>
...
describe('AP Actions', () => {
  ...
  describe('Queries (getAPSupplierStatements, getApStatementById, detail queries)', () => {
    ...
    it('should get single supplier statement detail', async () => {
      ...
      expect(db.query.apSupplierStatements.findFirst).toHaveBeenCalledWith(
    ...
    it('should get labor statements list', async () => {
      (db.query.apLaborStatements.findMany as any).mockResolvedValue([
        { id: 'l-1', worker: { name: 'W1' } },
      ]);
  ...
  describe('verifyPaymentBill', () => {
      ...
      const mockTx = {
        query: {
          paymentBills: {
            findFirst: vi
              .fn()
              .mockResolvedValue({ id: mockBillId, status: 'PENDING', items: [], version: 1 }),
          },
        },
    ...
    it('should fail if payment bill status is not auditable', async () => {
      const mockTx = {
        query: {
          paymentBills: {
            findFirst: vi.fn().mockResolvedValue({ id: mockBillId, status: 'PAID' }),
          },
        },
      };
      (db.transaction as any).mockImplementation(async (cb: any) => cb(mockTx));
  ...
  describe('createSupplierLiabilityStatement', () => {
    ...
    it('should reject if not FACTORY', async () => {
      ...
      await expect(createSupplierLiabilityStatement(mockNoticeId)).rejects.toThrow(
        '此定责单非供应商(工厂)责任'
      );
    });
  });
