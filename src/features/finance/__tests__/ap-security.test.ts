...
describe('[FN-20] verifyPaymentBill - 余额核销安全测试', () => {
  ...
  it('账户余额不足时应抛出错误，不允许负余额', async () => {
    ...
    vi.mocked(db.transaction).mockImplementation((fn: (...args: unknown[]) => unknown) =>
      (fn as (...args: unknown[]) => unknown)(mockTx)
    );
...
describe('[FN-21] generateLaborSettlement - 劳务结算聚合逻辑测试', () => {
  it('多个任务费用应精确聚合（Decimal.js）', () => {
    // 模拟劳务任务费用聚合
    const tasks = [{ laborFee: '100.00' }, { laborFee: '200.50' }, { laborFee: '50.25' }];
  ...
  it('已有对账单时不应重复创建（幂等性防护）', () => {
    ...
    const existingStatements = [{ workerId: 'worker-001', period: '2024-01' }];
