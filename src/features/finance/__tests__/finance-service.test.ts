...
describe('FinanceService', () => {
  ...
  describe('generateReceivables', () => {
    it('should generate correct payment schedules based on ratios', async () => {
      const result = await FinanceService.generateReceivables(
        'order-1',
        '1000',
        'tenant-1',
        [0.6, 0.4]
    ...
    it('should handle rounding for last installment', async () => {
      ...
      await FinanceService.generateReceivables(
        'order-1',
        '100',
        'tenant-1',
  ...
  describe('createPaymentOrder', () => {
    it('should create payment order within a transaction', async () => {
      ...
      vi.mocked(db.transaction).mockImplementation(async (cb: (tx: never) => unknown) =>
        cb(mockTx as never)
      );
  ...
  describe('verifyPaymentOrder (含佣金计算)', () => {
    ...
    it('PARTIAL 状态：部分收款应更新 receivedAmount 并保持 PARTIAL 状态', async () => {
      ...
      await FinanceService.verifyPaymentOrder(
        'order-pay-partial',
        'VERIFIED',
        'tenant-1',
        'user-1'
    ...
    it('BASE_PRICE 模式：应按 (成交价 - 结算底价) * 费率计算佣金', async () => {
      ...
      vi.mocked(mockTx.query.orderItems.findMany).mockResolvedValue([
        { productId: 'p1', quantity: 1 },
      ] as never);
      vi.mocked(mockTx.query.products.findMany).mockResolvedValue([
        { id: 'p1', floorPrice: '6000' },
      ] as never);
