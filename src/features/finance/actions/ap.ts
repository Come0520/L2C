  ...
  return unstable_cache(
    async () => {
      ...
      return await db.query.apSupplierStatements.findFirst({
        where: and(eq(apSupplierStatements.id, id), eq(apSupplierStatements.tenantId, tenantId)),
        with: {
          purchaseOrder: true,
          supplier: true,
        },
...
export async function getAPLaborStatements(params?: { limit?: number; offset?: number }) {
  ...
  // 权限检查：查看人工费用
  if (!(await checkPermission(session, PERMISSIONS.FINANCE.LABOR_VIEW)))
    throw new Error('权限不足：需要人工费查看权限');
  ...
  return unstable_cache(
    async () => {
      ...
      return await db.query.apLaborStatements.findFirst({
        where: and(eq(apLaborStatements.id, id), eq(apLaborStatements.tenantId, tenantId)),
        with: {
          worker: true,
          feeDetails: true,
        },
...
export async function getApStatementById(filters: { id: string }) {
  ...
  const laborStatement = await db.query.apLaborStatements.findFirst({
    where: and(eq(apLaborStatements.id, id), eq(apLaborStatements.tenantId, session.user.tenantId)),
    with: {
      worker: true,
 ...
 */
export const createPaymentBill = createSafeAction(
  ...
  async (data, { session }) => {
    ...
    const paymentBill = await db.transaction(async (tx) => {
      ...
      const [paymentBillResult] = await tx
        .insert(paymentBills)
        .values({
          ...billData,
    ...
    if (billData.amount > 0) {
      ...
      try {
        const isApprovalActive = await FinanceApprovalLogic.isFlowActive(
 ...
 * 审核付款单
 */
export const verifyPaymentBill = createSafeAction(
  verifyPaymentBillSchema,
  async (data, { session }) => {
    // 权限检查：审批财务
    if (!(await checkPermission(session, PERMISSIONS.FINANCE.APPROVE)))
      throw new Error('权限不足：需要财务审批权限');
    ...
    return await db.transaction(async (tx) => {
      ...
      const bill = await tx.query.paymentBills.findFirst({
        where: and(eq(paymentBills.id, id), eq(paymentBills.tenantId, session.user.tenantId)),
        with: {
          items: true,
        },
      });
      ...
      if (currentBalance.lt(amount)) {
        throw new Error(
          `账户余额不足。当前余额: ¥${currentBalance.toFixed(2, Decimal.ROUND_HALF_UP)}, 需付: ¥${amount.toFixed(2, Decimal.ROUND_HALF_UP)}`
        );
      }
...
export async function generateLaborSettlement() {
  ...
  return await db.transaction(async (tx) => {
    const [settledTaskIds, pendingLiabilities] = await Promise.all([
      tx
        .select({ id: apLaborFeeDetails.installTaskId })
    ...
    for (const workerId of allWorkerIds) {
      ...
      for (const ln of liabilities) {
        const deductionAmt = new Decimal(ln.amount || '0')
          .negated()
      ...
      logger.info('[finance] 劳务结算单项处理完成', {
        statementId: statement.id,
 ...
 */
export async function createSupplierRefundStatement(
  input: z.infer<typeof createSupplierRefundSchema>
) {
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (refundAmount.gt(paidAmount))
        return {
          success: false,
          error: `退款金额不能超过已付金额 ¥${paidAmount.toFixed(2, Decimal.ROUND_HALF_UP)}`,
        };
      ...
      // 3. 创建红字 AP 对账单（负数金额）
      const [refundStatement] = await tx
        .insert(apSupplierStatements)
        .values({
          tenantId,
...
export async function createApFromPoInternal(poId: string, tenantId: string) {
  return await db.transaction(async (tx) => {
    ...
    const totalCost = (po.items || [])
      .reduce((sum, item) => {
        const itemAmount = new Decimal((item as { amount?: string }).amount || '0').toDecimalPlaces(
          2,
...
export async function updatePaymentBill(
  data: z.infer<typeof createPaymentBillSchema> & { id: string }
) {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('未授权');
  ...
  return await db.transaction(async (tx) => {
    ...
    if (isApprovalActive) {
      ...
      if (approvalRes.success) {
        ...
        // 审批提交失败，抛出错误
        const errorMessage =
          'error' in approvalRes && typeof approvalRes.error === 'string'
            ? approvalRes.error
            : '审批提交失败';
