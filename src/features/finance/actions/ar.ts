  ...
  return unstable_cache(
    async () => {
      ...
      return await db.query.arStatements.findFirst({
        where: and(eq(arStatements.id, id), eq(arStatements.tenantId, tenantId)),
        with: {
          order: true,
          customer: true,
          channel: true,
...
export async function createPaymentOrder(data: z.infer<typeof createPaymentOrderSchema>) {
  ...
  const result = await FinanceService.createPaymentOrder(
    serviceData,
    session.user.tenantId,
    session.user.id!
  );
  updateTag(`finance-ar-${session.user.tenantId}`);
...
export async function verifyPaymentOrder(data: z.infer<typeof verifyPaymentOrderSchema>) {
  ...
  updateTag(`finance-ar-${session.user.tenantId}`);
  updateTag(`finance-ar-detail-${id}`);
...
export async function createRefundStatement(input: z.infer<typeof createRefundSchema>) {
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      const totalAmount = new Decimal(originalStatement.totalAmount || '0').toDecimalPlaces(
        2,
        Decimal.ROUND_HALF_UP
      );
      const receivedAmount = new Decimal(originalStatement.receivedAmount || '0').toDecimalPlaces(
        2,
        Decimal.ROUND_HALF_UP
      );
      ...
      // 4. 更新原对账单已收金额
      const newReceivedAmount = receivedAmount
        .minus(refundAmount)
        .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
      const newPendingAmount = totalAmount
        .minus(newReceivedAmount)
        .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
      const newStatus = (
        newReceivedAmount.gte(totalAmount)
          ? 'PAID'
          : newReceivedAmount.gt(0)
            ? 'PARTIAL'
            : 'INVOICED'
      ) as 'PAID' | 'PARTIAL' | 'INVOICED';
      ...
      if (originalStatement.channelId && originalStatement.orderId) {
        await handleCommissionClawback(originalStatement.orderId, data.refundAmount);
      }

      updateTag(`finance-ar-${tenantId}`);
      updateTag(`finance-ar-detail-${originalStatement.id}`);
