...
export async function createChartOfAccount(data: CreateAccountInput) {
    ...
    if (!await checkPermission(session, PERMISSIONS.FINANCE.CONFIG_MANAGE)) return { error: '权限不足：需要财务管理权限' };
    ...
    try {
        ...
        revalidateTag(`chart-of-accounts-${session.user.tenantId}`, {});
    ...
    try {
        ...
        await db.transaction(async (tx) => {
            ...
            await AuditService.log(tx, {
                ...
                oldValues: { code: existingAccount.code, name: existingAccount.name, description: existingAccount.description }
            });
        });
