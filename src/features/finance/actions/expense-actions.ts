 ...
 * 手工录入单笔费用记录
 */
const createExpenseActionInternal = createSafeAction(
  createExpenseSchema,
  async (params, { session }) => {
    ...
    const [account] = await db
      ...
      .where(and(eq(chartOfAccounts.id, accountId), eq(chartOfAccounts.tenantId, tenantId)));

    if (!account) return { error: '费用科目不存在' };
    if (account.category !== 'EXPENSE')
      return { error: `录入的并非费用类科目 (当前类别: ${account.category})` };
...
export async function createExpenseRecord(params: import('zod').infer<typeof createExpenseSchema>) {
  ...
  // 类型安全获取返回数据
  const resultData = res?.success
    ? (res.data as { expenseId: string; needToGenerateVoucher?: boolean })
    : null;
 ...
 * 导入多条费用记录
 */
const importExpensesActionInternal = createSafeAction(
      ...
      for (const item of insertedIds) {
        ...
        if (journalRes.success) {
          ...
          await db
            .update(expenseRecords)
            .set({ journalEntryId: journalRes.entryId })
            .where(eq(expenseRecords.id, item.id));
        } else {
...
export async function importExpenseRecords(
  params: import('zod').infer<typeof importExpensesSchema>
) {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('未授权');
