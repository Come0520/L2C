...
export async function getJournalEntries(params?: {
  ...
  if (
    !canViewReports(roles, isSimpleMode) &&
    !(await checkPermission(session, PERMISSIONS.FINANCE.JOURNAL_VIEW))
  ) {
  ...
  if (params?.status) {
    conditions.push(
      eq(journalEntries.status, params.status as 'DRAFT' | 'PENDING_REVIEW' | 'POSTED')
    );
  }
  if (params?.periodId) {
 ...
 * 创建草稿凭证
 */
export const createJournalEntry = createSafeAction(
  ...
  async (data, { session }) => {
    ...
    if (
      !canCreateJournal(roles, isSimpleMode) &&
      !(await checkPermission(session, PERMISSIONS.FINANCE.JOURNAL_CREATE))
    ) {
    ...
    if (!balanceCheck.isValid) {
      throw new Error(
        `借贷不平衡：借方合计 ${balanceCheck.totalDebit}, 贷方合计 ${balanceCheck.totalCredit}`
      );
    }
    ...
    return await db.transaction(async (tx) => {
      ...
      updateTag(`finance-journal-${session.user.tenantId}`);
      return newEntry;
    });
  }
);
 ...
 * 更新凭证状态（提交复核、审核通过、驳回）
 */
export const updateJournalEntryStatus = createSafeAction(
  updateJournalEntryStatusSchema,
  async (data, { session }) => {
    ...
    if (status === 'POSTED') {
      if (
        !canReviewJournal(roles, isSimpleMode) &&
...
export async function reverseJournal(id: string, description: string) {
  ...
  if (
    !canReverseJournal(roles, isSimpleMode) &&
    !(await checkPermission(session, PERMISSIONS.FINANCE.APPROVE))
  ) {
...
export async function getAccountOptions() {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('未授权');
  if (!(await checkPermission(session, PERMISSIONS.FINANCE.JOURNAL_VIEW)))
