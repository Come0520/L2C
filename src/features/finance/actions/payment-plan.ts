...
const createPaymentPlanActionInternal = createSafeAction(
  ...
  async (params, { session }) => {
    ...
    const statement = await db.query.arStatements.findFirst({
      where: and(eq(arStatements.id, arStatementId), eq(arStatements.tenantId, tenantId)),
    });
    ...
    const planNodes = await db.transaction(async (tx) => {
      // 1. 删除旧的计划节点
      await tx.delete(paymentPlanNodes).where(eq(paymentPlanNodes.arStatementId, arStatementId));
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (isLast) {
          // 最后一期：总额 - 已分配总额，确保无尾差
          nodeAmount = totalAmountDecimal
            .minus(accumulatedAmount)
            .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
            .toFixed(2);
        } else {
          const calculated = totalAmountDecimal
            .times(node.percentage)
            .div(100)
            .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
...
const getPaymentDueRemindersActionInternal = createSafeAction(
  ...
  async (params, { session }) => {
    ...
    const pendingStatements = await db.query.arStatements.findMany({
      where: and(eq(arStatements.tenantId, tenantId), lte(arStatements.createdAt, futureDate)),
    ...
    return {
      ...
      totalPendingAmount: dueItems
        .reduce((sum, i) => sum.plus(new Decimal(i.pendingAmount)), new Decimal(0))
        .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
        .toNumber(),
...
const submitBadDebtWriteOffActionInternal = createSafeAction(
  submitBadDebtWriteOffSchema,
  async (params, { session }) => {
    await checkPermission(session, PERMISSIONS.FINANCE.AP_CREATE);
...
const processBadDebtApprovalActionInternal = createSafeAction(
  processBadDebtApprovalSchema,
  async (params, { session }) => {
    await checkPermission(session, PERMISSIONS.FINANCE.AP_CREATE);
    ...
    return await db.transaction(async (tx) => {
      ...
      const currentPending = new Decimal(statement.pendingAmount || '0').toDecimalPlaces(
        2,
