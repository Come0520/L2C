...
export async function getReconciliations() {
  ...
  // 权限检查：对账
  if (!(await checkPermission(session, PERMISSIONS.FINANCE.AR_RECONCILE)))
    throw new Error('权限不足：需要对账权限');
    ...
    async () => {
      ...
      try {
        return await db.query.reconciliations.findFirst({
          where: and(eq(reconciliations.id, id), eq(reconciliations.tenantId, tenantId)),
          with: {
            details: true,
          },
        });
...
const generateAggregatedStatementActionInternal = createSafeAction(
  aggregateStatementsSchema,
  async (params, { session }) => {
    const { customerIds, startDate, endDate, title } = params;
    const tenantId = session.user.tenantId;

    logger.info('[finance] 开始生成汇总对账单', {
    ...
    for (const stmt of statements) {
      ...
      customerSummary[cid].totalAmount = customerSummary[cid].totalAmount.plus(
        stmt.totalAmount || '0'
      );
      customerSummary[cid].receivedAmount = customerSummary[cid].receivedAmount.plus(
        stmt.receivedAmount || '0'
      );
      customerSummary[cid].pendingAmount = customerSummary[cid].pendingAmount.plus(
        stmt.pendingAmount || '0'
      );
    ...
    logger.info('[finance] 汇总对账单生成成功', {
      customerCount: Object.keys(customerSummary).length,
    });

    updateTag(`finance-reconciliation-${session.user.tenantId}`);
    ...
    return {
      ...
      title: periodTitle,
      summary: {
        totalAmount: Object.values(customerSummary)
          .reduce((a, b) => a.plus(b.totalAmount), new Decimal(0))
          .toFixed(2, Decimal.ROUND_HALF_UP),
        receivedAmount: Object.values(customerSummary)
          .reduce((a, b) => a.plus(b.receivedAmount), new Decimal(0))
          .toFixed(2, Decimal.ROUND_HALF_UP),
        pendingAmount: Object.values(customerSummary)
          .reduce((a, b) => a.plus(b.pendingAmount), new Decimal(0))
          .toFixed(2, Decimal.ROUND_HALF_UP),
        customerCount: Object.keys(customerSummary).length,
 ...
 */
export async function generateAggregatedStatement(
  params: z.infer<typeof aggregateStatementsSchema>
) {
  const session = await auth();
...
const generatePeriodStatementsActionInternal = createSafeAction(
  generatePeriodStatementsSchema,
  async (params, { session }) => {
    ...
    const pendingStatements = await db.query.arStatements.findMany({
      where: and(
        ...
        or(eq(arStatements.status, 'PENDING_RECON'), isNull(arStatements.status))
      ),
      with: { customer: true },
    });
    ...
    return {
      ...
      totalPendingAmount: pendingStatements
        .reduce((sum, s) => sum.plus(new Decimal(s.pendingAmount || '0')), new Decimal(0))
      ...
      statements: pendingStatements.map((s) => ({
        ...
        totalAmount: new Decimal(s.totalAmount || '0')
          .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
          .toFixed(2),
        pendingAmount: new Decimal(s.pendingAmount || '0')
          .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
          .toFixed(2),
      })),
    };
 ...
 * @throws 并发或校验异常时终止抛错
 */
export async function generatePeriodStatements(
  params: z.infer<typeof generatePeriodStatementsSchema>
) {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('未授权');
  if (!(await checkPermission(session, PERMISSIONS.FINANCE.AR_RECONCILE)))
...
const batchWriteOffActionInternal = createSafeAction(
  batchWriteOffSchema,
  async (params, { session }) => {
    ...
    return await db.transaction(async (tx) => {
      ...
      const receipt = await tx.query.receiptBills.findFirst({
        where: and(eq(receiptBills.id, receiptId), eq(receiptBills.tenantId, tenantId)),
      });
      ...
      if (receipt.status !== 'VERIFIED' && receipt.status !== 'PARTIAL_USED') {
        logger.info('[finance] 批量核销失败：收款单状态不可用', {
      ...
      const statements = await tx.query.arStatements.findMany({
        where: and(eq(arStatements.tenantId, tenantId), inArray(arStatements.id, statementIds)),
      });
      ...
      const invalidStatements = statements.filter(
        (s) => s.status === 'PAID' || s.status === 'COMPLETED' || s.status === 'BAD_DEBT'
      );
      if (invalidStatements.length > 0) {
        logger.info('[finance] 批量核销失败：部分账单不可核销', {
          invalidStatements: invalidStatements.map((s) => s.statementNo),
        });
        return {
          error: `部分账单已结清或不可核销: ${invalidStatements.map((s) => s.statementNo).join(', ')}`,
        };
      }
      ...
      if (availableAmount.lte(0)) {
        logger.info('[finance] 批量核销失败：收款单可用金额不足', {
          receiptNo: receipt.receiptNo,
      ...
      if (allocations && allocations.length > 0) {
        const totalAllocated = allocations
          .reduce((sum, a) => sum.plus(new Decimal(a.amount)), new Decimal(0))
          .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
        if (totalAllocated.gt(availableAmount)) {
          return {
            error: `分配金额 (${totalAllocated.toFixed(2, Decimal.ROUND_HALF_UP)}) 超出可用金额 (${availableAmount.toFixed(2, Decimal.ROUND_HALF_UP)})`,
          };
        }
        ...
        logger.info('[finance] 批量核销使用指定分配', {
        ...
        for (const stmt of statements) {
          ...
          const pending = new Decimal(stmt.pendingAmount || '0').toDecimalPlaces(
            2,
          ...
          if (allocAmount.gt(0)) {
            ...
            remainingToAlloc = remainingToAlloc
              .minus(allocAmount)
              .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
          }
        }
        logger.info('[finance] 批量核销使用自动分配', {
      ...
      for (const alloc of finalAllocations) {
        ...
        const currentReceived = new Decimal(stmt.receivedAmount || '0').toDecimalPlaces(
          2,
          Decimal.ROUND_HALF_UP
        );
        const newReceived = currentReceived
          .plus(alloc.amount)
          .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
        const totalStmtAmount = new Decimal(stmt.totalAmount || '0').toDecimalPlaces(
          2,
          Decimal.ROUND_HALF_UP
        );
        const newPending = totalStmtAmount
          .minus(newReceived)
          .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
        const newStatus = newPending.lte(0) ? 'PAID' : 'PARTIAL';

        const [updatedStmt] = await tx
          .update(arStatements)
          .set({
      ...
      const usedAmountAfter = usedAmountBefore
        .plus(totalWrittenOff)
        .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
      ...
      await AuditService.log(tx, {
        ...
        details: {
          ...
          totalAmount: totalWrittenOff.toFixed(2, Decimal.ROUND_HALF_UP),
        },
      });

      logger.info('[finance] 批量核销执行完成', { totalWrittenOff: totalWrittenOff.toFixed(2) });

      updateTag(`finance-reconciliation-${tenantId}`);
...
const crossPeriodReconciliationActionInternal = createSafeAction(
  crossPeriodReconciliationSchema,
  async (params, { session }) => {
    ...
    const conditions = [
      ...
      or(eq(arStatements.status, 'PENDING_RECON'), eq(arStatements.status, 'PARTIAL')),
    ];
    ...
    const summary = {
      ...
      totalPendingAmount: pendingStatements
        .reduce((sum, s) => sum.plus(s.pendingAmount || '0'), new Decimal(0))
        .toFixed(2),
 ...
 * @throws 获取失败引发相应错异常
 */
export async function crossPeriodReconciliation(
  params: z.infer<typeof crossPeriodReconciliationSchema>
) {
  const session = await auth();
  if (!session?.user?.tenantId) throw new Error('未授权');
  if (!(await checkPermission(session, PERMISSIONS.FINANCE.AR_RECONCILE)))
