...
async function getOrCreateSimpleBaseAccounts(tenantId: string) {
    let incomeAccount = await db.query.chartOfAccounts.findFirst({
        where: and(
            ...
            eq(chartOfAccounts.code, 'SIMPLE_INCOME')
        )
    });
    ...
    let expenseAccount = await db.query.chartOfAccounts.findFirst({
        where: and(
            eq(chartOfAccounts.tenantId, tenantId),
            eq(chartOfAccounts.code, 'SIMPLE_EXPENSE')
        )
    });
...
// 提取指定月份的流水记录列表
export async function getSimpleTransactions(yearMonthQuery: string) { // 格式: "2026-02"
    const session = await auth();
    if (!session?.user?.tenantId) {
        throw new Error('未授权访问');
    }
...
export async function getSimpleSummary(yearMonthQuery: string) {
    ...
    try {
        ...
        const totalExpense = rows.filter(r => r.type === '支出').reduce((sum, r) => sum + r.amount, 0);
