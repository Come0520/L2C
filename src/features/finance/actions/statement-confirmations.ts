 ...
 * 3. 创建对账确认明细
 */
export async function generateStatementConfirmation(
  input: z.infer<typeof generateConfirmationSchema>
) {
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      // 生成周期标签
      const periodLabel =
        data.periodLabel || `${data.periodStart.slice(0, 7)} 至 ${data.periodEnd.slice(0, 7)}`;

      // 创建对账确认单
      const [confirmation] = await tx
        .insert(statementConfirmations)
        .values({
          tenantId,
      ...
      for (const stmt of statements) {
        await tx.insert(statementConfirmationDetails).values({
          ...
          documentDate:
            stmt.createdAt?.toISOString().slice(0, 10) || new Date().toISOString().slice(0, 10),
...
export async function confirmStatement(
  ...
  try {
    ...
    logger.info('[finance] 确认对账单', {
      confirmationId,
      confirmedBy,
      hasDisputes: !!(disputedItems && disputedItems.length > 0),
    });
    ...
    return await db.transaction(async (tx) => {
      ...
      if (hasDisputes) {
        for (const item of disputedItems) {
          ...
          if (detail) {
            await tx
              .update(statementConfirmationDetails)
              .set({
                status: 'DISPUTED',
