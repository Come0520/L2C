...
export async function createInternalTransfer(input: z.infer<typeof createTransferSchema>) {
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      const transferAmount = new Decimal(validatedData.amount).toDecimalPlaces(
        2,
      ...
      if (fromBalance.lt(transferAmount)) {
        return {
          success: false,
          error: `源账户余额不足，当前余额: ¥${fromBalance.toFixed(2, Decimal.ROUND_HALF_UP)}`,
        };
      }
      ...
      // 4. 扣减源账户余额
      const fromNewBalance = fromBalance
  ...
  return unstable_cache(
    async () => {
      ...
      const transfer = await db.query.internalTransfers.findFirst({
        where: and(eq(internalTransfers.id, id), eq(internalTransfers.tenantId, tenantId)),
        with: {
          fromAccount: true,
          toAccount: true,
