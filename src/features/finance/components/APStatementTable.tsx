 ...
 */
const APStatementTableRow = React.memo(function APStatementTableRow({
  item,
  type,
  onCreatePayment,
}: APStatementTableRowProps) {
    ...
    if (isSupplier) {
      ...
      return (
        <>
          ...
          <TableCell>
            <Link
              href={`/purchase-orders/${supplierItem.purchaseOrderId}`}
    ...
    <div className="space-y-4">
      ...
      {data.length === 0 ? (
        <FinanceEmptyState
          title={`暂无${title}记录`}
          description={
            type === 'SUPPLIER' ? '目前没有可结算的供应商应付对账单' : '目前没有可结算的劳务对账单'
          }
        />
      ) : (
