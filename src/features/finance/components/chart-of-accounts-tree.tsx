...
import { toggleChartOfAccountStatus, deleteChartOfAccount } from '../actions/chart-of-accounts-actions';
...
export function ChartOfAccountsTree({ initialData }: ChartOfAccountsTreeProps) {
    ...
    const groupedTreeData = useMemo(() => {
        // 1. 构建树
        const idMapping = initialData.reduce((acc, el, i) => {
            acc[el.id] = i;
            return acc;
    ...
    const handleToggleStatus = async (id: string, currentStatus: boolean, isSystemDefault: boolean) => {
        if (isSystemDefault) {
            ...
            <TableRow key={node.id}>
                <TableCell>
                    <div
                        ...
                        style={{ paddingLeft: `${levelIndex * 24}px` }}
                    >
                        {hasChildren ? (
                            <Button
                                variant="ghost"
                ...
                <TableCell>
                    {node.isSystemDefault && <Badge variant="secondary">系统内置</Badge>}
                </TableCell>
                <TableCell>
                    ...
                    <div className="flex justify-end gap-2">
                        {!node.isSystemDefault && (
                            <>
                                ...
                                >
                                    <PowerOff size={14} className={node.isActive ? "text-red-500" : "text-green-500"} />
                                </Button>
                                <Button
                                    variant="ghost"
                                    size="sm"
        ...
        return nodes.map(node => (
            <React.Fragment key={node.id}>
                ...
                {expandedNodes.has(node.id) && node.children && (
                    renderTree(node.children, levelIndex + 1)
                )}
            </React.Fragment>
        ));
    };
