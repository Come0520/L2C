...
export function ExpenseImport({ accounts, onSuccess }: ExpenseImportProps) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between p-4 border rounded-lg bg-muted/10">
                <div className="flex items-center gap-2">
                    <FileType className="w-5 h-5 text-muted-foreground" />
                    <div>
                        <h4 className="text-sm font-medium">第一步：下载导入模板</h4>
                        <p className="text-xs text-muted-foreground">请严格按照模板格式填写数据</p>
                    </div>
                </div>
            ...
            <div className="space-y-4 p-4 border rounded-lg">
                ...
                <div className="flex items-center gap-4">
                    <Input
                        ...
                        className="max-w-xs"
                    />
                    {isParsing && <span className="text-sm text-muted-foreground flex items-center"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> 解析中...</span>}
                </div>
                ...
                {previewData.length > 0 && (
                    <div className="mt-6">
                                ...
                                <tbody>
                                    {previewData.slice(0, 50).map((row, i) => (
                                        <tr key={i} className="border-t">
                                            <td className="p-2 truncate max-w-[100px]">{row.accountCode}</td>
                                            <td className="p-2 text-orange-600 font-medium">{row.amount.toFixed(2)}</td>
                                            <td className="p-2">{row.expenseDate instanceof Date ? row.expenseDate.toISOString().split('T')[0] : 'N/A'}</td>
                                            <td className="p-2 truncate max-w-[200px]">{row.description}</td>
                                        </tr>
                                    ))}
                                    {previewData.length > 50 && (
                                        <tr>
                                            <td colSpan={4} className="p-2 text-center text-muted-foreground bg-muted/20">... 只显示前 50 条预览 ...</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <div className="flex items-center justify-between mt-6 p-4 bg-muted/30 rounded-lg border">
                            <div className="flex items-center space-x-2">
                                <Switch id="import-voucher" checked={createVoucher} onCheckedChange={setCreateVoucher} />
