...
export function JournalEntryDetail({
  entry,
  onStatusChange,
  permissions = { canSubmit: false, canReview: false, canReverse: false },
  ...
  const handleUpdateStatus = (
    newStatus: 'DRAFT' | 'PENDING_REVIEW' | 'POSTED',
    rejectReason?: string
  ) => {
    startTransition(async () => {
      const result = await updateJournalEntryStatus({
        id: entry.id,
        status: newStatus,
        rejectReason,
      });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-4">
          ...
          <Badge
            variant={statusMap[entry.status]?.variant || 'default'}
        ...
        <CardContent>
          <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
            <div>
              <p className="text-muted-foreground mb-1 text-sm font-medium">记账日期</p>
              <p>{entry.entryDate ? format(new Date(entry.entryDate), 'yyyy-MM-dd') : '-'}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1 text-sm font-medium">会计账期</p>
              <p>{entry.period ? `${entry.period.year}年${entry.period.month}月` : '-'}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1 text-sm font-medium">凭证来源</p>
              <p>{entry.sourceType === 'MANUAL' ? '手工录入' : '业务生成'}</p>
            </div>
            <div>
              <p className="text-muted-foreground mb-1 text-sm font-medium">创建时间</p>
              <p>
                {entry.createdAt ? format(new Date(entry.createdAt), 'yyyy-MM-dd HH:mm:ss') : '-'}
              </p>
            </div>
            <div className="col-span-2 md:col-span-4">
              <p className="text-muted-foreground mb-1 text-sm font-medium">摘要说明</p>
              ...
              <TableBody>
                {entry.lines?.map((line: any, idx: number) => (
                  <TableRow key={line.id}>
                    ...
                    <TableCell>
                      <span className="mr-2 font-medium">{line.account?.code}</span>
                      {line.account?.name}
                    </TableCell>
