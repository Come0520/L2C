...
export function ReceiptBillDialog({
  ...
  useEffect(() => {
    if (open) {
      if (orderId) {
        ...
        form.reset({
          ...
          type: 'NORMAL',
          totalAmount: initialStatementPendingAmount
            ? parseFloat(initialStatementPendingAmount)
            : 0,
          paymentMethod: 'WECHAT',
          receivedAt: new Date(),
          proofUrl: '',
    ...
    initialStatement,
    initialStatementId,
    initialStatementPendingAmount,
    initialStatementCustomerId,
    initialStatementCustomerName,
    initialStatementCustomerPhone,
    initialStatementOrderId,
  ]);
      ...
      try {
        ...
        const actionData = {
          ...
          items: values.items?.map((item) => ({
            ...
            orderNo:
              initialStatementOrderId === item.orderId
                ? initialStatementOrderNo || 'UNKNOWN'
                : 'UNKNOWN',
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="输入姓名"
                        {...field}
                        disabled={!!orderId || !!initialStatement}
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <PhoneInput placeholder="输入手机号" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
