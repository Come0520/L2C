                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                                ...
                                <div className="flex justify-end gap-2 pt-4">
                                    <Button variant="outline" type="button" onClick={() => setIsOpen(false)}>取消</Button>
                    ...
                    {initialData.length === 0 ? (
                        ...
                        initialData.map((row) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell>
                                    {row.type === '收入' ? (
                                        <Badge variant="outline" className="border-green-500 text-green-600 bg-green-50">收入</Badge>
                                    ) : (
                                        <Badge variant="outline" className="border-red-500 text-red-600 bg-red-50">支出</Badge>
                                    )}
                                </TableCell>
                                <TableCell>{row.description}</TableCell>
                                <TableCell className={`text-right font-medium ${row.type === '收入' ? 'text-green-600' : 'text-red-500'}`}>
                                    {row.type === '收入' ? '+' : '-'}{row.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                                </TableCell>
                            </TableRow>
                        ))
                    )}
