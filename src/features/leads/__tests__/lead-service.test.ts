...
describe('LeadService', () => {
  ...
  describe('createLead', () => {
    it('should create a lead successfully with auto-distribution', async () => {
      ...
      vi.mocked(distributeToNextSales).mockResolvedValue({
        salesId: 'sales-1',
        salesName: 'Sales 1',
      } as never);
    ...
    it('should detect phone duplicate', async () => {
      ...
      const tx = {
        query: {
          leads: {
            findFirst: vi.fn().mockResolvedValue({ id: 'existing-id', tenantId: mockTenantId }),
          },
  ...
  describe('claimFromPool', () => {
    ...
    it('should throw if already assigned', async () => {
      ...
      await expect(LeadService.claimFromPool(mockLeadId, mockTenantId, mockUserId)).rejects.toThrow(
        '线索不是待分配状态或已被认领'
      );
    });
  });
  ...
  describe('convertLead', () => {
    it('should convert lead successfully', async () => {
      ...
      const customerId = await LeadService.convertLead(
        mockLeadId,
        undefined,
        mockTenantId,
        mockUserId
