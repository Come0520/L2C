...
describe('Pool Recycle Job', () => {
    ...
    vi.mocked(db.query.tenants.findMany).mockResolvedValue([
      {
        id: tenantId,
        isActive: true,
        settings: { leadSla: { autoRecycleEnabled: true, noContactDays: 3 } },
      },
    ] as any);
  ...
  it('应当回收超期未成交的线索 (Status: FOLLOWING_UP)', async () => {
    ...
    vi.mocked(db.query.tenants.findMany).mockResolvedValue([
      {
        id: tenantId,
        isActive: true,
        settings: { leadSla: { autoRecycleEnabled: true, noDealDays: 7 } },
      },
    ] as any);
    ...
    // 第二轮查询（未成交）：返回 1 条超期
    vi.mocked(db.query.leads.findMany).mockResolvedValueOnce([
      { id: 'l2', status: 'FOLLOWING_UP', assignedAt: new Date(Date.now() - 8 * 24 * 3600 * 1000) },
    ] as any);
