...
describe('Lead Queries (L5)', () => {
  ...
  describe('getLeadTimeline', () => {
    it('应当必须验证线索归属于当前租户才能返回活动记录', async () => {
      ...
      // 然后获取记录
      mockDb.query.leadActivities.findMany.mockResolvedValue([{ id: 'act-1', type: 'PHONE_CALL' }]);
    ...
    it('如果线索不属于当前租户则抛出错误', async () => {
      ...
      await expect(getLeadTimeline({ leadId: 'lead-1' })).rejects.toThrow(
  ...
  describe('getChannels', () => {
    it('应当返回渠道列表', async () => {
      mockDb.query.marketChannels.findMany.mockResolvedValue([{ id: 'ch-1', name: '线上广告' }]);
  ...
  describe('getSalesUsers', () => {
    it('应当返回同租户下的有效销售人员', async () => {
      mockDb.query.users.findMany.mockResolvedValue([{ id: 'user-1', name: '销售A' }]);
