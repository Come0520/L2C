...
export const getLeadById = cache(async ({ id }: { id: string }) => {
  ...
  const lead = await unstable_cache(
    async (lId: string, tId: string) => {
      ...
      logger.info('[leads] getLeadById 执行完成', {
...
export async function getLeadTimeline(input: z.infer<typeof getLeadTimelineLogsSchema>) {
  ...
  return unstable_cache(
    async (lId: string) => {
      ...
      logger.info('[leads] getLeadTimeline 执行完成', {
        durationMs: Date.now() - start,
        leadId: lId,
...
export async function getChannels(parentId?: string) {
  ...
  return unstable_cache(
    async (pId?: string) => {
      ...
      logger.info('[leads] getChannels 执行完成', {
        durationMs: Date.now() - start,
        count: rows.length,
        tenantId,
      });
...
export async function getSalesUsers() {
  ...
  return unstable_cache(
    async () => {
      ...
      logger.info('[leads] getSalesUsers 执行完成', {
        durationMs: Date.now() - start,
        count: result.length,
        tenantId,
      });
...
export async function getLeadFunnelStats(input?: z.infer<typeof analyticsDateRangeSchema>) {
  ...
  return unstable_cache(
    async (r) => {
      ...
      logger.info('[leads] getLeadFunnelStats 执行完成', {
        durationMs: Date.now() - start,
