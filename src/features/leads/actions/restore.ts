  ...
  try {
    ...
    if (flow) {
      ...
      try {
        ...
        if (result.success) {
          ...
          const errorMsg =
            'error' in result && typeof result.error === 'string' ? result.error : '审批提交失败';
          logger.error('[leads] 恢复作废线索审批提交失败:', {
            error: errorMsg,
            leadId: id,
            tenantId,
          });
    ...
    const txResult = await db.transaction(async (tx) => {
      ...
      const lastHistory = await tx.query.leadStatusHistory.findFirst({
        where: and(eq(leadStatusHistory.leadId, id), eq(leadStatusHistory.newStatus, 'INVALID')),
