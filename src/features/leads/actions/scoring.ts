 ...
 * 基于：意向度 + 预算 + 来源权重
 */
const calculateLeadScoreInternal = createSafeAction(
  ...
  async ({ leadId }, { session }) => {
    ...
    const intentionScore =
      INTENTION_WEIGHTS[lead.intentionLevel || 'LOW'] || DEFAULT_INTENTION_SCORE;
 ...
 * 手机号 + 地址组合去重
 */
const checkLeadDuplicateInternal = createSafeAction(
...
const batchCheckLeadDuplicatesInternal = createSafeAction(
