      ...
      try {
        ...
        formattedFunnel.forEach((item, index) => {
          if (index > 0 && formattedFunnel[index - 1].count > 0) {
            item.conversionRate = ((item.count / formattedFunnel[index - 1].count) * 100).toFixed(
              1
            );
          } else {
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="primary" size="sm" onClick={() => loadData()} disabled={isPending}>
            {isPending ? (
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {roiStats.reduce((acc, curr) => acc + curr.leadCount, 0)}
            </div>
            <p className="text-muted-foreground text-xs">所选时间范围内</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ¥{roiStats.reduce((acc, curr) => acc + curr.totalAmount, 0).toLocaleString()}
            </div>
            <p className="text-muted-foreground text-xs">订单确认金额</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(
                (roiStats.reduce((acc, curr) => acc + curr.orderCount, 0) /
                  (roiStats.reduce((acc, curr) => acc + curr.leadCount, 0) || 1)) *
                100
              ).toFixed(1)}
              %
            </div>
            <p className="text-muted-foreground text-xs">线索 {'->'} 订单</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">线索流入到 WON 耗时</p>
          </CardContent>
        </Card>
      </div>
