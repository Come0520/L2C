...
export function LeadsAdvancedFilter({ tenantId, salesList = [] }: LeadsAdvancedFilterProps) {
  ...
  const handleClear = () => {
    ...
    const params = new URLSearchParams(searchParams.toString());
    ['intentionLevel', 'channelId', 'salesId', 'tags', 'dateFrom', 'dateTo'].forEach((k) =>
      params.delete(k)
    );
    params.set('page', '1');
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      >
        <SelectTrigger className="bg-muted/20 h-9 w-[120px] border-white/10">
      ...
      <div className="min-w-[140px]">
        <MultiSelect
          ...
          placeholder="选择标签"
          className="bg-muted/20 h-9 min-h-auto border-white/10 py-0"
        />
      </div>

      {/* 日期范围 */}
      <div className="w-[240px]">
        <DatePickerWithRange date={dateRange} setDate={handleDateRangeChange} className="h-9" />
      </div>
