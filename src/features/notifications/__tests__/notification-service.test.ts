...
vi.mock('../adapters/sms-adapter', () => ({
  SmsAdapter: class {
    send = (a: any) => hoisted.mocks.smsSend(a);
  },
}));
vi.mock('../adapters/lark-adapter', () => ({
  LarkAdapter: class {
    send = (a: any) => hoisted.mocks.larkSend(a);
  },
}));
vi.mock('../adapters/wechat-adapter', () => ({
  WeChatAdapter: class {
    send = (a: any) => hoisted.mocks.wechatSend(a);
  },
}));
...
vi.mock('@/features/settings/actions/system-settings-actions', () => ({
  getSetting: hoisted.mocks.getSetting,
}));
vi.mock('@/shared/services/audit-service', () => ({
  AuditService: { log: hoisted.mocks.logAudit },
}));
vi.mock('@/shared/lib/role-permission-service', () => ({
  RolePermissionService: { hasPermission: hoisted.mocks.hasPermission },
}));
vi.mock('next/cache', () => ({
  revalidatePath: vi.fn(),
  revalidateTag: vi.fn(),
  updateTag: vi.fn(),
  unstable_cache: vi.fn((f) => f),
}));
    ...
    it('应该跳过同一天内相同 Token 的重复通知', async () => {
      ...
      vi.mocked(db.transaction).mockImplementationOnce(
        async (cb: (tx: unknown) => Promise<unknown>) => {
          const txMock: any = {
            query: {
              notificationQueue: { findFirst: vi.fn().mockResolvedValue({ id: 'existing_1' }) },
            },
            insert: vi.fn(() => hoisted.createDrizzleMock()),
            update: vi.fn(() => hoisted.createDrizzleMock()),
            select: vi.fn(() => hoisted.createDrizzleMock()),
