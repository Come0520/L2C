...
import { notificationTemplates, systemAnnouncements } from '@/shared/api/schema/notifications';
...
export const getCachedAnnouncements = unstable_cache(
  async (tenantId: string, userRole?: string) => {
    ...
    return await db.query.systemAnnouncements.findMany({
      where: and(
        or(eq(systemAnnouncements.tenantId, tenantId), isNull(systemAnnouncements.tenantId)),
        ...
        or(
          ...
          userRole
            ? sql`${systemAnnouncements.targetRoles} @> ${sql.param(JSON.stringify([userRole]))}::jsonb`
            : undefined
        )
      ),
