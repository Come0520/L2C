...
export const notificationService = {
  async send(payload: NotificationPayload & { forceChannels?: string[] }) {
      ...
      const pref = await db.query.notificationPreferences.findFirst({
        where: and(
          ...
          eq(
            ...
            type as (typeof notificationTypeEnum.enumValues)[number]
          )
        ),
      });
    ...
    if (channelSet.has('FEISHU') || channelSet.has('LARK'))
      promises.push(larkAdapter.send(payload));

    // [Notify-03] 微信服务号模板消息
    if (channelSet.has('WECHAT')) {
      promises.push(
        wechatAdapter.send({ ...payload, metadata: { ...metadata, wechatChannel: 'OFFICIAL' } })
      );
    }
    ...
    if (channelSet.has('WECHAT_MINI')) {
      promises.push(
        wechatAdapter.send({ ...payload, metadata: { ...metadata, wechatChannel: 'MINI' } })
      );
    }
