...
export const slaChecker = {
  ...
  async checkLeadFollowupSLA() {
    ...
    for (const tenant of activeTenants) {
      ...
      for (const lead of overdueLeads) {
        ...
        const isAlreadyNotified = recentWarnings.some(
          (n) =>
  ...
  async checkApprovalSLA() {
    ...
    for (const tenant of activeTenants) {
      ...
      logger.info(
        `[SLAChecker] Tenant: ${tenant.name} (${tenant.id}), approveDays: ${approveDays}, threshold: ${approveThreshold.toISOString()}`
      );
      ...
      for (const task of longPendingTasks) {
        ...
        logger.info(
          `[SLAChecker] Auto-approving task ${task.id} for tenant ${tenant.name} due to ${approveDays}-day SLA`
        );
        ...
        try {
          ...
          logger.info(
            `[SLAChecker] Task ${currentTask?.id} current status: ${currentTask?.status}`
          );

          if (!currentTask || currentTask.status !== 'PENDING') {
            logger.warn(
              `[SLAChecker] Task ${task.id} status changed, skipping auto-approval. Status: ${currentTask?.status}`
            );
            continue;
          }
