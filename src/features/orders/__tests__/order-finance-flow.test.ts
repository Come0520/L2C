...
const { mockDbQuery, mockDbInsert, mockDbUpdate, mockSession } = vi.hoisted(() => {
  ...
  const mockDbInsert = vi.fn().mockReturnValue({
    values: vi.fn().mockReturnValue({
      returning: vi
        .fn()
        .mockResolvedValue([{ id: 'new-id-uuid', orderNo: 'OD-123', statementNo: 'AR-123' }]),
    }),
  });
...
describe('Order & Finance Integration Flow', () => {
  ...
  it('Scenario 1: Cash Order - Create, Deposit, Confirm Production', async () => {
    ...
    mockDbQuery.orders.findFirst.mockResolvedValueOnce(null).mockResolvedValue(createdOrderMock);
