...
const {
  mockQueryFindFirst,
  mockConvertFromQuote,
  mockAuditRecord,
  mockCheckAndGenerateCommission,
} = vi.hoisted(() => {
  return {
...
describe('Order Creation Actions', () => {
  ...
  describe('createOrderFromQuote', () => {
    ...
    it('成功从报价单创建订单', async () => {
      ...
      expect(mockConvertFromQuote).toHaveBeenCalledWith(mockQuoteId, mockTenantId, 'u-001', {
        paymentAmount: '1000.00',
        remark: undefined,
        usedPrepayments: undefined,
      ...
      // 验证佣金异步处理被触发
      expect(mockCheckAndGenerateCommission).toHaveBeenCalledWith(mockOrderId, 'ORDER_CREATED');
    });

    it('报价单不存在时抛出异常', async () => {
      mockQueryFindFirst.mockResolvedValueOnce(null);

      await expect(createOrderFromQuote(validInput)).rejects.toThrow('Quote not found');
    });

    it('未登录时抛出异常', async () => {
      vi.mocked(auth).mockResolvedValueOnce(null);

      await expect(createOrderFromQuote(validInput)).rejects.toThrow('Unauthorized');
    });
    ...
    it('OrderService 异常时正确传播错误', async () => {
      ...
      await expect(createOrderFromQuote(validInput)).rejects.toThrow('转换失败');
    });
  });
});
