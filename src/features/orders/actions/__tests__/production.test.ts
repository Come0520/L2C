...
describe('Order Production Actions', () => {
  ...
  describe('confirmOrderProduction', () => {
    it('PAID 状态订单成功开始生产', async () => {
      ...
      // 验证自动拆单被触发
      expect(mockExecuteSplitRouting).toHaveBeenCalledWith(mockOrderId, mockTenantId, mockSession);
    });
    ...
    it('非 PAID 且非 MONTHLY 状态不允许开始生产', async () => {
      ...
      await expect(confirmOrderProduction({ orderId: mockOrderId, version: 1 })).rejects.toThrow(
        'Order must be PAID to start production'
      );
    });
  ...
  describe('splitOrder', () => {
    ...
    it('拆单引擎异常时正确传播', async () => {
      ...
      await expect(splitOrder(validInput)).rejects.toThrow('库存不足');
    });
    ...
    it('未登录时抛出异常', async () => {
      ...
      await expect(splitOrder(validInput)).rejects.toThrow('Unauthorized');
    });
  });
});
