  ...
  if (!changes || changes.length === 0) {
    return (
      <Card className={cn('glass-liquid border-white/10', className)}>
        ...
        <CardContent>
          <p className="text-muted-foreground py-4 text-center text-sm">暂无变更记录</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className={cn('glass-liquid border-white/10', className)}>
      ...
      <CardContent>
        <div className="relative pl-6">
          {/* 时间线竖线 */}
          <div className="bg-border absolute top-2 bottom-2 left-2 w-0.5" />
                  ...
                  <div className="pl-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      {change.diffAmount !== undefined && change.diffAmount !== 0 && (
                        ...
                        >
                          {change.diffAmount > 0 ? '+' : ''}¥
                          {Math.abs(change.diffAmount).toFixed(2)}
                        </span>
                      )}
                    </div>
                    <p className="text-muted-foreground mt-0.5 text-sm">{change.reason}</p>
