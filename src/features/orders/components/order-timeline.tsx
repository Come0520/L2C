        ...
        return (
          <div key={step.status} className="relative flex gap-4">
            {index < steps.length - 1 && (
              <div
                className={cn(
                  'absolute top-6 bottom-0 left-[11px] w-[2px]',
            ...
            <div className="relative z-10 mt-1">
              {isCompleted ? (
                <CheckCircle2 className="text-primary bg-card h-6 w-6 rounded-full" />
              ) : isCurrent ? (
                <Clock className="text-primary bg-card h-6 w-6 rounded-full" />
              ) : (
                <Circle className="text-muted bg-card h-6 w-6 rounded-full" />
              )}
            </div>
            <div className="pb-6">
              <div
