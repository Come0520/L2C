...
export function SplitOrderDialog({
  ...
  const updateSupplier = (itemId: string, supplierId: string) => {
    setSplitItems((prev) =>
      prev.map((item) => (item.itemId === itemId ? { ...item, supplierId } : item))
    );
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>选择需要采购的商品，指定供应商后将自动生成采购单</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {splitItems.length === 0 ? (
            <div className="text-muted-foreground py-8 text-center">所有商品已生成采购单</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
