...
const rejectReasonSchema = z
  .string()
  .trim()
  .min(1, '请填写原因（包含该项的拒绝原因或驳回原因）')
  .max(500, '理由过长 (最多500字符)');
 ...
 */
export async function getPendingTenants(options?: { search?: string }): Promise<{
  success: boolean;
  data?: PendingTenant[];
  error?: string;
}> {
...
/**
 * 为租户 BOSS 生成一次性登录链接（Magic Link）
 *
 * 使用场景：租户管理员忘记密码，平台管理员在后台为其生成一个安全的一次性登录链接。
 * 用户点击链接后将自动登录并被强制跳转到密码修改页面。
 *
 * 安全机制：
 * - 只允许平台管理员调用
 * - 旧的 MAGIC_LOGIN token 会被立即撤销（防止滥用）
 * - 链接有效期 24 小时，仅可使用一次
 * - 操作全程记录审计日志
 *
 * @param {string} tenantId - 目标租户 UUID
 * @returns {Promise<{success: boolean; magicLink?: string; error?: string;}>} 包含完整 URL 的结果
 */
export async function generateMagicLink(tenantId: string): Promise<{
  success: boolean;
  magicLink?: string;
  error?: string;
}> {
  try {
    ...
    if (!bossUser) {
      return { success: false, error: '该租户下未找到用户' };
    }

    if (!bossUser.isActive) {
      return { success: false, error: '该用户账号已被停用，无法生成登录链接' };
    }
    ...
    logger.info('[Admin:MagicLink] 一次性登录链接已生成', {
      adminId,
      tenantId,
      targetUserId: bossUser.id,
      expiresAt,
