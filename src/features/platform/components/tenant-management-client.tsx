...
import { enterTenantView } from '@/features/platform/actions/impersonate-actions';
...
export function TenantManagementClient({
  ...
  /** 进入租户视角 */
  async function handleEnterTenantView(tenantId: string) {
    try {
      const res = await enterTenantView(tenantId);
      if (res.success) {
        toast.success('切换租户视角成功');
        setTimeout(() => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">管理平台所有租户的状态、套餐和基本信息。</p>
      </div>
      ...
      {isEmpty ? (
        <div
          className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 py-16 text-gray-400"
        ...
        <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
          <table className="min-w-full divide-y divide-gray-100">
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {tenants.map((tenant) => {
                ...
                return (
                  <tr key={tenant.id} className="transition-colors hover:bg-gray-50/50">
                    <td className="px-6 py-4">
                      <p className="font-medium text-gray-900">{tenant.name}</p>
                      <p className="mt-0.5 text-xs text-gray-400">{tenant.applicantEmail}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="flex items-center justify-end gap-3 px-6 py-4 text-right">
                      {tenant.status === 'active' && (
                        <button
                          onClick={() => handleEnterTenantView(tenant.id)}
          ...
          <div className="border-t border-gray-50 bg-gray-50/50 px-6 py-3 text-sm text-gray-400">
            共 {initialTotal} 条记录
          </div>
        </div>
      )}
