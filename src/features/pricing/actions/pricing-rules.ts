 ...
 */
export const createPricingRuleAction = createSafeAction(
 ...
 */
export const updatePricingRuleAction = createSafeAction(
  ...
  async (params, { session }) => {
    ...
    try {
      ...
      if (!existingRule) {
        logger.warn('修改定价规则被拒绝：记录不存在或跨租户访问', {
          ruleId: params.id,
          tenantId: session.user.tenantId,
        });
 ...
 */
export const deletePricingRuleAction = createSafeAction(
  ...
  async ({ id }, { session }) => {
    ...
    try {
      ...
      if (!existingRule) {
        logger.warn('删除定价规则被拒绝：记录不存在或跨租户访问', {
          ruleId: id,
          tenantId: session.user.tenantId,
        });
        return { success: false, error: '未找到定价规则' };
 ...
 */
export const batchUpdatePricingRuleAction = createSafeAction(
  batchUpdatePricingRuleSchema,
  async ({ rules }, { session }) => {
    /** 批量操作也需同样界别的授权认证 */
    await checkPermission(session, 'settings:write');
    try {
