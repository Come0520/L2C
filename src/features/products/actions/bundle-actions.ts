...
export async function getBundleById(id: string) {
  try {
    ...
    const [data] = await db
      .select()
      .from(productBundles)
      .where(and(eq(productBundles.id, id), eq(productBundles.tenantId, tenantId)));
    ...
    const items = await db
      ...
      .where(and(eq(productBundleItems.bundleId, id), eq(productBundleItems.tenantId, tenantId)));
