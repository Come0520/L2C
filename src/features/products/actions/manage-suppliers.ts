 ...
 */
const getProductSuppliersActionInternal = createSafeAction(
  getProductSuppliersSchema,
 ...
 */
const addProductSupplierActionInternal = createSafeAction(
  addProductSupplierSchema,
  async (data, { session }) => {
 ...
 * @throws 当用户缺少 `PRODUCTS.MANAGE` 权限时抛出权限异常
 */
const updateProductSupplierActionInternal = createSafeAction(
 ...
 * @throws 当用户缺少 `PRODUCTS.MANAGE` 权限时抛出权限异常
 */
const removeProductSupplierActionInternal = createSafeAction(
 ...
 */
const compareSupplierPricesActionInternal = createSafeAction(
  ...
  async ({ productId }, { session }) => {
    ...
    const recommended =
      sortedByPrice.find((s) => (s.leadTimeDays || 0) <= avgLeadTime) || sortedByPrice[0];
    ...
    return {
      ...
      comparison: {
        ...
        priceDiffPercent:
          lowestPrice > 0
            ? Math.round(((highestPrice - lowestPrice) / lowestPrice) * 10000) / 100
            : 0,
 ...
 * @throws 当用户缺少 `PRODUCTS.MANAGE` 权限时抛出权限异常
 */
const autoSwitchDefaultSupplierActionInternal = createSafeAction(
  autoSwitchDefaultSupplierSchema,
  async ({ productId, strategy }, { session }) => {
    await checkPermission(session, PERMISSIONS.PRODUCTS.MANAGE);
    ...
    switch (strategy) {
      ...
      default:
        ...
        bestSupplier = supplierList.reduce((best, curr) => {
          ...
          // 归一化后加权评分（越低越好）
          const currScore =
            (maxPrice > 0 ? (currPrice / maxPrice) * 0.6 : 0) +
            (maxLeadTime > 0 ? (currLeadTime / maxLeadTime) * 0.4 : 0);
          const bestScore =
            (maxPrice > 0 ? (bestPrice / maxPrice) * 0.6 : 0) +
...
export async function autoSwitchDefaultSupplier(
  params: z.infer<typeof autoSwitchDefaultSupplierSchema>
) {
