...
export function AttributeTemplateManager() {
  const [selectedCategory, setSelectedCategory] =
    useState<TemplateFormValues['category']>('CURTAIN_FABRIC');
      ...
      try {
        ...
        if (result.error) {
          ...
          form.reset({
            ...
            templateSchema: Array.isArray(result.data.templateSchema)
              ? result.data.templateSchema
              : [],
          });
        }
      } catch (error) {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className="bg-muted/10 flex items-start gap-4 rounded-md border p-4"
                >
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} placeholder="e.g. 幅宽" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
            ...
            <Button
              ...
              onClick={() =>
                append({ key: '', label: '', type: 'STRING', required: false, showInQuote: false })
              }
            >
