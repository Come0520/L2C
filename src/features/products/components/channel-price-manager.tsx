    ...
    <div className="space-y-4">
          ...
          <TableBody>
            {loading ? (
              ...
              filteredPrices.map((price) => {
                ...
                const discount =
                  retailPrice > 0 ? ((1 - specialPrice / retailPrice) * 100).toFixed(0) : '0';

                return (
                  <TableRow key={price.id}>
                    <TableCell className="font-medium">{price.channel?.name || '-'}</TableCell>
                    ...
                    <TableCell className="text-right text-green-600">-{discount}%</TableCell>
                    <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingPrice ? '编辑渠道专属价' : '新增渠道专属价'}</DialogTitle>
            <DialogDescription>为特定渠道设置商品的专属结算价格</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
                      ...
                      {channels.length === 0 ? (
                        ...
                        channels.map((channel) => (
                          <SelectItem key={channel.id} value={channel.id}>
                            {channel.name}
                            {channel.code && (
                              <span className="text-muted-foreground ml-2">({channel.code})</span>
                            )}
                          </SelectItem>
                        ))
                      )}
                        ...
                        products.map((product) => (
                          <SelectItem key={product.id} value={product.id}>
                            ...
                            <span className="text-muted-foreground ml-2">
                              ({product.sku}){product.retailPrice && ` ¥${product.retailPrice}`}
                            </span>
                          </SelectItem>
                        ))
                      )}
            ...
            <div className="space-y-2">
              <Label>
                专属价格 <span className="text-red-500">*</span>
              </Label>
              <Input
                type="number"
                placeholder="输入专属价格"
