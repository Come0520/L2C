...
export function PackageFormDialog({
  ...
  const form = useForm<z.infer<typeof packageFormSchema>>({
    resolver: zodResolver(packageFormSchema) as unknown as Resolver<
      z.infer<typeof packageFormSchema>
    >,
    defaultValues: {
      packageNo: '',
      packageName: '',
      ...
      <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col overflow-hidden p-0">
        ...
        >
          <div className="border-b px-6">
            <TabsList className="h-12 w-full justify-start space-x-6 border-b-0 bg-transparent">
              <TabsTrigger
                value="basic"
                className="data-[state=active]:border-primary rounded-none border-b-2 border-transparent pb-3 data-[state=active]:bg-transparent"
              >
                基础信息
              </TabsTrigger>
              <TabsTrigger
                  ...
                  <TabsContent value="products" className="m-0 space-y-6">
                    <div className="space-y-4">
                      <div className="flex gap-2">
                        <div className="relative flex-1">
                          ...
                          <Input
                            ...
                            onKeyDown={(e) =>
                              e.key === 'Enter' && (e.preventDefault(), handleSearchProducts())
                            }
                          />
                        </div>
                      ...
                      {searchResults.length > 0 && (
                        <div className="bg-muted/10 max-h-48 space-y-1 overflow-auto rounded-md border p-2">
                          {searchResults.map((p) => (
                            <div
                              key={p.id}
                              className="bg-card/50 glass-row-hover flex items-center justify-between rounded border p-2 text-sm shadow-sm"
                            >
                              <span>
                                {p.name} ({p.sku})
                              </span>
                    ...
                    <div className="space-y-2">
                      <h4 className="text-sm font-medium">已包含商品</h4>
                      <Table>
                        <TableHeader>
                          <TableRow>
