...
export function ProductImageBatchUploadDialog({ onSuccess }: ProductImageBatchUploadDialogProps) {
  ...
  const parseFilename = (
    filename: string
  ): { sku: string; type: 'images' | 'materialImages' | 'sceneImages' } | null => {
    // 预期格式: [SKU]-主图-1.jpg
  ...
  const handleUpload = async () => {
    ...
    const groupedResults: Record<
      string,
      { images: string[]; materialImages: string[]; sceneImages: string[] }
    > = {};
        ...
        <div className="bg-background flex h-[80vh] flex-col overflow-hidden rounded-lg border shadow-lg">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto p-6">
            <Alert>
              ...
              <AlertDescription>
                请确保图片按照以下格式命名以供自动匹配：<b>[SKU型号]-主图-1.jpg</b>、
                <b>[SKU型号]-材质图-2.png</b>、<b>[SKU型号]-场景图-1.jpg</b>
                。选择多张图片后，系统将一次性上传并绑定到对应的商品 SKU 下。
              </AlertDescription>
            </Alert>

            <div className="flex items-center gap-4">
              <Button
                onClick={() => document.getElementById('image-batch-upload')?.click()}
              ...
              <ScrollArea className="h-[350px] rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {files.map((file, i) => (
                      <TableRow key={i}>
                        <TableCell className="max-w-[200px] truncate" title={file.file.name}>
                          {file.file.name}
                        </TableCell>
                        ...
                        <TableCell>
                          {file.status === 'pending' && (
                            <span className="text-muted-foreground text-xs">等待上传</span>
                          )}
                          ...
                          {file.status === 'error' && (
                            <span
                              className="text-destructive max-w-[100px] truncate text-xs"
                              title={file.error}
                            >
                              {file.error}
                            </span>
