...
describe('报价计算服务 (Calculation Service)', () => {
  ...
  it('能够根据分类正常派发到标品策略 (Dispatch standard strategy)', async () => {
    ...
    const result = await QuoteCalculationService.recalculateItem('item-standard', {
      width: 0,
      height: 0,
    });
  ...
  it('窗帘计算策略处理 (Dispatch curtain strategy)', async () => {
    ...
    const result = await QuoteCalculationService.recalculateItem('item-curtain', {
      width: 200,
      height: 250,
    });
  ...
  it('墙纸计算策略处理 (Dispatch wallpaper strategy)', async () => {
    ...
    const result = await QuoteCalculationService.recalculateItem('item-wallpaper', {
      width: 300,
      height: 250,
    });
