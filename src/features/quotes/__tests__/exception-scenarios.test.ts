...
describe('报价单异常场景测试', () => {
  ...
  it('操作不存在的报价单应抛出异常', async () => {
    ...
    await expect(
      QuoteLifecycleService.submit(nonExistentId, mockTenantId, mockUserId)
    ).rejects.toThrow();
  });
  ...
  it('跨租户操作应被拦截', async () => {
    ...
    await expect(QuoteLifecycleService.submit(quoteId, wrongTenantId, mockUserId)).rejects.toThrow(
      /不存在|无权/
    );
  });
