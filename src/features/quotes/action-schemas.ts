...
export const QuoteItemAttachmentSchema = z
  .object({
...
export const QuoteItemInputSchema = z
  .object({
    ...
    installPosition: z
      .enum(['CURTAIN_BOX', 'INSIDE', 'OUTSIDE', 'CUSTOM'])
      .optional()
      .describe('安装位置'),
...
export const getQuoteBundlesSchema = z
  .object({
    page: z.number().optional().describe('页码'),
    pageSize: z.number().optional().describe('每页条数'),
    status: z
      .enum(['ALL', 'DRAFT', 'ACTIVE', 'LOCKED', 'EXPIRED'])
      .optional()
      .describe('报价单状态过滤'),
...
export const activateQuoteItemVersionSchema = z
  .object({
...
export const getQuoteItemVersionsSchema = z
  .object({
    quoteId: z.string().describe('主报价单 ID'),
    roomId: z.string().optional().describe('所属空间 ID'),
  })
...
export const createQuoteBundleWithQuotesSchema = z
  .object({
    customerId: z.string().describe('客户 ID'),
    leadId: z.string().optional().describe('关联线索 ID'),
    summaryMode: z
    ...
    quotes: z
      ...
      .min(1)
      .describe('按分类构建的初始报价单数据列表'),
  })
  .describe('创建带有初始报价数据的合集参数');

export const updateQuoteBundleWithQuotesSchema = createQuoteBundleWithQuotesSchema
  .extend({
