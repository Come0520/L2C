...
describe('Quote Actions Audit Integration', () => {
  beforeEach(() => {
    ...
    vi.mocked(auth).mockResolvedValue(
      MOCK_SESSION as Parameters<typeof auth>[0] extends never
        ? typeof MOCK_SESSION
    ...
    vi.mocked(QuoteVersionService.copyQuote).mockResolvedValue({
      id: MOCK_UUID.NEW_QUOTE,
      quoteNo: 'QT-NEW',
    } as Awaited<ReturnType<typeof QuoteVersionService.copyQuote>>);
    vi.mocked(QuoteVersionService.createNextVersion).mockResolvedValue({
      id: MOCK_UUID.NEW_QUOTE,
      quoteNo: 'QT-VER',
    } as Awaited<ReturnType<typeof QuoteVersionService.createNextVersion>>);
    vi.mocked(QuoteLifecycleService.convertToOrder).mockResolvedValue({
      id: MOCK_UUID.ORDER,
    } as Awaited<ReturnType<typeof QuoteLifecycleService.convertToOrder>>);
  ...
  describe('copyQuote', () => {
    it('复制报价单应记录审计日志', async () => {
      // 模拟源报价单存在
      vi.mocked(db.query.quotes.findFirst).mockResolvedValue({ id: MOCK_UUID.QUOTE } as Awaited<
        ReturnType<typeof db.query.quotes.findFirst>
      >);
