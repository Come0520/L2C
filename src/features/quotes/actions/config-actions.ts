...
const updateGlobalQuoteConfigActionInternal = createSafeAction(
  ...
  async (data) => {
    ...
    await QuoteConfigService.updateTenantConfig(session.user.tenantId, updateData);
...
export async function getPlanSettings(plan: 'ECONOMIC' | 'COMFORT' | 'LUXURY') {
  ...
  return await QuoteConfigService.getPlanSettings(session.user.tenantId, session.user.id, plan);
}
