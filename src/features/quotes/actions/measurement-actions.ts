...
export async function getImportableMeasureTasks(quoteId: string) {
  ...
  const quote = await db.query.quotes.findFirst({
    where: (quotes, { eq, and }) =>
      and(eq(quotes.id, quoteId), eq(quotes.tenantId, session.user.tenantId)),
    with: {
      customer: true,
    },
  });
 ...
 * @returns 预览差异和操作列表
 */
const previewMeasurementImportActionInternal = createSafeAction(
  previewImportSchema,
  async (data) => {
    const session = await auth();
    if (!session?.user?.tenantId) throw new Error('Unauthorized');

    const result = await QuoteService.previewMeasurementImport(
      data.quoteId,
      data.measureTaskId,
 ...
 * @returns 执行成功状态及相关详情
 */
const executeMeasurementImportActionInternal = createSafeAction(
  executeImportSchema,
  async (data) => {
    const session = await auth();
    if (!session?.user?.tenantId) throw new Error('Unauthorized');

    const result = await QuoteService.executeMeasurementImport(
      data.quoteId,
      data.actions as ImportAction[],
      session.user.tenantId
    );
