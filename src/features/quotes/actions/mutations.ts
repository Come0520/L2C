  ...
  async (data, context) => {
    ...
    if (data.productId) {
      ...
      if (product) {
        ...
        if (specs.patternRepeat && !attributes.patternRepeat)
          attributes.patternRepeat = specs.patternRepeat;
    ...
    if (data.category === 'CURTAIN' && data.width && data.height) {
      const calcParams = {
        ...
        headerLossWrapped:
          presetLoss.curtain.headerLossWrapped ?? presetLoss.curtain.headerLoss ?? 20,
        headerLossAttached: presetLoss.curtain.headerLossAttached ?? 7,
        headerType: ((attributes.headerType as string) ||
          presetLoss.curtain.defaultHeaderType ||
          'WRAPPED') as 'WRAPPED' | 'ATTACHED',
...
export const createQuickQuote = createSafeAction(createQuickQuoteSchema, async (data, context) => {
  ...
  try {
    ...
    logger.error('[createQuickQuote] Quote insert failed:', insertError);
    throw insertError;
  }
