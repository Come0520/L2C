...
export function ProductAutocomplete({ value, onSelect, category, allowedCategories, placeholder = "选择商品...", disabled, onDoubleClick }: ProductAutocompleteProps) {
    ...
    React.useEffect(() => {
        ...
        const effectiveCategory = allowedCategories && allowedCategories.length > 0
        ...
        const fetchProducts = async () => {
            ...
            try {
                ...
                const results = await searchProducts(debouncedQuery, category, recentIds, allowedCategories);
                setData(results);
    ...
    // 检查商品是否为最近使用
    const isRecentProduct = React.useCallback((productId: string) => {
                        ...
                        <CommandGroup>
                            {data.map((product) => (
                                <CommandItem
                                    ...
                                    // 修复 cmdk shouldFilter={false} 时鼠标点击无响应问题
                                    onMouseDown={(e) => {
                                        e.preventDefault();
                                ...
                                >
                                    <div className="flex flex-col flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-muted-foreground bg-muted px-1 rounded shrink-0">{product.category}</span>
                                        </div>
