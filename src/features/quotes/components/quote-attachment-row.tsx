...
export function QuoteAttachmentRow({
   ...
   * 商品选择处理（预留接口）
   */
  const _handleProductSelect = useCallback(
  ...
  return (
    <div
      className={cn(
        'grid grid-cols-[100px_1fr_100px_70px_60px_80px_90px_70px] items-center gap-2 px-3 py-2',
        'bg-muted/20 border-t',
        isNew && 'border-primary/30 bg-primary/5 border-dashed'
      )}
    >
      {/* 附件类型下拉 */}
      ...
      {/* 单位 */}
      <span className="text-muted-foreground text-center text-sm">{unit}</span>
      ...
      {/* 小计 */}
      <span className="text-right text-sm font-medium">¥{subtotal.toFixed(2)}</span>
...
export function AddAttachmentButton({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'text-muted-foreground hover:text-primary flex items-center gap-2 px-3 py-2 text-sm',
        'border-muted-foreground/30 hover:border-primary/50 rounded border border-dashed',
