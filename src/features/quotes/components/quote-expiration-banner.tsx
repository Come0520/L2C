...
export function QuoteExpirationBanner({
  ...
  const isExpired =
    status === 'EXPIRED' ||
    (validUntilDate && validUntilDate < now && status !== 'ACCEPTED' && status !== 'REJECTED');
  ...
  if (isExpired) {
    return (
      <Alert variant="destructive" className="mb-6 border-red-200 bg-red-50 dark:bg-red-900/20">
        ...
        <AlertTitle className="mb-1 flex items-center gap-2 font-semibold">报价已过期</AlertTitle>
        <AlertDescription className="flex flex-wrap items-center justify-between gap-4">
          ...
          {!isReadOnly && (
            <Button
              size="sm"
              variant="outline"
              className="border-red-200 bg-white text-red-700 shadow-sm transition-all hover:border-red-300 hover:bg-red-50"
  ...
  if (daysLeft <= 3 && daysLeft > 0) {
    return (
      <Alert className="mb-6 border-amber-200 bg-amber-50 text-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
