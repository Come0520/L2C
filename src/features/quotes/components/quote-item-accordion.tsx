...
export function QuoteItemAccordion({
  ...
  return (
    <div className={cn('overflow-hidden rounded-lg border', className)}>
      ...
      <div
        className={cn(
          'bg-card flex items-center gap-2 px-3 py-2 transition-colors',
      ...
      >
        ...
        {/* 尺寸 */}
        <div className="text-muted-foreground w-28 text-center text-sm">
          {item.width || '-'} × {item.height || '-'}
        </div>

        {/* 倍数 */}
        {!isAccessory && <div className="w-14 text-center text-sm">{item.foldRatio || '-'}</div>}

        {/* 数量 */}
        <div className="w-16 text-center text-sm font-medium">{item.quantity || '-'}</div>

        {/* 单价 */}
        <div className="w-20 text-right text-sm">¥{formatAmount(item.unitPrice)}</div>

        {/* 金额 */}
        <div className="w-24 text-right text-sm font-medium">¥{formatAmount(item.subtotal)}</div>
        ...
        <div className="bg-muted/30 space-y-4 border-t p-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            <AdvancedField
              ...
              readOnly={readOnly}
            />
            <AdvancedField label="材质" value={item.attributes?.material} readOnly={readOnly} />
            <AdvancedField
              label="克重"
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <AdvancedField
              ...
              readOnly={readOnly}
            />
            <AdvancedField label="褶皱倍数" value={item.foldRatio} readOnly={readOnly} />
            <AdvancedField
              label="折扣"
          ...
          {/* 备注 */}
          <div>
            <label className="text-muted-foreground mb-1 block text-xs">备注</label>
            <Input
              value={item.remark || ''}
              placeholder="输入备注"
              disabled={readOnly}
          ...
          <div className="space-y-2">
            ...
            {item.children && item.children.length > 0 ? (
              ...
              <div className="text-muted-foreground rounded border border-dashed py-2 text-center text-sm">
                暂无附件
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end border-t pt-2">
            <div className="flex items-baseline gap-2">
              <span className="text-muted-foreground text-sm">商品小计（含附件）</span>
  ...
  return (
    <div>
      ...
      {readOnly ? (
        ...
        <Input className="h-8" defaultValue={value ?? ''} placeholder={`输入${label}`} />
      )}
    </div>
  );
}
...
function AccessoryRow({
  ...
  return (
    <div className="bg-background/50 flex items-center gap-2 rounded px-2 py-1.5 text-sm">
      <CornerDownRight className="text-muted-foreground h-3 w-3 shrink-0" />
      <span className="flex-1 truncate">{item.productName}</span>
      <span className="text-muted-foreground w-16 text-center">{item.quantity || '-'}</span>
      <span className="w-16 text-right">¥{parseFloat(String(item.unitPrice || 0)).toFixed(0)}</span>
      <span className="w-16 text-right font-medium">
        ¥{parseFloat(String(item.subtotal || 0)).toFixed(0)}
      </span>
      {!readOnly && (
        <Button
          variant="ghost"
