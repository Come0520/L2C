...
export function QuoteItemDialog({
  ...
  // Extra Curtain Attributes
  const [clearance, setClearance] = useState<number>(0);
  const [openingType, setOpeningType] = useState<'SINGLE' | 'DOUBLE'>('DOUBLE');
  const [sideLoss, setSideLoss] = useState<number>(0);
  ...
  const handleSubmit = async () => {
    ...
    if (['CURTAIN_FABRIC', 'CURTAIN_SHEER'].includes(category) && !heightOverflowConfirmed) {
      ...
      // 考虑离地高度
      const effectiveHeight = height - clearance;
      const maxEffectiveHeight = fabricWidth - headerLoss - bottomLoss;

      if (effectiveHeight > maxEffectiveHeight) {
        // 生成替代方案并打开弹窗
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="grid gap-4 py-4">
                      ...
                      {products.map((product) => (
                        <CommandItem
                          ...
                          onSelect={() => {
                            ...
                            if (product.defaultFoldRatio)
                              setFoldRatio(Number(product.defaultFoldRatio));
                          }}
                        >
                          <Check
                            className={cn(
          ...
          {['CURTAIN_FABRIC', 'CURTAIN_SHEER'].includes(category) ? (
            <div className="col-span-4">
              <CurtainFabricQuoteForm
                ...
                onChange={(values, result) => {
                  // Sync back to Dialog State
                  setWidth(values.measuredWidth);
                  setHeight(values.measuredHeight);
                  setFoldRatio(values.foldRatio);
                  setSelectedHeaderType(values.headerType);
                  setSelectedBottomLoss(values.bottomLoss);
                  setClearance(values.clearance);
                  setOpeningType(values.openingType);
                  setSideLoss(values.sideLoss);
                  // 对于窗帘，使用计算出的用量
                  if (result) {
                    setQuantity(result.usage);
                  }
                }}
              />
            </div>
