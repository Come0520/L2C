  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="start" className="w-[320px] p-0">
        <DropdownMenuLabel className="bg-muted/50 flex items-center justify-between px-3 py-2">
            ...
            {sortedVersions.map((version, index) => {
              ...
              return (
                <DropdownMenuItem
                  ...
                  className={cn(
                    'group mb-1 flex cursor-pointer items-start justify-between rounded-md p-2',
                  ...
                  <div className="flex gap-2">
                    <div className="mt-0.5 flex w-4 flex-shrink-0 justify-center">
                      {isCurrent ? (
                        ...
                        <div className="bg-muted-foreground/30 group-hover:bg-primary/50 mt-1.5 h-1.5 w-1.5 rounded-full" />
                      )}
                    </div>
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className={cn('text-sm font-medium', isCurrent && 'text-primary/90')}>
                          版本 {version.version}
                        </span>
                        <Badge variant={variant} className="h-4 px-1 py-0 text-[10px] font-normal">
                          {label}
                        </Badge>
                        {isLatest && !isCurrent && (
                          <span className="rounded-sm border border-emerald-200 bg-emerald-50 px-1 text-[10px] font-medium text-emerald-600">
                            最新
                          </span>
                        )}
                        {isCurrent && (
                          <span className="text-primary border-primary/20 bg-primary/5 rounded-sm border px-1 text-[10px] font-medium">
                            当前
                          </span>
                        )}
                      </div>
        ...
        <div className="bg-muted/20 p-1.5">
          <DropdownMenuItem
            className="text-primary focus:bg-primary/10 focus:text-primary flex cursor-pointer items-center gap-2 py-2 transition-colors"
            onClick={(e) => {
              e.preventDefault();
              handleCreateVersion();
            }}
          ...
          {onCopy && (
            <DropdownMenuItem
              className="text-muted-foreground focus:bg-muted flex cursor-pointer items-center gap-2 py-2"
              onClick={() => {
                setOpen(false);
                onCopy();
              }}
