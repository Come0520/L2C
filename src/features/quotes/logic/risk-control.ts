...
export function checkDiscountRisk(
  ...
  // Defaults
  const minDiscountRate = settings.quoteConfig?.minDiscountRate ?? 0.9;
  ...
  if (originalAmount > 0) {
    ...
    if (currentDiscount < minDiscountRate) {
      result.isRisk = true;
      result.reason.push(
        `当前折扣 (${(currentDiscount * 10).toFixed(2)}折) 低于最低折扣限制 (${(minDiscountRate * 10).toFixed(2)}折)。`
      );
    }
  }
  ...
  if (finalAmount > 0) {
    ...
    if (margin < minProfitMargin) {
      result.isRisk = true;
      result.reason.push(
        `当前毛利率 (${(margin * 100).toFixed(1)}%) 低于最低毛利限制 (${(minProfitMargin * 100).toFixed(1)}%)。`
      );
    }
  }
