...
describe('Sales 模块安全测试', () => {
  ...
  describe('getAnnualTargets', () => {
    it('未登录应返回 success: false', async () => {
      mockAuth.mockResolvedValue(null as never);
      const { getAnnualTargets } = await import('../actions/annual-targets');
      const result = await getAnnualTargets(2026);
      expect(result.success).toBe(false);
      expect(result.error).toBe('未登录');
    });

    it('已登录可以获取年度目标列表', async () => {
      mockAuth.mockResolvedValue(makeSession() as never);
      const { getAnnualTargets } = await import('../actions/annual-targets');
      const result = await getAnnualTargets(2026);
  ...
  describe('getSalesTargets', () => {
    it('未登录应返回 success: false', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Unauthorized'); // note targets.ts checks session
    });
  });
