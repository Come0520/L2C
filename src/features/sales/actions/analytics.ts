      ...
      async () => {
        ...
        for (let i = 5; i >= 0; i--) {
          ...
          const completionRate =
            targetAmount > 0 ? Math.min(Math.round((achievedAmount / targetAmount) * 100), 100) : 0;
...
export async function getSalesRanking(
  ...
  try {
    ...
    // 权限检查：使用权限矩阵
    const hasViewAll = await checkPermission(session, PERMISSIONS.ANALYTICS.VIEW_ALL);
    if (!hasViewAll) {
      logger.error(
        `[sales][analytics] Permission denied for getSalesRanking: user ${session.user.id}`
      );
...
export async function getSalesTargetWarnings(): Promise<{
  ...
  try {
    ...
    if (!hasViewAll) {
      logger.error(
        `[sales][analytics] Permission denied for getSalesTargetWarnings: user ${session.user.id}`
      );
      return { success: false, error: 'Permission denied' };
    }
    ...
    logger.info(
      `[sales][analytics] getSalesTargetWarnings success, found ${atRiskWarnings.length} warnings`
    );
