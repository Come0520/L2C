 ...
 */
export async function getSalesDashboardStats(): Promise<{
  success: boolean;
  data?: DashboardStats;
  error?: string;
}> {
  try {
    ...
    const getCachedStats = unstable_cache(
      async () => {
        ...
        if (isTeamView) {
          ...
          pendingLeads = leadsStats
            .filter((s) => ['UNASSIGNED', 'PENDING_FOLLOWUP'].includes(s.status || ''))
            .reduce((acc, c) => acc + c.count, 0);
          followingLeads = leadsStats
            .filter((s) =>
              ['PENDING_QUOTE', 'QUOTE_SENT', 'PENDING_MEASUREMENT'].includes(s.status || '')
            )
            .reduce((acc, c) => acc + c.count, 0);
          wonLeads = leadsStats
            .filter((s) =>
              ['IN_PRODUCTION', 'PENDING_DELIVERY', 'PENDING_INSTALLATION', 'COMPLETED'].includes(
                s.status || ''
              )
            )
            .reduce((acc, c) => acc + c.count, 0);
          ...
          totalCash = confirmedQuotes.reduce(
          ...
          pendingLeads = myLeadsStats
            .filter((s) => ['UNASSIGNED', 'PENDING_FOLLOWUP'].includes(s.status || ''))
            .reduce((acc, c) => acc + c.count, 0);
          followingLeads = myLeadsStats
            .filter((s) =>
              ['PENDING_QUOTE', 'QUOTE_SENT', 'PENDING_MEASUREMENT'].includes(s.status || '')
            )
            .reduce((acc, c) => acc + c.count, 0);
          wonLeads = myLeadsStats
            .filter((s) =>
              ['IN_PRODUCTION', 'PENDING_DELIVERY', 'PENDING_INSTALLATION', 'COMPLETED'].includes(
                s.status || ''
              )
    ...
    logger.info(
      `[sales][dashboard] Successfully retrieved dashboard stats, target: ${data.target.amount}, user: ${user.id}`
    );
