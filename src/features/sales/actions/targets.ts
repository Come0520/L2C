...
import { auth } from '@/shared/lib/auth'; // Assuming auth is available
...
export async function getSalesTargets(year: number, month: number): Promise<{ success: boolean; data?: SalesTargetDTO[]; error?: string }> {
    try {
...
export async function updateSalesTarget(userId: string, year: number, month: number, amount: number) {
    try {
        const session = await auth();
        if (!session?.user?.tenantId) return { success: false, error: 'Unauthorized' };
...
export async function getMySalesTarget(year?: number, month?: number) {
    try {
        ...
        console.error('getMySalesTarget error:', error);
