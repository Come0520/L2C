...
vi.mock('@/shared/api/db', () => ({
  db: {
    ...
    select: vi.fn(() => ({
      from: vi.fn(() => ({ where: vi.fn().mockResolvedValue([{ count: 3 }]) })),
    })),
    insert: vi.fn(() => ({ values: vi.fn(() => ({ returning: vi.fn(() => [{}]) })) })),
    transaction: vi.fn((cb: (...args: unknown[]) => unknown) =>
      cb({
        update: vi.fn(() => ({ set: vi.fn(() => ({ where: vi.fn().mockResolvedValue([{}]) })) })),
        insert: vi.fn(() => ({ values: vi.fn(() => ({ returning: vi.fn(() => [{}]) })) })),
        query: {
...
vi.mock('@/shared/api/schema', () => ({
  installTasks: {
    id: 'install_tasks.id',
    tenantId: 'install_tasks.tenantId',
    installerId: 'install_tasks.installerId',
    $inferSelect: {},
  },
  measureTasks: {
    id: 'measure_tasks.id',
    tenantId: 'measure_tasks.tenantId',
    assignedWorkerId: 'measure_tasks.assignedWorkerId',
    $inferSelect: {},
  },
