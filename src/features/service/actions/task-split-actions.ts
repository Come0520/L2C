...
export async function suggestTaskSplit(quoteId: string): Promise<{
  ...
  try {
    ...
    const items = await db.query.quoteItems.findMany({
      where: and(eq(quoteItems.quoteId, quoteId), eq(quoteItems.tenantId, session.user.tenantId)),
      with: {
        product: true,
      },
    });
    ...
    const validItems = items.filter((item) => {
      if (item.quantity === undefined) return true;
      return (Number(item.quantity) || 0) > 0;
    });
    if (validItems.length === 0)
      return { success: false, error: '可拆分的商品数量已被耗尽或原始数量异常' };
    const categoryGroups = new Map<string, typeof items>();
    for (const item of items.filter(
      (i) => i.quantity === undefined || (Number(i.quantity) || 0) > 0
    )) {
    ...
    const skillTypesToSearch = categories.map(
      (c) => `INSTALL_${c}` as (typeof workerSkills.$inferSelect)['skillType']
    );
