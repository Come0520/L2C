...
export function TicketList({
  ...
  const handleUpdate = async () => {
    ...
    const res = await updateTicketStatus(
      selectedTicket.id,
      status as 'PENDING' | 'PROCESSING' | 'PENDING_VERIFY' | 'CLOSED',
      result
    );
    setIsUpdating(false);
    if (res.success) {
      ...
      setLocalTickets((prev) =>
        prev.map((t: TicketData) => (t.id === selectedTicket.id ? { ...t, status, result } : t))
      );
    } else {
      toast.error('更新失败');
    }
              ...
              {localTickets.map((ticket: TicketData) => (
                <TableRow key={ticket.id}>
                  ...
                  <TableCell>
                    ...
                    <div className="text-muted-foreground text-xs">{ticket.customer?.phone}</div>
                  </TableCell>
