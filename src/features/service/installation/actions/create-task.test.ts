...
describe.skip('Installation Tasks Logic', () => {
  ...
  it('should split order into install tasks correctly', async () => {
    ...
    const logicResult = result.data as {
      success: boolean;
      data?: { createdTaskIds: string[] };
      error?: string;
    };
    ...
    const wallpaperTask = tasks.find(
      (t: { category: string | null }) => t.category === 'WALLPAPER'
    );
  ...
  it('should create measure task correctly', async () => {
    ...
    const logicResult = result.data as {
      ...
      error?: string;
      message?: string;
    }; // safe action wrapper unwrapping
  ...
  it('should check in measure task successfully with GPS validation', async () => {
    ...
    const logicResult = result.data as {
      success: boolean;
      data: { gpsResult: { isWithinRange: boolean }; lateMinutes: number };
      error?: string;
      message?: string;
    };
    expect(logicResult.success).toBe(true);
  ...
  it('should reject task and increment count', async () => {
    ...
    const logicResult = result.data as {
      success: boolean;
      data: { rejectCount: number; status: string };
      error?: string;
      message?: string;
    };
    expect(logicResult.success).toBe(true);
