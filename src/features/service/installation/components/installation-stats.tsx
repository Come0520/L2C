...
import { ClipboardList, UserPlus, Truck, CheckCircle2, Clock } from 'lucide-react';
    ...
    <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
      {statItems.map((item) => (
        <div
          key={item.label}
          className={cn(
            'rounded-2xl border p-4 backdrop-blur-sm transition-all hover:scale-[1.02]',
            'glass-liquid bg-white/5',
            item.borderColor
          )}
        >
          <div className="mb-2 flex items-center justify-between">
            <div className={cn('rounded-xl p-2', item.bgColor)}>
              <item.icon className={cn('h-5 w-5', item.color)} />
            </div>
            <span className="text-2xl font-bold tracking-tight text-white">{item.value}</span>
          </div>
          <p className="text-xs font-medium tracking-wider text-zinc-500 uppercase">{item.label}</p>
        </div>
      ))}
    </div>
  );
