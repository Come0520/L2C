...
const { mockDb, mockDbTransaction, mockAuditService, mockCheckFee, mockSubmitApproval, mockAuth } =
  vi.hoisted(() => {
    const mockDbQuery = {
      customers: {
        findFirst: vi.fn(),
    ...
    return {
      ...
      mockDbTransaction: {
        // export inner mocks for assertions
        query: mockDbQuery,
        insert: mockDbInsert,
        update: mockDbUpdate,
      },
...
describe('Measurement Action: createMeasureTask', () => {
  ...
  it('should create task successfully with fee waiver (VIP)', async () => {
    ...
    const mockReturning = vi
      .fn()
      .mockResolvedValue([{ id: 'new-task-id', status: 'PENDING_APPROVAL' }]);
    mockDbTransaction.insert.mockReturnValue({
      // Use mockDbTransaction.insert
