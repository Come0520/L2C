...
export async function splitMeasureTask(input: unknown) {
  ...
  logger.info(
    `[MeasureSplit] 开始拆分测量任务: 原ID ${originalTaskId}, 拆分数量: ${splits.length}`,
    { reason }
  );
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(
          and(eq(measureTasks.id, originalTaskId), eq(measureTasks.tenantId, session.user.tenantId))
        );
...
export async function requestFeeWaiver(input: unknown) {
  ...
  await db
    ...
    .where(and(eq(measureTasks.id, taskId), eq(measureTasks.tenantId, session.user.tenantId)));
