...
export function CreateMeasureTaskDialog() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          formData.isFeeExempt && formData.type !== 'SALES_SELF'
            ? '任务已提交审批'
            : '测量任务已创建'
        );
        setOpen(false);
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, scheduledAt: new Date(e.target.value) })
                }
              />
            </div>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="isFeeExempt" className="text-right">
              免收测量费
            </Label>
            <div className="col-span-3 flex items-center space-x-2">
              ...
              />
              <span className="text-muted-foreground text-sm">免收且非自测需审批</span>
            </div>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
