    ...
    <div className="rounded-md border">
            ...
            data.map((item) => (
              <TableRow key={item.id}>
                <TableCell className="font-medium">
                  <Link
                    href={`/service/measurement/${item.id}`}
                ...
                <TableCell
                  ...
                  title={item.lead?.address || item.lead?.community || '-'}
                >
                  <div className="flex items-center gap-1">
                    <MapPin className="text-muted-foreground h-3 w-3" />
                    <span>
                      {item.lead?.community} {item.lead?.address}
                    </span>
                  </div>
                </TableCell>
                <TableCell>
      ...
      {selectedTask && (
        <DispatchDialog
          ...
          taskNo={selectedTask.measureNo ?? undefined}
          defaultScheduledAt={
            selectedTask.scheduledAt ? new Date(selectedTask.scheduledAt) : undefined
          }
        />
      )}
    </div>
...
function StatusBadge({ status }: { status: MeasureTaskStatus }) {
  ...
  switch (status) {
    ...
    case 'PENDING_VISIT':
      variant = 'default';
      break; // Highlight active task
    case 'PENDING_CONFIRM':
      variant = 'destructive';
      break; // Needs attention
    case 'COMPLETED':
      variant = 'outline';
      break;
    case 'CANCELLED':
