...
export function MeasurementToolbar({
  ...
  return (
    ...
    >
      <Select
        value={workerOptions.some((w) => w.id === workerId) ? workerId : 'ALL'}
        onValueChange={(v) => handleFilterChange('workerId', v)}
      >
        <SelectTrigger className="bg-muted/20 w-[120px] border-white/10">
      ...
      <Select
        value={salesOptions.some((s) => s.id === salesId) ? salesId : 'ALL'}
      ...
      <Select
        value={channelId !== 'ALL' ? channelId : 'ALL'}
