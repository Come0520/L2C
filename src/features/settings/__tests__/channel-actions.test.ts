...
import { getChannelCategories, getChannels, createChannel, deleteChannel } from '../actions';
...
describe('Channel Actions - Task 4: Channel CRUD & Cascade Delete', () => {
  ...
  describe('getChannelCategories', () => {
    it('应该返回所有顶级渠道分类', async () => {
      ...
      vi.mocked(db.query.marketChannels.findMany).mockResolvedValue(
        mockCategories as unknown as Array<{ id: string; name: string; parentId: string | null }>
      );
  ...
  describe('getChannels', () => {
    it('应该返回当前租户的渠道列表', async () => {
      ...
      vi.mocked(db.query.marketChannels.findMany).mockResolvedValue(
        mockChannels as unknown as Array<{ id: string; name: string; tenantId: string }>
      );
  ...
  describe('deleteChannel', () => {
    ...
    it('存在子渠道时应禁止删除', async () => {
      ...
      vi.mocked(db.query.marketChannels.findFirst)
        .mockResolvedValueOnce({ id: channelId, tenantId: mockTenantId } as unknown as {
          id: string;
          tenantId: string;
        })
        .mockResolvedValueOnce({ id: 'child-1', parentId: channelId } as unknown as {
          id: string;
          parentId: string;
        });
