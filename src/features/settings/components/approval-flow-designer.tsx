...
export function ApprovalFlowDesigner({ initialFlow, roles, onSave }: ApprovalFlowDesignerProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative flex flex-col items-center space-y-4 pb-20">
        <div className="bg-border absolute top-0 bottom-0 left-1/2 -z-10 w-0.5" />

        <div className="bg-primary/10 text-primary border-primary/20 bg-background z-10 rounded-full border px-4 py-2 text-sm font-medium">
          开�?
        </div>
        ...
        {nodes.map((node, index) => (
          <div key={index} className="relative z-10 flex flex-col items-center">
            <Card className="border-l-primary w-[300px] border-l-4 shadow-sm transition-shadow hover:shadow-md">
              <CardContent className="flex items-center justify-between p-4">
                <div>
                  <div className="font-medium">{node.name}</div>
                  <div className="text-muted-foreground mt-1 text-xs">
                    {node.type === 'role' ? '角色审批' : '指定人员'} :
                    <Badge variant="outline" className="ml-1 h-5 py-0 text-[10px]">
                      {node.type === 'role'
                  ...
                  {node.condition && (
                    <div className="mt-2 inline-block rounded border border-yellow-200 bg-yellow-50 px-1 py-0.5 text-[10px] text-yellow-700">
                      条件: {node.condition}
                    </div>
                  )}
                </div>
            ...
            {index < nodes.length - 1 && (
              <ArrowDown className="text-muted-foreground my-2 h-4 w-4" />
            )}
          </div>
        ))}
        ...
        <div className="bg-background text-muted-foreground z-10 mt-8 rounded-full border px-4 py-2 text-sm font-medium">
          结束
        </div>
      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label>审批类型</Label>
              <Select
                onValueChange={(v) => setValue('type', v as 'role' | 'specific_user')}
            ...
            <div className="space-y-2">
              ...
              {errors.targetId && (
                <p className="text-destructive text-xs">{errors.targetId.message}</p>
              )}
            </div>
