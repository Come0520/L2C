    ...
    startTransition(async () => {
      try {
        const result = await updateNotificationPreferenceAction({
          notificationType: notificationType as Parameters<
            typeof updateNotificationPreferenceAction
          >[0]['notificationType'],
          channels: newChannels as Parameters<
            typeof updateNotificationPreferenceAction
          >[0]['channels'],
        });
        if (result.success) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          管理您接收通知的方式。站内通知始终开启，确保您不会错过重要信息。
        </CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4">
          {prefsData.notificationTypes.map((type) => {
            ...
            return (
              <div
                key={type}
                className="bg-card hover:bg-accent/5 flex flex-col justify-between gap-4 rounded-lg border p-4 transition-colors sm:flex-row sm:items-center"
              >
                {/* 通知类型说明 */}
                <div className="space-y-1">
                  <Label className="text-base font-medium">{typeConfig.label}</Label>
                  <p className="text-muted-foreground text-sm">{typeConfig.description}</p>
                </div>
                    ...
                    return (
                      <div key={channel} className="flex items-center gap-2">
                        ...
                        <Label
                          ...
                          className={`flex cursor-pointer items-center gap-1.5 text-sm ${isDisabled ? 'cursor-not-allowed opacity-50' : ''}`}
                        >
                          <Icon className="h-4 w-4" />
