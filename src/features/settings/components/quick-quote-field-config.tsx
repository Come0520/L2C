                ...
                <div key={groupKey} className="space-y-3">
                  <h4 className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
                    ...
                    <Badge variant="secondary" className="text-xs font-normal">
                      {groupedFields[groupKey]?.filter((f) => config.quickModeFields.includes(f.id))
                        .length ?? 0}
                  ...
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
                    {groupedFields[groupKey]?.map((field) => (
                      <label
                        key={field.id}
                        className="bg-card hover:bg-accent/50 flex cursor-pointer items-center gap-2 rounded-md border p-2 transition-colors"
                      >
                        <Checkbox
      ...
      <div className="space-y-4">
        <Card className="sticky top-4">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">以下字段将在快速报价模式下显示</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {Object.entries(FIELD_GROUPS)
              ...
              {selectedFields.length === 0 && (
                <p className="text-muted-foreground py-4 text-center text-sm">请勾选至少一个字段</p>
              )}
