      ...
      try {
        ...
        if (settings && Object.keys(settings).length > 0 && isMounted) {
          ...
          const mappedData: ShowroomSettingsFormData = {
            ENABLE_SHOWROOM_WRITE_FOR_ALL:
              (settings.ENABLE_SHOWROOM_WRITE_FOR_ALL as boolean) ?? true,
          };
          form.reset(mappedData);
        }
      } catch (err) {
        if (isMounted) {
          logger.error('加载云展厅设置失败:', err);
          setError(err instanceof Error ? err.message : '获取配置失败');
          toast.error('加载云展厅设置失败');
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  if (error) {
    return (
      <Card className="border-destructive/50">
        <CardContent className="text-destructive flex flex-col items-center justify-center py-12 text-center">
          <p className="mb-2 text-sm font-medium">配置加载失败</p>
