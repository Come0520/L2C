...
export function SplitRulesConfig() {
  ...
  const openDialog = (rule?: SplitRule) => {
    if (rule) {
      setEditingRule(rule);
      const ruleType =
        RULE_TYPE_OPTIONS.find((opt) => opt.conditions === rule.conditions)?.value || 'supplier';
      setFormData({
        name: rule.name,
        ruleType,
        priority: rule.priority,
  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        const selectedType = RULE_TYPE_OPTIONS.find((opt) => opt.value === formData.ruleType);
        ...
        if (editingRule) {
          ...
          setRules(
            rules.map((r) => (r.id === editingRule.id ? { ...r, ...input, isActive: true } : r))
          );
          toast.success('规则已更新');
        } else {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground text-sm">规则按优先级顺序执行（数字越大优先级越高）</p>
            ...
            {rules.length === 0 ? (
              ...
              rules.map((rule) => (
                <TableRow key={rule.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{getRuleTypeLabel(rule.conditions)}</Badge>
                  </TableCell>
                  <TableCell>
                    <Switch
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingRule ? '编辑拆单规则' : '新增拆单规则'}</DialogTitle>
          </DialogHeader>
