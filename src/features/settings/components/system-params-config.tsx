...
export function SystemParamsConfig({ initialValues }: SystemParamsConfigProps) {
  ...
  const onSubmit = async (data: SystemParamsFormData) => {
    try {
      const { batchUpdateSettings } =
        await import('@/features/settings/actions/system-settings-actions');
      await batchUpdateSettings(
        Object.fromEntries(Object.entries(data).map(([key, value]) => [`system.${key}`, value]))
      );
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <h4 className="text-muted-foreground text-xs font-medium">报价设置</h4>
            ...
            {/* 提醒规则 */}
            <div className="space-y-2">
              <h4 className="text-muted-foreground text-xs font-medium">提醒规则</h4>
              <div className="grid grid-cols-2 gap-3">
                <FormField
                  control={form.control}
            ...
            {/* 服务排期 */}
            <div className="space-y-2">
              <h4 className="text-muted-foreground text-xs font-medium">服务排期</h4>
              <div className="grid grid-cols-2 gap-3">
                <FormField
                  control={form.control}
