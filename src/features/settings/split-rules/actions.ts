...
const createSplitRuleActionInternal = createSafeAction(
  mockActionSchema,
  async (_data, { session }) => {
    // 权限检查：需要设置管理权限
...
const updateSplitRuleActionInternal = createSafeAction(
  mockActionSchema,
  async (_data, { session }) => {
    // 权限检查：需要设置管理权限
    await checkPermission(session, PERMISSIONS.SETTINGS.MANAGE);
...
const deleteSplitRuleActionInternal = createSafeAction(
  mockActionSchema,
  async (_data, { session }) => {
    // 权限检查：需要设置管理权限
    await checkPermission(session, PERMISSIONS.SETTINGS.MANAGE);
