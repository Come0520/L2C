 ...
 * 1. 响应式主图展示
 * 2. 交互式缩略图选框
 * 3. 模态窗口 lightbox 预览 (支持 framer-motion 动画)
 *
 * @param props 画廊配置 (图片路径列表、Alt 等)
 */
export function ImageGallery({ images, alt = '商品图片', className }: ImageGalleryProps) {
  ...
  // 确保至少有一张占位图
  const safeImages =
    images.length > 0 ? images : ['https://via.placeholder.com/800x600?text=No+Image'];
  ...
  return (
    <div className={cn('space-y-3', className)}>
      {/* 主图区域 */}
      <div className="group bg-muted relative aspect-square w-full overflow-hidden rounded-2xl">
        ...
        {safeImages.length > 1 && (
          <>
            <button
              onClick={() => paginate(-1)}
              className="absolute top-1/2 left-2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-black/30 text-white opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100 hover:bg-black/50"
              aria-label="上一张"
            >
            ...
            <button
              onClick={() => paginate(1)}
              className="absolute top-1/2 right-2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-black/30 text-white opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100 hover:bg-black/50"
              aria-label="下一张"
            >
        ...
        <div className="scrollbar-hide flex gap-2 overflow-x-auto pb-1">
          {safeImages.map((img, i) => (
            ...
            >
              ...
              <img src={img} alt={`缩略图 ${i + 1}`} className="h-full w-full object-cover" />
            </button>
          ))}
        </div>
      )}
