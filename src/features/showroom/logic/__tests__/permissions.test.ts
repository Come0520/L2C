...
describe('canManageShowroomItem 权限逻辑单元测试', () => {
    it('Session 为空时应返回 false', async () => {
        ...
        expect(await canManageShowroomItem({ user: {} } as any, 'u1')).toBe(false);
    });

    it('超级管理员 (SUPER_ADMIN) 应有权管理任何素材', async () => {
        const session = { user: { id: 'admin1', role: 'SUPER_ADMIN' } } as any;
        expect(await canManageShowroomItem(session, 'u1')).toBe(true);
    });

    it('管理员 (ADMIN) 应有权管理任何素材', async () => {
        const session = { user: { id: 'admin2', role: 'ADMIN' } } as any;
        expect(await canManageShowroomItem(session, 'u1')).toBe(true);
    });

    it('创建者本人应有权管理该素材', async () => {
        const session = { user: { id: 'u1', role: 'SALES' } } as any;
        expect(await canManageShowroomItem(session, 'u1')).toBe(true);
    });

    it('非创建者且非管理员应被拒绝', async () => {
        const session = { user: { id: 'u2', role: 'SALES' } } as any;
