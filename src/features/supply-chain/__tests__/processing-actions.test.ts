  ...
  db: {
    ...
    update: vi.fn().mockImplementation(() => ({
      set: vi.fn().mockImplementation(() => ({
        where: vi.fn().mockImplementation(() => ({
          returning: vi
            .fn()
            .mockResolvedValue([{ id: '123e4567-e89b-12d3-a456-426614174003', status: 'UPDATED' }]),
        })),
      })),
    })),
    ...
    select: vi.fn().mockImplementation(() => ({
      from: vi.fn().mockImplementation(() => ({
        leftJoin: vi.fn().mockImplementation(function () {
          return this;
        }),
        ...
        then: (resolve: any) =>
          resolve([{ id: '123e4567-e89b-12d3-a456-426614174003', status: 'PENDING', total: 1 }]),
      })),
    })),
    query: {
      processingOrders: {
  ...
  describe('createProcessingOrder', () => {
    it('should create a processing order successfully', async () => {
      const input = {
        ...
        items: [{ orderItemId: '123e4567-e89b-12d3-a456-426614174004' }],
        remark: 'Test Remark',
      };
