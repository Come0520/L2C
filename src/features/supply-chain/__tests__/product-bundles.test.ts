...
describe('产品套件 (Product Bundles)', () => {
  beforeEach(() => {
    ...
    (db.transaction as ReturnType<typeof vi.fn>).mockImplementation(async (cb: CallableFunction) =>
      cb(mockTx)
    );
    // 恢复 mockTx 的默认返回值
  ...
  describe('createProductBundle - 创建套件', () => {
    ...
    it('SKU 已存在时应返回错误', async () => {
      ...
      const input = {
        ...
        retailPrice: 100,
        channelPrice: 80,
        items: [{ productId: 'prod-1', quantity: 1, unit: '个' }],
      };
  ...
  describe('updateProductBundle - 更新套件', () => {
    it('成功更新套件及其子项', async () => {
      const input = {
        id: 'bundle-1',
        name: '更新后的套件名',
        items: [{ productId: 'prod-3', quantity: 3, unit: '块' }],
      };
