...
describe('Supplier Actions (L5)', () => {
      ...
      mockDb.insert.mockReturnValue({
        values: vi.fn().mockReturnValue({
          returning: vi
            .fn()
            .mockResolvedValue([
              { id: 'sup-1', supplierNo: 'SUP-000001', name: 'Vendor A', supplierType: 'SUPPLIER' },
            ]),
        }),
      } as any);
  ...
  describe('updateSupplier', () => {
    ...
    it('未找到该供应商时应当抛出错误', async () => {
      ...
      await expect(updateSupplier({ id: 'sup-not', name: 'Vendor B' })).rejects.toThrow(
        '更新失败，未找到供应商'
      );
    });
  });
  ...
  describe('deleteSupplier', () => {
    ...
    it('当有采购数据时应当阻止删除', async () => {
      ...
      await expect(deleteSupplier({ id: 'sup-1' })).rejects.toThrow(
        '该供应商已有采购数据，无法删除'
      );
      ...
      mockDb.query.purchaseOrders.findMany.mockResolvedValue([
        {
          id: 'po-1',
          createdAt: new Date(),
          shippedAt: new Date(Date.now() - 100000),
