 ...
 */
export async function getPendingPurchaseItems(input: z.infer<typeof getPendingItemsSchema>) {
  ...
  logger.info('[supply-chain] getPendingPurchaseItems 开始执行:', {
    itemType,
    productType,
    supplierId,
    orderId,
  ...
  logger.info('[supply-chain] getPendingPurchaseItems 执行完成', {
 ...
 */
export async function assignToSupplier(input: z.infer<typeof assignToSupplierSchema>) {
  const session = await auth();
  if (!session?.user) throw new Error('Unauthorized');
  ...
  logger.info('[supply-chain] assignToSupplier 开始执行:', {
      ...
      const updateResult = await tx
        ...
        .where(
          and(
            ...
            eq(orderItems.tenantId, tenantId),
            isNull(orderItems.poId) // 乐观锁：必须还处于未分配状态
          )
        )
        .returning({ id: orderItems.id });

      if (updateResult.length !== itemIds.length) {
        // 如果更新成功的行数不对，说明被并发抢占，直接打断整个大事务
        throw new Error(
          `并发冲突：订单编号为 ${groupOrderId} 的部分明细已被其他操作抢先分配，请刷新列表后再试`
        );
      }
    }
  });

  logger.info('[supply-chain] assignToSupplier 执行成功:', {
 ...
 */
export async function submitForApproval(input: z.infer<typeof submitForApprovalSchema>) {
  const session = await auth();
  if (!session?.user) throw new Error('Unauthorized');
  ...
  logger.info('[supply-chain] submitForApproval 开始执行:', { poIdsCount: poIds.length });
  ...
  return await db.transaction(async (tx) => {
    ...
    const updateResult = await tx
      ...
      // P1-05 修复 [CAS保护]: 只有当状态严格为 DRAFT 时才允许流转
      .where(
        and(
          inArray(purchaseOrders.id, draftPoIds),
          eq(purchaseOrders.tenantId, tenantId),
          eq(purchaseOrders.status, PO_STATUS.DRAFT)
        )
      )
    ...
    if (updateResult.length !== draftPoIds.length) {
      throw new Error(
        `状态已变更：预期提交 ${draftPoIds.length} 张单据，实际只锁定了 ${updateResult.length} 张单据，请刷新列表重试`
      );
    }
    ...
    logger.info('[supply-chain] submitForApproval 执行成功:', {
 ...
 */
export async function mergeToPurchaseOrder(input: z.infer<typeof mergeToPurchaseOrderSchema>) {
  const session = await auth();
  if (!session?.user) throw new Error('Unauthorized');
  ...
  logger.info('[supply-chain] mergeToPurchaseOrder 开始执行:', {
    orderItemIdsCount: orderItemIds.length,
    forcedSupplierId,
  });
  ...
  await db.transaction(async (tx) => {
    for (const [groupSupplierId, groupItems] of supplierGroups) {
      ...
      if (updateResult.length !== itemIds.length) {
        // 如果更新成功的行数不对，说明合并期间条目被他人分配走，打断大事务
        throw new Error(`并发冲突：部分面料/成品已被其他操作抢先建单采购，请刷新列表后再试`);
      }
    }
  });

  logger.info('[supply-chain] mergeToPurchaseOrder 执行成功:', {
    createdPOCount: createdPOIds.length,
