...
export async function getProcessingOrders(params: {
  ...
  const [results, [{ total: totalCount }]] = await Promise.all([resultsPromise, countPromise]);
 ...
 */
export async function getProcessingOrderById({
  id,
}: {
  id: string;
}): Promise<{ success: boolean; data?: ProcessingOrderDetail; error?: string }> {
  ...
  const result = await db
    ...
    .leftJoin(suppliers, eq(workOrders.supplierId, suppliers.id))
    .leftJoin(orders, eq(workOrders.orderId, orders.id))
    .where(and(eq(workOrders.id, id), eq(workOrders.tenantId, session.user.tenantId)));
...
export async function createProcessingOrder(data: z.infer<typeof createProcessingOrderSchema>) {
  ...
  logger.info('[supply-chain] createProcessingOrder 开始创建:', {
 ...
 */
export async function updateProcessingOrder(
  id: string,
  data: z.infer<typeof updateProcessingOrderSchema>
) {
  const session = await auth();
  if (!session?.user?.id) return { success: false, error: '未授权' };
