...
const createProductBundleActionInternal = createSafeAction(
  createProductBundleSchema,
  async (data, { session }) => {
    logger.info('[supply-chain] createProductBundle 开始执行:', {
...
export async function createProductBundle(
  params: Parameters<typeof createProductBundleActionInternal>[0]
) {
  return createProductBundleActionInternal(params);
}

const updateProductBundleActionInternal = createSafeAction(
  ...
  async (data, { session }) => {
    ...
    return await db.transaction(async (tx) => {
      const [bundle] = await tx
        ...
        .where(and(eq(productBundles.id, id), eq(productBundles.tenantId, session.user.tenantId)))
        .returning();
      ...
      if (items) {
        await tx.delete(productBundleItems).where(eq(productBundleItems.bundleId, id));
...
export async function updateProductBundle(
  params: Parameters<typeof updateProductBundleActionInternal>[0]
) {
...
const deleteProductBundleActionInternal = createSafeAction(
  deleteProductBundleSchema,
  async ({ id }, { session }) => {
