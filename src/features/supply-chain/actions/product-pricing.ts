...
export const calculateProductCost = async (
  ...
  try {
    ...
    const totalCost = purchasePrice.add(logisticsCost).add(processingCost).add(lossCost);

    logger.info('[supply-chain] calculateProductCost 执行成功:', {
...
export const calculateChannelPrice = (
  ...
  logger.info('[supply-chain] calculateChannelPrice 输入:', {
    basePrice,
    mode,
    fixedPrice,
    discountRate,
