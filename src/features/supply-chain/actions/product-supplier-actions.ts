...
export async function getProductSuppliers(
  productId: string
): Promise<{ success: boolean; data: ProductSupplierListItem[]; error?: string }> {
...
export async function addProductSupplier(data: z.infer<typeof productSupplierSchema>) {
  logger.info('[supply-chain] addProductSupplier 开始关联:', {
...
export async function updateProductSupplier(
  ...
  const [existing] = await db
    ...
    .from(productSuppliers)
    .where(and(eq(productSuppliers.id, id), eq(productSuppliers.tenantId, tenantId)))
    .limit(1);
  ...
  if (parsed.data.leadTimeDays !== undefined) updateFields.leadTimeDays = parsed.data.leadTimeDays;
