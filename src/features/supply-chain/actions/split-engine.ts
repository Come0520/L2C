...
interface EnrichedOrderItem {
  ...
  status?: string | null; // 添加 status 字段用于幂等性校验
}
...
export async function executeSplitRouting(
  ...
  // 1. 获取订单项并联查产品信息
  let enrichedItems = await getEnrichedOrderItems(orderId, tenantId);

  // 【幂等性二次保护】过滤掉已经被拆分的项 (状态不等于 PENDING 的项)
  const originalLength = enrichedItems.length;
  enrichedItems = enrichedItems.filter((item) => item.status === 'PENDING');
  if (enrichedItems.length < originalLength) {
    logger.warn(
      `[supply-chain] executeSplitRouting: 拦截了 ${originalLength - enrichedItems.length} 个非 PENDING 状态的项，防止重复拆单`
    );
  }

  if (enrichedItems.length === 0) {
    logger.info('[supply-chain] executeSplitRouting: 订单无有效待拆分项，跳过');
    return {
      createdPOIds: [],
      createdTaskIds: [],
      pendingPoolItemIds: [],
  ...
  for (const item of items) {
    ...
    switch (supplierType) {
      ...
      case 'BOTH':
        ...
        logger.info('[supply-chain] resolveBySupplierType: 检测到兼备供应商，流向设为 PO(FABRIC)', {
          supplierId,
        });
        result.push({
          ...item,
...
async function loadSplitRouteRules(tenantId: string): Promise<SplitRuleRow[]> {
  const rules = await db.query.splitRouteRules.findMany({
    where: and(eq(splitRouteRules.tenantId, tenantId), eq(splitRouteRules.isActive, true)),
 ...
 */
function matchConditionRules(item: EnrichedOrderItem, rules: SplitRuleRow[]): SplitRuleRow | null {
...
async function generateSplitResult(
  ...
  const customCount = routedItems.filter(
    (i) => i.poType === 'FABRIC' || i.targetDocType === 'WO'
  ).length;
...
function groupBySupplierId(
  items: SplitRoutedItem[]
): Map<string, { key: GroupKey; items: SplitRoutedItem[] }> {
 ...
 * 获取订单项并联查产品信息
 */
async function getEnrichedOrderItems(
  orderId: string,
  tenantId: string
  ...
  const items = await db
    ...
    .where(
      and(
        ...
        eq(orderItems.status, 'PENDING') // 【幂等性保护】仅拆分处于 PENDING 状态的订单项
      )
    );
