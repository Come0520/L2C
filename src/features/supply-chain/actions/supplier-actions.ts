...
import { SupplierRating, SupplierRanking } from '../types';
import { logger } from '@/shared/lib/logger';

const createSupplierActionInternal = createSafeAction(
  createSupplierSchema,
  async (data, { session }) => {
    try {
      await checkPermission(session, PERMISSIONS.SUPPLY_CHAIN.SUPPLIER_MANAGE);
      logger.info('[supply-chain] 创建供应商:', {
        name: data.name,
        tenantId: session.user.tenantId,
      });

      const existing = await db.query.suppliers.findFirst({
        where: and(eq(suppliers.tenantId, session.user.tenantId), eq(suppliers.name, data.name)),
      });
...
const getSuppliersActionInternal = createSafeAction(
  getSuppliersSchema,
  async (params, { session }) => {
    ...
    if (params.query) {
      conditions.push(
        sql`(${suppliers.name} ILIKE ${`%${params.query}%`} OR ${suppliers.supplierNo} ILIKE ${`%${params.query}%`})`
      );
    }
...
const getSupplierByIdActionInternal = createSafeAction(
  getSupplierByIdSchema,
  async ({ id }, { session }) => {
    await checkPermission(session, PERMISSIONS.SUPPLY_CHAIN.VIEW);

    const supplier = await db.query.suppliers.findFirst({
      where: and(eq(suppliers.tenantId, session.user.tenantId), eq(suppliers.id, id)),
    });
...
const updateSupplierActionInternal = createSafeAction(
  updateSupplierSchema,
  async (data, { session }) => {
    try {
      await checkPermission(session, PERMISSIONS.SUPPLY_CHAIN.SUPPLIER_MANAGE);
...
const getSupplierRatingActionInternal = createSafeAction(
  getSupplierRatingSchema,
  async ({ supplierId, startDate, endDate }, { session }) => {
    await checkPermission(session, PERMISSIONS.SUPPLY_CHAIN.VIEW);
    ...
    const qualityRate =
      totalDelivered > 0 ? Math.max(0, Math.round((1 - issueCount / totalDelivered) * 100)) : null;

    // 3. 综合评分
    const overallScore =
      onTimeRate !== null && qualityRate !== null
...
export async function getSupplierRating(
  params: z.infer<typeof getSupplierRatingSchema>
): Promise<ActionState<SupplierRating>> {
  const session = await auth();

  if (!session?.user?.id) throw new Error('未授权');

  const { supplierId, startDate = '', endDate = '' } = params;
  logger.info('[supply-chain] getSupplierRating 获取评分:', {
    supplierId,
...
const getSupplierRankingsActionInternal = createSafeAction(
  getSupplierRankingsSchema,
  async (_params, { session }) => {
    ...
    const poStatsPromise = db
      ...
      .from(purchaseOrders)
      .where(and(eq(purchaseOrders.tenantId, tenantId), eq(purchaseOrders.status, 'DELIVERED')))
    ...
    const rankings = allSuppliers
      ...
      .toSorted((a, b) => b.deliveredPOs - a.deliveredPOs);

    return { rankings, total: rankings.length };
  }
);

const deleteSupplierActionInternal = createSafeAction(
  deleteSupplierSchema,
  async ({ id }, { session }) => {
    try {
      await checkPermission(session, PERMISSIONS.SUPPLY_CHAIN.SUPPLIER_MANAGE);
 ...
 */
export async function getSupplierRankings(): Promise<
  ActionState<{ rankings: SupplierRanking[]; total: number }>
> {
