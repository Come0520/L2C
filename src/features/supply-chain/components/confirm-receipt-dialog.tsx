...
export function ConfirmReceiptDialog({ open, onOpenChange, po }: ConfirmReceiptDialogProps) {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          if (res.data.length > 0) {
            setTimeout(() => {
              if (isActive) setValue('warehouseId', res.data[0].id);
            }, 0);
          }
        }
      } finally {
        if (isActive) setLoadingWarehouses(false);
      }
    };
    loadData();
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="receivedDate" type="datetime-local" {...register('receivedDate')} />
              {errors.receivedDate && (
          ...
          <div className="max-h-[300px] space-y-2 overflow-y-auto rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {fields.map((field, index) => {
                  ...
                  return (
                    <TableRow key={field.id}>
                      ...
                      <TableCell>{Number(orderedItem?.quantity || 0)}</TableCell>
                      <TableCell>
                        <Input
                          type="number"
                          min="0"
            ...
            {errors.items && <p className="px-4 py-2 text-sm text-red-500">请检查入库数量</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="remark">备注</Label>
            <Textarea id="remark" rows={2} placeholder="填写收货备注..." {...register('remark')} />
