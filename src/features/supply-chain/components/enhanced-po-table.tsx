...
export function EnhancedPOTable({ data }: POTableProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showFilters && (
        <div className="glass-panel flex gap-4 rounded-md border p-4">
          <div className="flex-1">
            <label className="mb-2 block text-sm font-medium">付款状态</label>
            <Select value={initialPaymentStatus} onValueChange={handlePaymentStatusChange}>
              <SelectTrigger>
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <span
                      className={
                        item.type === 'FINISHED'
                          ? 'glass-alert-info rounded px-2 py-1 text-xs'
                          : item.type === 'FABRIC'
                            ? 'glass-alert-success rounded px-2 py-1 text-xs'
                            : 'glass-step-inactive rounded px-2 py-1 text-xs'
                      }
                    >
                      {item.type === 'FINISHED'
                        ? '成品'
                        ...
                        item.status === 'COMPLETED'
                          ...
                          : item.status === 'CANCELLED'
                            ...
                            : item.status === 'PENDING_PAYMENT' ||
                              ...
                              ? 'glass-alert-warning rounded px-2 py-1 text-xs'
                              : item.status === 'DRAFT'
                                ? 'glass-step-inactive rounded px-2 py-1 text-xs'
                                : 'glass-alert-info rounded px-2 py-1 text-xs'
                      }
