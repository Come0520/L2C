...
export function InventoryList({ initialItems, pagination }: InventoryListProps) {
  return (
            ...
            {initialItems.length === 0 ? (
              ...
              initialItems.map((item) => (
                <TableRow key={item.id}>
                  <TableCell className="font-medium">{item.productName || '未知产品'}</TableCell>
                  <TableCell>{item.warehouseName || '未知仓库'}</TableCell>
                  <TableCell>
                    <span className={item.quantity < 0 ? 'font-bold text-red-500' : ''}>
                      {item.quantity}
                    </span>
                  </TableCell>
                  <TableCell>
