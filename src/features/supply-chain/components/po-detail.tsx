...
export function PODetail({ data, onUpdateStatus }: PODetailProps) {
  ...
  const handleConfirmOrder = useCallback(
    () => handleStatusChange('IN_PRODUCTION'),
    [handleStatusChange]
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {data.status === 'DRAFT' && <Button onClick={handleConfirmOrder}>确认下单</Button>}
          {data.status === 'IN_PRODUCTION' && <Button onClick={handleMarkReady}>备货完成</Button>}
          {data.status === 'READY' && (
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-muted-foreground text-sm">采购单号</label>
                <p className="font-medium">{data.poNo}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">关联订单</label>
                <p className="font-medium">{data.orderNo || '-'}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">供应商</label>
              ...
              <div>
                <label className="text-muted-foreground text-sm">采购成本</label>
                <p className="font-medium">¥{data.totalAmount}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">付款状态</label>
            ...
            <div className="space-y-2">
              <div>
                <label className="text-muted-foreground text-sm">外部单号</label>
                <Input defaultValue={data.externalPoNo || ''} placeholder="工厂方单号" />
              </div>
              <div>
                <label className="text-muted-foreground text-sm">备注</label>
                <Textarea defaultValue={data.remark || ''} placeholder="备注信息" rows={3} />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-muted-foreground text-sm">供应商确认截图</label>
              {data.supplierQuoteImg ? (
                ...
                <div className="mt-2 rounded-md border-2 border-dashed p-8 text-center">
                  <ImageIcon className="text-muted-foreground mx-auto h-12 w-12" />
                  <p className="text-muted-foreground mt-2 text-sm">暂无确认截图</p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="mt-4"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-muted-foreground text-sm">发送方式</label>
                <p className="font-medium">{data.sentMethod || '-'}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">发送时间</label>
                <p className="font-medium">
                  {data.sentAt ? format(new Date(data.sentAt), 'yyyy-MM-dd HH:mm') : '-'}
                </p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">生产完成时间</label>
                <p className="font-medium">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-muted-foreground text-sm">物流公司</label>
                <p className="font-medium">{data.logisticsCompany}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">物流单号</label>
                <p className="font-medium">{data.logisticsNo}</p>
              </div>
              <div>
                <label className="text-muted-foreground text-sm">发货时间</label>
                <p className="font-medium">
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                  ...
                  <div key={index} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      ...
                      {index < (data.auditLogs?.length || 0) - 1 && (
                        <div className="bg-muted w-0.5 flex-1" />
                      )}
                    </div>
                ...
                {(!data.auditLogs || data.auditLogs.length === 0) && (
                  <p className="text-muted-foreground text-center">暂无操作日志</p>
                )}
              </div>
            </CardContent>
          </Card>
