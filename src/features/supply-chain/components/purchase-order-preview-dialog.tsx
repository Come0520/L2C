            ...
            <span
              className={
                data.status === 'COMPLETED'
                  ? 'glass-alert-success rounded-full px-3 py-1 text-sm'
                  : data.status === 'CANCELLED'
                    ? 'glass-step-inactive rounded-full px-3 py-1 text-sm'
                    : data.status === 'PENDING_PAYMENT' || data.status === 'PENDING_CONFIRMATION'
                      ? 'glass-alert-warning rounded-full px-3 py-1 text-sm'
                      : data.status === 'DRAFT'
                        ? 'glass-step-inactive rounded-full px-3 py-1 text-sm'
                        : 'glass-alert-info rounded-full px-3 py-1 text-sm'
              }
            >
              {PO_STATUS_LABELS[data.status ?? ''] || data.status || '-'}
            </span>
          </DialogTitle>
          <DialogDescription>采购单号: {data.poNo}</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p>
                  {data.createdAt ? format(new Date(data.createdAt), 'yyyy-MM-dd HH:mm:ss') : '-'}
                </p>
              </div>
              {data.expectedDate && (
                <div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-primary text-xl font-bold">¥{data.totalAmount}</p>
              </div>
            </div>
          </div>
