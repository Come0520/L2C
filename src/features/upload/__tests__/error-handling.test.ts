...
vi.mock('@/shared/lib/server-action', () => ({
  createSafeAction:
    (_schema: unknown, handler: (p: unknown, ctx: unknown) => unknown) => (input: unknown) =>
      handler(input, { session: MOCK_SESSION }),
}));
...
describe('Upload 错误处理', () => {
  ...
  it('文件存储服务不可用时应返回上传失败错误', async () => {
    ...
    formData.append(
      'file',
      createMockFile({ name: 'no-space.jpg', size: 2048, type: 'image/jpeg' })
    );
  ...
  it('writeFile 抛出权限错误时不应导致未捕获异常', async () => {
    ...
    formData.append(
      'file',
      createMockFile({ name: 'perm-denied.png', size: 512, type: 'image/png' })
    );
