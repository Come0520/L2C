...
vi.mock('@/shared/lib/server-action', () => ({
  createSafeAction:
    (_schema: unknown, handler: (p: unknown, ctx: unknown) => unknown) => (input: unknown) =>
      handler(input, { session: MOCK_SESSION_A }),
}));
...
describe('Upload 安全与权限', () => {
  ...
  it('租户 A 的上传文件 URL 包含自身 tenantId，而非租户 B', async () => {
    ...
    formData.append(
      'file',
      createMockFile({ name: 'report.pdf', size: 5000, type: 'application/pdf' })
    );
  ...
  it('上传操作应记录审计日志（AuditService）', async () => {
    ...
    formData.append(
      'file',
      createMockFile({ name: 'audit-test.jpg', size: 2048, type: 'image/jpeg' })
    );
