...
export class ChangeOrderService {
  ...
  static async calculateDiff(
    ...
    for (const change of changes) {
      ...
      const item = await db.query.orderItems.findFirst({
        where: and(eq(orderItems.id, change.orderItemId), eq(orderItems.tenantId, tenantId)),
        columns: {
          productName: true,
          quantity: true,
          unitPrice: true,
  ...
  static async approveRequest(
    ...
    const runInTx = async (tx: any) => {
      ...
      // 2. Apply Changes Logic - 添加租户隔离与乐观锁支持 (F2 & F4)
      if (request.type === 'FIELD_CHANGE' && request.newData) {
        ...
        if (currentOrder) {
          ...
          delete updateData.costPrice; // 禁止在普通字段修改中串改成本
          ...
          if (!res) throw new Error('订单已被并发修改，无法执行变更');
        }
      }
      ...
      if (diffAmount !== 0) {
        const order = await tx.query.orders.findFirst({
          where: and(eq(orders.id, request.orderId), eq(orders.tenantId, tenantId)),
          columns: { totalAmount: true, balanceAmount: true, version: true },
        });
        ...
        if (order) {
          ...
          // If it was already updated above, the version changed. Let's just do a blind bump on whatever it is right now.
          // To be strictly lock-safe, we should probably do a single update instead of two independent ones, but since this runs inside a transaction, the snapshot is isolated depending on isolation level.
          // Using raw SQL increment is actually safer for balances:
          const [resAmount] = await tx
            .update(orders)
            .set({
          ...
          if (!resAmount) throw new Error('订单总额已被并发修改，差价未能合并');
        }
      }
