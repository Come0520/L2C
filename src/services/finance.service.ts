...
export class FinanceService {
   ...
   */
  static async generateReceivables(
    orderId: string,
    totalAmount: string,
    tenantId: string,
    ratios: number[] = [0.6, 0.4],
    userId: string
  ) {
  ...
  static validateDownPaymentRatio(
   ...
   */
  static async verifyPaymentOrder(
    id: string,
    ...
    return await db.transaction(async (tx) => {
      const order = await tx.query.paymentOrders.findFirst({
        where: and(eq(paymentOrders.id, id), eq(paymentOrders.tenantId, tenantId)),
        with: {
          items: true,
        },
      });
      ...
      if (order.items && order.items.length > 0) {
        for (const item of order.items) {
          const statement = await tx.query.arStatements.findFirst({
            where: and(eq(arStatements.orderId, item.orderId), eq(arStatements.tenantId, tenantId)),
            with: {
              channel: true,
              order: true,
  ...
  private static async calculateCommission(
    tx: DbTransaction,
    statement: InferSelectModel<typeof arStatements> & {
      channel?: InferSelectModel<typeof marketChannels> | null;
    },
    tenantId: string
  ) {
    ...
    if (mode === 'REBATE') {
      ...
      const items = await tx.query.orderItems.findMany({
        where: and(eq(orderItems.orderId, statement.orderId), eq(orderItems.tenantId, tenantId)),
      });
