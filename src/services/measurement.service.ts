...
export class MeasurementService {
  ...
  static async checkFeeStatus(_leadId: string, _tenantId: string) {
    ...
    // 使用真实费用检查逻辑 (RC-05)
    const { checkMeasureFeeAdmission } =
      await import('@/features/service/measurement/logic/fee-admission');
  ...
  static async dispatchTask(
    ...
    return await db.transaction(async (tx) => {
      ...
      if (
        isCheckEnabled &&
        !task.isFeeExempt &&
        task.feeCheckStatus !== 'PAID' &&
        task.feeCheckStatus !== 'WAIVED' &&
        task.feeCheckStatus !== 'NONE'
      ) {
