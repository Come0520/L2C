...
export class POStatusAggregator {
  ...
  private static async determineOrderStatus(
    ...
    const allFinishedReady = finishedPOs.every((p) =>
      ['READY', 'SHIPPED', 'DELIVERED', 'COMPLETED'].includes(p.status || '')
    );
    // Note: STOCKED is not in PurchaseOrderStatusEnum (defined in schema), using DELIVERED instead
    const allFabricStocked = fabricPOs.every((p) =>
      ['DELIVERED', 'COMPLETED'].includes(p.status || '')
    );
    const allStockReady = stockPOs.every((p) =>
      ['READY', 'SHIPPED', 'DELIVERED', 'COMPLETED'].includes(p.status || '')
    );
    ...
    const allShipped = allPOs.every((p) =>
      ['SHIPPED', 'DELIVERED', 'COMPLETED'].includes(p.status || '')
    );
    if (allShipped) {
