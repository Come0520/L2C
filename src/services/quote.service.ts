...
export class QuoteService {
  ...
  static async checkAndExpireQuote(
    ...
    const { QuoteExpirationService } =
      await import('../features/quotes/services/quote-expiration.service');
   ...
   * @returns 过期状态详情
   */
  static async getExpirationInfo(
    quoteId: string,
    tenantId: string
  ): Promise<{
    isExpired: boolean;
  ...
  static async createFromTemplate(
    ...
    const {
      quoteTemplates,
      quoteTemplateRooms: _quoteTemplateRooms,
      quoteTemplateItems: _quoteTemplateItems,
    } = await import('@/shared/api/schema/quotes');
  ...
  static async saveAsTemplate(
    ...
    const { quoteTemplates, quoteTemplateRooms, quoteTemplateItems } =
      await import('@/shared/api/schema/quotes');
    ...
    return await db.transaction(async (tx) => {
      ...
      for (const room of sourceQuote.rooms) {
        const [tRoom] = await tx
          .insert(quoteTemplateRooms)
          .values({
            tenantId,
      ...
      for (const item of sortedItems) {
        ...
        const [tItem] = await tx
          .insert(quoteTemplateItems)
          .values({
            tenantId,
            templateId: template.id,
            roomId: mappedRoomId ?? null,
