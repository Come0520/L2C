    ...
    return await db.transaction(async (tx) => {
      ...
      if (bill.accountId) {
        ...
        if (account) {
          ...
          const balanceBefore = new Decimal(account.balance || '0').toDecimalPlaces(
            2,
      ...
      if (bill.items && bill.items.length > 0) {
        for (const item of bill.items) {
          const statement = await tx.query.arStatements.findFirst({
            where: and(eq(arStatements.id, item.statementId!), eq(arStatements.tenantId, tenantId)),
          });

          if (statement) {
            const receivedBefore = new Decimal(statement.receivedAmount || '0');
            const itemAmount = new Decimal(item.amount);
            const receivedAfter = receivedBefore
              .plus(itemAmount)
              .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
            ...
            if (!statementUpdated) {
              throw new Error('对账单状态已过期（可能已被并发处理），请重跑核销确认');
            }
            ...
            await AuditService.log(tx, {
              ...
              oldValues: {
                ...
                receivedAmount: receivedBefore.toFixed(2),
                pendingAmount: statement.pendingAmount,
              },
              newValues: {
                status: newStatus,
                receivedAmount: receivedAfter.toFixed(2),
                ...
                const [orderUpdated] = await tx
                  ...
                  .where(
                    and(
                      ...
                      eq(orders.version, relatedOrder.version)
                    )
                  )
                  .returning({ id: orders.id });

                if (!orderUpdated) {
                  throw new Error('订单状态已过期（可能已被并发推进），请刷新重试');
                }
