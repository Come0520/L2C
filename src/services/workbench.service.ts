...
export class WorkbenchService {
  ...
  static async getUnifiedTodos(
    ...
    return unstable_cache(
      async () => {
        const isAdmin = userRoles.some((r) => ['ADMIN', 'ADMIN', 'ADMIN'].includes(r));

        try {
          // 并行查询所有分类，各子项异常不阻塞整体
          const [pendingLeads, draftOrders, draftPOs, pendingPrd, pendingAS, pendingApprovals] =
            await Promise.all([
              // 1. 待跟进线索
              db.query.leads
                .findMany({
          ...
          const categories: TodoCategoryMeta[] = [
            ...
            },
            {
              category: 'APPROVAL',
              label: '待审批',
              count: normalizedApprovals.length,
              icon: 'ClipboardCheck',
              color: 'orange',
            },
          ];
