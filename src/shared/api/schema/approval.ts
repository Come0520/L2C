...
// 审批节点定义
export const approvalNodes = pgTable(
  'approval_nodes',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
...
export const approvals = pgTable(
  ...
  (table) => ({
    ...
    apprTenantRequesterIdx: index('idx_approvals_tenant_requester').on(
      table.tenantId,
      table.requesterId
    ),
  })
...
export const approvalTasks = pgTable(
  ...
  (table) => ({
    ...
    taskTenantApproverStatusIdx: index('idx_approval_tasks_tenant_approver_status').on(
      table.tenantId,
      table.approverId,
      table.status
    ),
