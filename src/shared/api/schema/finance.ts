...
// 财务配置表
export const financeConfigs = pgTable(
  'finance_configs',
  {
...
export const financeAccounts = pgTable(
  ...
  (table) => ({
    ...
    accountNoTenantIdx: uniqueIndex('idx_finance_accounts_no_tenant').on(
      table.accountNo,
      table.tenantId
    ),
  })
...
// AR对账单 (AR Statements)
export const arStatements = pgTable(
  'ar_statements',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
...
export const paymentPlanNodes = pgTable(
  'payment_plan_nodes',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
...
export const receiptBills = pgTable(
  'receipt_bills',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
...
export const receiptBillItems = pgTable(
  'receipt_bill_items',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
      .references(() => tenants.id)
      .notNull(),
    receiptBillId: uuid('receipt_bill_id')
      .references(() => receiptBills.id, { onDelete: 'cascade' })
      .notNull(),
    orderId: uuid('order_id')
      .references(() => orders.id)
      .notNull(),
...
export const apSupplierStatements = pgTable(
  'ap_supplier_statements',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
      .references(() => tenants.id)
...
export const paymentBillItems = pgTable(
  'payment_bill_items',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
...
export const apLaborStatements = pgTable(
  'ap_labor_statements',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
...
// 支持正常安装费用和售后扣款记录
export const apLaborFeeDetails = pgTable(
  'ap_labor_fee_details',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
      .references(() => tenants.id)
...
export const reconciliations = pgTable(
  'reconciliations',
  {
    ...
    unmatchedAmount: decimal('unmatched_amount', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
...
export const reconciliationDetails = pgTable(
 ...
 * - 供应商确认应付金额
 */
export const statementConfirmations = pgTable(
 ...
 * 包含每张对账单的明细
 */
export const statementConfirmationDetails = pgTable(
  'statement_confirmation_details',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    tenantId: uuid('tenant_id')
      .references(() => tenants.id)
  ...
  (table) => ({
    confirmationIdx: index('idx_statement_confirmation_details_confirmation').on(
...
export const accountingPeriods = pgTable(
  ...
  (table) => ({
    ...
    yearMonthTenantIdx: uniqueIndex('idx_accounting_periods_ym_tenant').on(
      table.year,
      table.month,
      table.tenantId
    ),
...
export const journalEntries = pgTable(
  'journal_entries',
  {
    ...
    exchangeRate: decimal('exchange_rate', { precision: 15, scale: 6 })
      .notNull()
      .default('1.000000'), // [优化预留] 多币种架构支持：记账本位币汇率
  ...
  (table) => ({
    ...
    voucherNoTenantIdx: uniqueIndex('idx_journal_entries_voucher_tenant').on(
      table.voucherNo,
      table.tenantId
    ),
...
export const voucherTemplates = pgTable(
  ...
  (table) => ({
    ...
    sourceTypeTenantIdx: uniqueIndex('idx_voucher_templates_source_tenant').on(
      table.sourceType,
      table.tenantId
    ),
  })
