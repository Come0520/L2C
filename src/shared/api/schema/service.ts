...
export const measureTasks = pgTable(
  'measure_tasks',
  {
    ...
    feeBreakdown: jsonb('fee_breakdown'), // 计费明细 JSON
...
export const measureItems = pgTable(
  'measure_items',
  {
    ...
    sheetId: uuid('sheet_id')
      .references(() => measureSheets.id, { onDelete: 'cascade' })
      .notNull(),
...
export const installTasks = pgTable(
  'install_tasks',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
...
export const installItems = pgTable(
  'install_items',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
    ...
    installTaskId: uuid('install_task_id')
      .references(() => installTasks.id, { onDelete: 'cascade' })
      .notNull(),
...
export const measureTaskSplits = pgTable(
  'measure_task_splits',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
