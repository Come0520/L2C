...
export function GlassIcons({ items, className }: GlassIconsProps) {
  return (
    <div className={cn('mx-auto grid grid-cols-3 gap-12 overflow-visible py-8', className)}>
...
function GlassIconButton({ item }: { item: GlassIconItem }) {
  const content = (
    <button
      ...
      className={cn(
        'relative cursor-pointer border-none bg-transparent outline-none',
        'h-[4.5em] w-[4.5em]',
    ...
    >
      ...
      <span
        className={cn(
          'absolute top-0 left-0 block h-full w-full rounded-[1.25em]',
      ...
      <span
        className={cn(
          'absolute top-0 left-0 h-full w-full rounded-[1.25em]',
      ...
      >
        <span className="m-auto flex h-[1.5em] w-[1.5em] items-center justify-center text-white">
      ...
      <span
        className={cn(
          ...
          'text-center text-sm leading-[2] font-medium whitespace-nowrap',
          'opacity-0 transition-[opacity,transform] duration-300',
          'translate-y-0 ease-[cubic-bezier(0.83,0,0.17,1)]',
