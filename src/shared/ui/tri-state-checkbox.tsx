...
export function TriStateCheckbox({
  ...
  const getStateStyles = () => {
    const baseStyles =
      'w-8 h-8 rounded-md flex items-center justify-center cursor-pointer transition-all duration-150';
  ...
  return (
    <button
      ...
      disabled={disabled}
      title={getTitle()}
      className={cn(getStateStyles(), disabled && 'cursor-not-allowed opacity-50', className)}
    >
      {getIcon()}
    </button>
  );
