...
CREATE TABLE IF NOT EXISTS order_assignment_history (
  ...
  old_assignee_id integer REFERENCES users(id) ON DELETE SET NULL,
  new_assignee_id integer NOT NULL REFERENCES users(id) ON DELETE SET NULL,
  assigned_by_id integer NOT NULL REFERENCES users(id) ON DELETE SET NULL,
...
CREATE OR REPLACE FUNCTION get_order_assignment_history(p_order_id integer)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT 
    ...
    u_old.real_name as old_assignee_name,
    u_new.real_name as new_assignee_name,
