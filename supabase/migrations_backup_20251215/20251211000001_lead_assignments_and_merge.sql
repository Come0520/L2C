...
-- 如果表已存在（由 init_schema 创建），进行列迁移
DO $$
BEGIN
    -- 1. 处理 user_id -> assigned_to_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'user_id') THEN
        ALTER TABLE lead_assignments RENAME COLUMN user_id TO assigned_to_id;
    END IF;

    -- 2. 处理 assigned_by -> assigned_by_id
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'assigned_by') THEN
        ALTER TABLE lead_assignments RENAME COLUMN assigned_by TO assigned_by_id;
    END IF;

    -- 如果没有 assigned_by_id (既没重命名也没原生)，则添加
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'assigned_by_id') THEN
         ALTER TABLE lead_assignments ADD COLUMN assigned_by_id UUID REFERENCES users(id) ON DELETE SET NULL;
    END IF;

    -- 放宽 assigned_by_id 约束 (如果来自旧表可能是 NOT NULL)
    ALTER TABLE lead_assignments ALTER COLUMN assigned_by_id DROP NOT NULL;

    -- 3. 处理 assigned_at -> created_at
    -- 如果有 assigned_at 且没有 created_at，则重命名
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'assigned_at') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'created_at') THEN
            ALTER TABLE lead_assignments RENAME COLUMN assigned_at TO created_at;
        END IF;
    END IF;

    -- 4. 添加缺失列
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'created_at') THEN
        ALTER TABLE lead_assignments ADD COLUMN created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'updated_at') THEN
        ALTER TABLE lead_assignments ADD COLUMN updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'assignment_method') THEN
        ALTER TABLE lead_assignments ADD COLUMN assignment_method VARCHAR(20) DEFAULT 'manual';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lead_assignments' AND column_name = 'reason') THEN
        ALTER TABLE lead_assignments ADD COLUMN reason TEXT;
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS lead_merge_history (
    ...
    duplicate_lead_ids UUID[] NOT NULL,
